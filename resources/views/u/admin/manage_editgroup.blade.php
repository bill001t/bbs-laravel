...
@foreach ($allGroups as $item)
					...
					<option value="{{ $item['gid'] }}"{{ App\Core\Tool::isSelected($info['groupid'] == $item['gid']) }}>{{ $item['name'] }}</option>
					<!--# } #-->
					<option value="0"{{ App\Core\Tool::isSelected($info['groupid'] == 0) }}>普通组</option>
				</select>
			</td>
			...
			<td>
				<div class="cross">
					<ul id="J_u_group_default">
						...
						foreach ($allGroups as $item) {
								if (App\Core\Tool::inArray($item['gid'], $defaultGroups)) continue;
								$check = $canCheck = '';;
								$disabled = 'disabled=disabled';
								$disableclass = 'disabled';
								if (App\Core\Tool::inArray($item['gid'], $myGids)) {
									$check = 'checked=checked';
									$disabled = $disableclass = '';
								}
								if (App\Core\Tool::inArray($info['groupid'], $defaultGroups)) {
									$canCheck = 'disabled=disabled';
								}
								$endtime = $userGroups[$item['gid']]['endtime'] ? App\Core\Tool::time2Str($userGroups[$item['gid']]['endtime'], 'Y-m-d H:i') : '';
						#-->
						<li>
