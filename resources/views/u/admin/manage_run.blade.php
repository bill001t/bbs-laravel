...
<div class="wrap">
	...
	<div class="search_type cc mb10">
		...
		<select name="gid[]" size="5" class="mr10" multiple>
			<option value="-1"{{ App\Core\Tool::isSelected(App\Core\Tool::inArray(-1, $args['gid'])) }}>不限制</option>

@foreach($groups as $item)

				<option value="{{ $item['gid'] }}"{{ App\Core\Tool::isSelected(App\Core\Tool::inArray($item['gid'], $args['gid'])) }}>{{ $item['name'] }}</option>
			<!--#}#-->
		</select>
