...
<div class="table_full">
	<table width="100%">
		...
		<tr>
			<th>头像</th>
			<td><div><img id="J_face_img" class="J_avatar" data-type="middle" src="{{ App\Core\Tool::getAvatar($info['uid']) }}?t={{ App\Core\Tool::getTime() }}" class="face"></div><a id="J_set_def" href="{{ url('u/manage/defaultAvatar?uid=' . $info['uid']) }}">[恢复为系统头像]</a></td>
		</tr>
		<tr>
			<th>新密码</th>
		...
		<tr>
			<th>注册时间</th>
			<td>
				<input name="regdate" type="text" class="input length_5 J_datetime date" value="{{ App\Core\Tool::time2str($info['regdate'], 'Y-m-d H:i:s') }}">
			</td>
		</tr>
		<tr>
			<th>注册IP</th>
		...
		<tr>
			<th>最后登录时间</th>
			<td>{{ App\Core\Tool::time2str($info['lastvisit'], 'Y-m-d H:i:s') }}
			</td>
		</tr>
		<tr>
			<th>在线时间（小时）</th>
		...
		<tr>
			...
			<td>
				<select name="gender" class="select_5">
					<option value="1" {{ App\Core\Tool::isSelected($info['gender'] == 1) }}>女</option>
					<option value="0" {{ App\Core\Tool::isSelected($info['gender'] == 0) }}>男</option>
				</select>
			</td>
		</tr>
		<tr>
...
@foreach ($workList as $_item)
					...
					<p style="line-height:26px;">于{$_item['starty']}年{$_item['startm']}月在《{$_item['company']}》工作至
@if(!$_item['endy'])
今
@else
{$_item['endy']}年{$_item['endm']}月<!--#}#--></p>
				<!--#}#-->
			</td>
		</tr>
		<tr>
	...
	$('#J_set_def').on('click', function(e){
		...
		$.post(this.href, function(data){
			if(data.state == 'success') {
				$('#J_face_img').attr('src', {{ asset('assets/images') }}/face/face_middle.jpg');
				$this.hide();
			}else{
				//common.js
				resultTip({
