...
@foreach ($credits as $key => $credit)
			...
			<tr data-key="{{ $key }}">
				...
				<!-- <td><input name="credits[{{ $key }}][open]" type="checkbox" value="1"{{ App\Core\Tool::ifcheck($credit['open']) }}></td> -->

@if($key > 4)

				<td><a href="{{ url('windid/credit/doDeletecredit') }}" class="mr10 J_ajax_del" data-pdata="{'creditId': '{{ $key }}'}">[删除]</a></td>
