...
<!--#}#-->
	...
	<form class="J_ajaxForm J_loading_show" action="{{ url('windid/areadata/update') }}" method="post" style="display:none;">
...
@if ($hasLevel < 2)
					...
					<a href="{{ url('windid/areadata/run?parentid=' . $_i['areaid']) }}" class="mr10 J_region_next" data-name="{{ $_i['name'] }}" data-type="{{ $data_role }}" data-id="{{ $_i['areaid'] }}">[{{ $data_name }}]</a>
					<!--#}#-->
					<a data-id="{{ $_i['areaid'] }}" data-role="{{ $data_role }}" href="{{ url('windid/areadata/delete?areaid=' . $_i['areaid']) }}" class="J_region_del" data-pdata="{'areaid': {{ $_i['areaid'] }}}">[删除]</a></td>
			</tr>
<!--#}#-->
		</tbody>
		</table>
...
<div class="core_pop_wrap" id="J_region_pop" style="display:none;">
	<div class="core_pop">
		<form action="{{ url('windid/areadata/run') }}" method="post">
...
Wind.use('dialog', 'draggable', function() {
	...
	//table列表中省市区的html
	var tr_province = '<tr><th data-id="_ID"><div class="J_items">_TEXT</div></th><td><a data-id="_ID" data-type="province" class="mr10 J_region_next" data-name="_PNAME" href="#">[下级城市]</a><a class="J_region_del" data-pdata="{{ \'areaid\': \'_ID\' }}" data-role="province" href="{{ url('windid/areadata/delete?areaid=_ID') }}" data-id="_ID">[删除]</a></td></tr>',
		tr_city = '<tr><th data-id="_ID"><div class="J_items">_TEXT</div></th><td><a data-id="_ID" data-type="city" class="mr10 J_region_next" data-name="_CNAME" href="#">[下级区县]</a><a class="J_region_del" data-pdata="{{ \'areaid\': \'_ID\' }}" data-role="city" href="{{ url('windid/areadata/delete?areaid=_ID') }}" data-id="_ID">[删除]</a></td></tr>',
		tr_district = '<tr><th data-id="_ID"><div class="J_items">_TEXT</div></th><td><a class="J_region_del" data-pdata="{{ \'areaid\': \'_ID\' }}" data-role="district" href="{{ url('windid/areadata/delete?areaid=_ID') }}" data-id="_ID">[删除]</a></td></tr>';
