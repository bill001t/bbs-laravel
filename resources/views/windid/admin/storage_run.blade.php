...
<div class="prompt_text">
	<ul>
		...
		<li>如果服务端更改了存储设置，请在客户端同步提交设置。</li>
	</ul>
</div>

<form method="post" class="J_ajaxForm" action="{{ url('windid/storage/dostroage') }}" data-role="list">
...
#-->
						<li><span class="mr20"><label><input name="att_storage" data-msg="
$value['description']}" value="{{ $var }}" type="radio" {{ App\Core\Tool::ifcheck($storageType==$var) }}>{{ $value['name'] }}</label></span>

@if($value['managelink']): #-->
							<a href="
url($value['managelink'])|url}">[设置]</a>
<!--# endif; #-->
						</li>
<!--# endforeach; #-->
					</ul>
