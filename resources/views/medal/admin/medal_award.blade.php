...
@foreach ($list as $log)
$expired = $log['expired_time'] > 0 ? App\Core\Tool::time2str($log['expired_time']) : '长期'; #-->
			<tr>
				...
				<td>{{ App\Core\Tool::time2str($log['created_time']) }}</td>
				<td>{{ $expired }}</td>
...
$(function(){
	//勋章数据，1 2是key
	var MEDAL_JSON = {{ Security::escapeEncodeJson($jsonMedals) }};
