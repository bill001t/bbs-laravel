...
<!--#}}#-->
		...
		<div class="current">
			<h2>我当前的用户组 - {{ $allGroups[Core::getLoginUser()->gid]['name'] }}</h2>
			<ul>
				<li>我的积分{{ $myCredit }}</li>
...
@if ($compare)
		...
		<div class="contrast">
			...
			<ul>
				<li>
@if ('member' == $allGroups[$comparegid]['type'])
升级所需积分{$allGroups[$comparegid]['points']}<!--#}#--></li>
...
@foreach ($switchGroups as $_gid)
						...
						<li><label><input type="radio" value='{{ $_gid }}' name='gid' {{ App\Core\Tool::ifcheck($_gid == Core::getLoginUser()->gid) }}/><span>{{ $allGroups[$_gid]['name'] }}</span></label></li>
<!--#}#-->
					</ul>
				</div>
