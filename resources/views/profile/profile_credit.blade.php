...
@foreach ($creditBo->cType as $key => $value)
				...
				<dd>
					...
					<span class="num">{{ Core::getLoginUser()->info['credit' . $key] }} {{ $creditBo->cUnit[$key] }}</span>
					<span class="operate">
@if (isset($transfer[$key]) && $transfer[$key]['ifopen'])
<a href="javascript:;" class="mr5 J_credit_transfer" data-id="{{ $key }}">[积分转账]</a><!--# } #-->
@if (isset($exchange[$key]))
<a href="#" class="mr5 J_credit_change" data-id="{{ $key }}">[转换为其他积分]</a><!--# } #-->
					</span>
				</dd>
				<!--# } #-->
			</dl>
