<textarea name="atc_content" id="J_wind_editor"
          style="height:300px;width:100%;padding:0;margin:0;">{{ $atc_content or '' }}
</textarea>
...
<script>
    ...
    var POST_TYPE = Post_Type_Enum['{{ $postType }}'];
    var EDIT_CONFIG = {
        sell: {!! App\Core\Tool::array2str($sellConfig) !!},
        enhide: {!! App\Core\Tool::array2str($enhideConfig) !!},
        ifhide: '1'
    };
    var IMAGE_CONFIG = {
        tabs: ['local', 'album', 'network'],
        filetype: {!! App\Core\Tool::array2str(App\Core\Tool::subArray(Core::C('attachment', 'extsize'), array('jpg', 'jpeg', 'png', 'gif', 'bmp'))) !!},
        uploadUrl: "{{ url('bbs/upload/dorun?_json=1') }}",//上传地址
        postData: {fid: '{{ $fid }}', uid: '{{ Core::getLoginUser()->uid }}'}
    };
    var ATTACH_CONFIG = {
        ifopen: '{{ $allowUpload }}',
        uploadUrl: "{{ url('bbs/upload/dorun?_json=1') }}",//上传地址
        ...
        list: '{!! App\Core\Tool::array2str(isset($attach) ? $attach : '') !!}',
        postData: '{fid: "{{ $fid }}", csrf_token: GV.TOKEN}',
        remoteAttUrl: '{{ $db_ftpweb or ''}}',
        filetype: {!! App\Core\Tool::array2str(Core::C('attachment', 'extsize')) !!},
        atUserList: [{{ $atusers or ''}}]
    };
    //表情数据的地址
    ...
    //应用插件配置
    var editorApp = {
                root: '{{ asset('assets/themes/extres') }}/ ',
                items: {{ App\Core\Security::escapeEncodeJson($editor_app_config) }},
            }
            ;
    /*
     * 引用编辑器js并初始化编辑器
    ...
    Wind.ready('global.js', function () {
        Wind.use('jquery', 'windeditor', function () {
            ...
            // @提醒权限
            var REMIND_OPEN = '{{ Core::getLoginUser()->getPermission('remind_open') }}',
                    VIDEO_OPEN = '{{ Core::getLoginUser()->getPermission('allow_publish_vedio') }}',
                    MUSIC_OPEN = '{{ Core::getLoginUser()->getPermission('allow_publish_music') }}';
            //根据服务器端配置判断是否加载编辑器功能插件
            var customPlugins = [];
            if (ATTACH_CONFIG.ifopen == 1) {
                customPlugins.push('insertFile');
            }
            if (VIDEO_OPEN == 1) {
                customPlugins.push('insertVideo');
            }
            if (MUSIC_OPEN == 1) {
                customPlugins.push('insertMusic');
            }
            if (EDIT_CONFIG.enhide.ifopen == 1) {
                customPlugins.push('insertHide');
            }
            if (EDIT_CONFIG.sell.ifopen == 1) {
                customPlugins.push('insertSell');
            }
            if (REMIND_OPEN == 1) {
                customPlugins.push('atremind');
            }
            //插入代码和openApp的插件放最后
            ...
            $('#J_wind_editor').windeditor({
                ...
                plugins: plugins,
                defaultToolbarMode: '{{ Core::C('bbs','editor.style') == 1 ? 'mini' : 'default' }}',//编辑器的默认模式，有default和mini模式
                mini: mini.join(' '),
                theme: 'buyiui',
                localSave: 'POST_TYPE === Post_Type_Enum.NEW_POST ? true : false',// 发新帖才保存本地，IE本地存储还有问题
                iframeCss: ['{{ asset('assets/themes/site/default/css/dev/editor_content.css') }}'],//为了更好的可见即所得，加载阅读页的css文件
                onInit: function () {
