...
@foreach ($threadb as $key => $value)
				...
				<tr>
					...
					<td width="600">
						<div class="mb5">
							<a href="{{ url('bbs/thread/run?fid=' . $value['fid']|pw) }}" class="mr5" target="_blank">[{!! $forumlist[$value['fid']]['name'] !!}]</a><a href="{{ url('bbs/read/run?tid=' . $value['tid'] . '&fid=' . $value['fid']|pw) }}" target="_blank">{{ $value['subject'] }}</a>
						</div>
						<div class="mb5">
					...
					<td>
						<ul class="single_list cc">
							<li><a class="J_ajax_refresh mr5" href="{{ url('bbs/contentcheck/doPassThread') }}" data-pdata="{'tid': {{ $value['tid'] }}}">[通过]</a></li>
							<li><a class="J_ajax_del" href="{{ url('bbs/contentcheck/doDeleteThread') }}" data-pdata="{'tid': {{ $value['tid'] }}}">[删除]</a></li>
						</ul>
					</td>
				</tr>
				<!--# } #-->
