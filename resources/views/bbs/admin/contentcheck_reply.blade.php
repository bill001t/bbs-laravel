...
@foreach ($postdb as $key => $value)
					...
					<!--# !$value['subject'] && $value['subject'] = App\Core\Tool::substrs($value['content'], 26, 0, true); #-->
				<tr>
					<td width="10"><input type="checkbox" name="pid[]" value="{{ $value['pid'] }}" class="J_check" data-yid="J_check_y" data-xid="J_check_x"></td>
					<td width="600">
						<div class="mb5">
							<a href="{{ url('bbs/thread/run?fid=' . $value['fid']|pw) }}" class="mr5" target="_blank">[{!! $forumlist[$value['fid']]['name'] !!}]</a><a href="{{ url('bbs/read/run?tid=' . $value['tid'] . '&fid=' . $value['fid']|pw) }}" target="_blank" title="{{ $value['content'] }}">{{ $value['subject'] }}</a>
						</div>
						<div class="mb5">
							作者：<a href="{{ url('space/index/run?uid=' . $value['created_userid']|pw) }}" target="_blank" class="mr15">{{ $value['created_username'] }}</a>发帖时间：{{ App\Core\Tool::time2str($value['created_time']) }}
						</div>
					...
					<td>
						<ul class="single_list cc">
							<li><a href="{{ url('bbs/contentcheck/doPassPost') }}" class="mr5 J_ajax_refresh" data-pdata="{'pid': {{ $value['pid'] }}}">[通过]</a></li>
							<li><a class="J_ajax_del" data-pdata="{'pid': {{ $value['pid'] }}}" href="{{ url('bbs/contentcheck/doDeletePost') }}">[删除]</a></li>
						</ul>
					</td>
				</tr>
				<!--# } #-->
