...
<html>
	<head>
		...
		<link href="{{ asset('assets/themes/site/default/css/dev') }}/admin_layout.css" rel="stylesheet" />
		<style>
			...
			<table width="100%" height="100%" style="table-layout:fixed;">
				<tr class="head">
					...
					<div class="login_info">
						<a href="{{ url('index/run|pw') }}" class="home" target="_blank">前台首页</a><span class="mr10">管理员： {{ Core::getLoginUser()->username }}</span><a href="{{ url('index/logout') }}" class="mr10">[注销]</a>
					</div></td>
				</tr>
				<tr class="tab">
					<th>
					<div class="search">
						...
						<button type="button" name="keyword" id="J_search" value="" data-url="{{ url('/find/run') }}">搜索</button>
					</div></th>
					<td>
				...
				<tr class="content">
					...
					<td id="B_frame">
						...
						<iframe id="iframe_default" src="{{ url(Core::app()->homeLink) }}" style="height: 100%; width: 100%;display:none;" data-id="default" frameborder="0" scrolling="auto"></iframe>
					</td>
				</tr>
			</table>
		</div>
...
var USUALL = [], /*常用的功能模块*/
	...
	SUBMENU_CONFIG = {{ Security::escapeEncodeJson($menus) }},/*主菜单区*/
	imgpath = '{!! asset('assets/images') !!}',
	times = 0,
	getdescurl = '{!! $getdesc !!}',
	searchurl = '{!! $search !!}',
	token = "{{ $token }}";
...
<!--#
$isFounder =  app('ADMIN:service.srv.AdminFounderService')->isFounder(Core::getLoginUser()->username);
if ($isFounder) {
...
$siteurl = rawurlencode(Core::C('site', 'info.url'));
$version = rawurlencode(NEXT_VERSION);
$uid = Core::getLoginUser()->uid;
$ts = PW::getTime();
$sitehash = Core::C('site', 'hash');
