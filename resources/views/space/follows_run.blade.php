...
@foreach ($follows as $key => $value)
				...
				<dl class="cc">
					<dt><a href="{{ url('space/index/run?uid=' . $value['touid']) }}" data-uid="{{ $value['touid'] }}" class="J_user_card_show"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($value['touid'], 'small') }}" data-type="small" width="50" height="50" /></a><a rel="nofollow" href="{{ url('message/message/pop?username=' . $value['username']) }}" data-name="{{ $value['username'] }}" class="called J_send_msg_pop J_qlogin_trigger">打招呼</a></dt>
					<dd>
						<div class="title">
