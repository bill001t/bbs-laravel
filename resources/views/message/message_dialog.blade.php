							...
							<div class="hd cc">
									<!--# $dialogUsername = $dialog['last_message']['from_uid'] == $dialog['to_uid'] ? 
											...
											$dialogUserid = $dialog['last_message']['from_uid'] == $dialog['to_uid'] ? 
												...
												if ($blackList) {
													$isBlack = App\Core\Tool::inArray($dialogUserid,$blackList) ? true : false;
												}
									 #-->
									<span class="fr">
...
@else
						...
						<dl class="cc message_reply">
							<dt><a href="{{ url('space/index/run?uid=' . $dialog['to_uid']) }}" class="face J_user_card_show" data-uid="{{ $dialog['to_uid'] }}"  target="_blank"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($dialog['to_uid'],'small') }}" data-type="small" width="50" height="50" alt="" /></a></dt>
							<dd>
...
$value['tousername'] = $value['from_uid'] == $dialog['to_uid'] ? '我' : $dialogUsername;
							...
							#-->
							<dl class="cc{{ $bubbles }}">
								<dt><a href="{{ url('space/index/run?uid=' . $value['from_uid']) }}" class="face J_user_card_show" data-uid="{{ $value['from_uid'] }}" target="_blank"><img src="{{ App\Core\Tool::getAvatar($value['from_uid'],'small') }}" data-type="small" width="50" height="50" onerror="this.onerror=null;this.src={{ asset('assets/images') }}/face/face_small.jpg'" alt="{{ $value['tousername'] }}" /></a></dt>
								<dd>
									<div class="arrow"><em></em><span></span></div>
									<div class="title">
										<a href="{{ url('space/index/run?uid=' . $value['from_uid']) }}" class="b J_user_card_show" data-uid="{{ $value['from_uid'] }}"  target="_blank">{{ $value['tousername'] }}</a>：{!! $value['content'] !!}
									</div>
									<div class="info">
...
@if ($value['from_uid'] != $dialog['to_uid'] && Core::getLoginUser()->getPermission('allow_report'))

									 | <a href="{{ url('report/index/report?type=message&type_id=' . $value['message_id']) }}" data-pid="{{ $value['message_id'] }}" class="J_report">举报</a>
									<!--# } #-->
									 </span><span class="time" title="{{ App\Core\Tool::time2str($value['created_time'], 'Y-m-d H:i') }}">{{ App\Core\Tool::time2str($value['created_time'], 'auto') }}</span></div>
								</dd>
							</dl>
							<!--# } #-->
						</div>
