...
@else
		...
		<dd class="notice_segment_cont">
			<div class="summary"> {!! $replyUser !!} 回复了您的回帖《<a href="{{ url('bbs/read/jump?tid=' . $v['extend_params']['threadId'] . '&pid=' . $v['extend_params']['pid']) }}" target="_blank">{{ $v['extend_params']['postTitle'] }}</a>》
			</div>
...
<!--弹窗详情-->
	...
	<div class="notice_segment_wrap">
		<dl class="notice_segment_list cc">
			...
			<dd class="notice_segment_cont">
				<div class="summary"> {!! $replyUser !!} 回复了您的回帖《<a href="{{ url('bbs/read/jump?tid=' . $notice['extend_params']['threadId'] . '&pid=' . $notice['extend_params']['pid']) }}" target="_blank">{{ $notice['extend_params']['postTitle'] }}</a>》
				</div>
	...
	<div class="my_message_bottom">
		<a href="{{ url('message/notice/ignore?id=' . $notice['id'] . '&ignore=' . $doIgnore) }}" data-id="{{ $notice['id'] }}" data-type="此帖的回复" data-ignore="{{ $doIgnore }}" data-tid="{{ $notice['extend_params']['postId'] }}" class="fr J_notice_ignore">{{ $ignoreString }}</a>
...
@else
				...
				<!--# } #-->
				</span> {!! $replyUser !!} 回复了您的回帖《<a href="{{ url('bbs/read/jump?tid=' . $v['extend_params']['threadId'] . '&pid=' . $v['extend_params']['pid']) }}" target="_blank">{{ $v['extend_params']['postTitle'] }}</a>》
			</div>
			<div class="title J_notice_all" style="display:none;"></div>
			<div class="c"></div>
			<div class="info"><span class="time"><!--# echo App\Core\Tool::time2str($v['modified_time'],'auto');#--></span><span class="operating"><span class="line">|</span><a class="J_msg_del" href="#" data-uri="{{ url('message/notice/delete') }}" data-pdata="{'id':{{ $v['id'] }}}">删除</a><span class="line">|</span><a href="{{ url('message/notice/ignore?id=' . $v['id'] . '&ignore=' . $doIgnore) }}" data-id="{{ $v['id'] }}" data-type="{{ $type }}" data-role="reply" data-tid="{{ $v['extend_params']['postId'] }}" class="J_notice_ignore">{{ $ignoreString }}</a></span></div>
		</div>
	</div>
</hook-action>
...
<!--页详情-->
	...
	#-->
			...
			<span class="notice_tip_icon">
			<span class="icon_system" title="已读"></span>
			</span> {!! $replyUser !!}&nbsp;回复了您的回帖《<a href="{{ url('bbs/read/run?tid=' . $notice['extend_params']['threadId']) }}#{{ $notice['param'] }}" target="_blank">{{ $notice['extend_params']['postTitle'] }}</a>》</p>
</hook-action>