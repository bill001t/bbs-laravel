...
list($v['type']) = explode('_',$v['type']);
	...
	#-->
					<li data-url="{{ url('message/notice/detail?id=' . $v['id']) }}" class="li{{ $class }}"><i></i>
						{{-- <segment alias='notice_minilist' tpl="$typeTpl" args='$v' name="minilist" /> --}}
					</li>
	<!--# }#-->
				</ul>
...
@else
	...
	<!--# } #-->
			<div class="my_message_bottom tac">
				<a href="{{ url('message/message/run') }}">查看所有消息&nbsp;&gt;&gt;</a>
			</div>
		</div>
