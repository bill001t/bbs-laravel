...
<!--弹窗列表-->
	<!--# Wind::import('LIB:ubb.PwUbbCode');
		...
		$v['title'] = App\Core\Tool::substrs(WindSecurity::escapeHTML($v['message_extend_params']['title']), 32);
		$v['title'] = PwUbbCode::parseEmotion($v['title']);
	#-->
		<div class="fe"><img src="{{ App\Core\Tool::getAvatar($from_uid, 'small') }}" data-type="small" width="50" height="50" onerror="this.onerror=null;this.src={{ asset('assets/images') }}/face/face_small.jpg'" alt="{{ $username }}" /></div>
		<div class="ct"> {!! $username !!}：{!! $v['title'] !!}&nbsp;
@if ($v['message_extend_params']['unread_count'])
<span class="org J_unread_multi" data-unread="{{ $v['message_extend_params']['unread_count'] }}">({$v['message_extend_params']['unread_count']}条未读)</span><!--# } #-->
			<div class="io">
				<span class="num">共{$v['message_extend_params']['message_count']}条</span>
				<span class="time"><!--#echo App\Core\Tool::time2str($v['modified_time'],'auto');#--></span></div>
		</div>
</hook-action>
...
@if ($detailList['dialog']['unread_count'])
	...
	<!--# 
		...
		if (App\Core\Tool::inArray($notice['extend_params']['from_uid'],$blacklist)) {
		#-->
...
$username = $v['from_uid'] == Core::getLoginUser()->uid ? '我' : $notice['extend_params']['from_username'];
		$bubbles = $v['from_uid'] == Core::getLoginUser()->uid ? 'my cc' : 'cc';
	...
	#-->
		<div class="{{ $bubbles }}">
			<div class="face"><a href="{{ url('space/index/run?uid=' . $v['from_uid']) }}"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($v['from_uid'], 'small') }}" data-type="small" width="25" height="25" onerror="this.onerror=null;this.src={{ asset('assets/images') }}/face/face_small.jpg'" alt="{{ $username }}" /></a></div>
			<div class="bubble">
				<div class="arrow"><em></em><span></span></div>
				<a href="{{ url('space/index/run?uid=' . $v['from_uid']) }}" class="b">{{ $username }}</a>：{!! $v['content'] !!}
				<div class="io"><span class="time"><!--# echo App\Core\Tool::time2str($v['created_time'],'auto'); #--></span></div>
			</div>
		</div>
	<!--# } #-->
	</div>
