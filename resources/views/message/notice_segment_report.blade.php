...
@if ($notice['extend_params']['operateUsername'])
	...
	<div class="tips" style="border-left:0;border-right:0;border-top:0;">管理日志：<a href="{{ url('space/index/run?uid=' . $notice['extend_params']['operateUserId']) }}" target="_blank">{{ $notice['extend_params']['operateUsername'] }}</a> 在<!--# echo App\Core\Tool::time2str($notice['extend_params']['operateTime'],'auto');#-->已{$notice['extend_params']['operateType']}举报内容</div>
	<!--# } #-->
...
@else
			...
			<div class="c"></div>
			<div class="info">
				<span class="time"><!--# echo App\Core\Tool::time2str($v['modified_time'],'auto');#--></span><span class="operating"><span class="line">|</span><a class="J_msg_del" href="#" data-uri="{{ url('message/notice/delete') }}" data-pdata="{'id':{{ $v['id'] }}}">删除</a></span>
			</div>
		</div>
	</div>
</hook-action>
...
@if ($notice['extend_params']['operateUsername'])
			...
			管理日志：<a href="{{ url('space/index/run?uid=' . $notice['extend_params']['operateUserId']) }}" target="_blank">{{ $notice['extend_params']['operateUsername'] }}</a> 在<!--# echo App\Core\Tool::time2str($notice['extend_params']['operateTime'],'auto');#-->已{$notice['extend_params']['operateType']}举报内容
			<!--# } #-->
			<!-- 已处理举报end -->
</div>
</hook-action>