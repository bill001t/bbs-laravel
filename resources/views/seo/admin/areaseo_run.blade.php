...
<li>页面名称:{pagename}（应用范围：门户页面）</li>
</ol>

以上标签（必须包含大括号"{{  }}"）可以通过添加在下面来优化页面SEO设置，多个标签之间可以用半角连字符"-"、半角","或半角空格隔开。留空为默认SEO设置

</div>
<form action="{{ url('/seo/manage/doRun?mod=area') }}" method="post" class="J_ajaxForm">

@foreach($pages as $alias => $title)
if($alias == 'custom') {
			foreach($list as $v) {
#-->
	<div class="h_a">{{ $v['title'] }}</div>
	<div class="table_full">
		<table width="100%">
			...
			<tr>
				<th>title [标题]</th>
				<td>
					<input data-id="{{ $alias }}" name="seo[{{ $alias }}][{{ $v['id'] }}][title]" type="text" class="input length_5 J_seo_input" value="{{ $seo[$alias][$v['id']]['title'] }}">
				</td>
			...
			<tr>
				...
				<td>
					<input data-id="{{ $alias }}" name="seo[{{ $alias }}][{{ $v['id'] }}][description]" type="text" class="input length_5 J_seo_input" value="{{ $seo[$alias][$v['id']]['description'] }}">
				</td>
				<td><div class="fun_tips"></div></td>
			</tr>
			<tr>
				<th>keywords [关键字]</th>
				<td>
					<input data-id="{{ $alias }}" name="seo[{{ $alias }}][{{ $v['id'] }}][keywords]" type="text" class="input length_5 J_seo_input" value="{{ $seo[$alias][$v['id']]['keywords'] }}">
				</td>
				<td><div class="fun_tips"></div></td>
			</tr>
		</table>
