...
<div class="wrap">
	<div class="nav">
		<ul class="cc">
			<li class="{{ App\Core\Tool::isCurrent($type == 3) }}"><a href="{{ url('admin/windidclient/schooldata/run?type=3') }}">大学管理</a></li>
			<li class="{{ App\Core\Tool::isCurrent($type == 2) }}"><a href="{{ url('admin/windidclient/schooldata/run?type=2') }}">中学管理</a></li>
			<li class="{{ App\Core\Tool::isCurrent($type == 1) }}"><a href="{{ url('admin/windidclient/schooldata/run?type=1') }}">小学管理</a></li>
		</ul>
	</div>
...
$_var = $type == 3 ? 'province' : 'other';
	...
	<form class="J_ajaxForm" action="{{ url('windidclient/schooldata/update') }}" method="post">
	<div class="table_full">
...
@foreach ($list as $_i)
			...
			<tr>
				...
				<td><a href="{{ url('windidclient/schooldata/delete?schoolid=' . $_i['schoolid']) }}" class="J_school_del">[删除]</a></td>
			</tr>
<!--#}#-->
		</tbody>
		</table>
...
<script>
var SCHOOL_DEL = "{{ url('windidclient/schooldata/delete') }}"; //schoolid
