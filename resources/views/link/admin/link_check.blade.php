...
@foreach ($typesList as $k => $v)
							...
							<li><label><input class="J_cate_check" data-text="{{ $v['typename'] }}" type="checkbox" name="data[{{ $value['lid'] }}][typeid][]" value="{{ $v['typeid'] }}" {{ App\Core\Tool::ifcheck(App\Core\Tool::inArray($v['typeid'], $value['typeids'])) }}>{{ $v['typename'] }}</label></li>
							<!--# } #-->
						</ul>
					</div>
