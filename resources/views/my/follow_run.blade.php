...
{{-- @include('common.header') --}}
	<div class="main_wrap">
		<div class="bread_crumb">
			...
			<a href="{{ url('my/follow/run') }}">我关注的人</a>
@if($type)
<em>&gt;</em><a href="{{ url('my/follow/run?type=' . $type) }}">{{ $typeArr[$type] }}</a><!--#}#-->
		</div>
		<div class="main cc">
			<div class="main_body">
...
$birthday = $value['byear'] ? '生日：' . $value['byear'] . '年'.$value['bmonth'].'月'.$value['bday'].'日' : '';
							...
							<dl class="cc J_friends_items">
								<dt><a href="{{ url('space/index/run?uid=' . $value['touid']) }}" data-uid="{{ $value['touid'] }}" class="J_user_card_show"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($value['touid'], 'small') }}" data-type="small" width="50" height="50" alt="{{ $value['username'] }}" /></a><a href="{{ url('message/message/pop?username=' . $value['username']) }}" data-name="{{ $value['username'] }}" class="called J_send_msg_pop">打招呼</a></dt>
								<dd>
									<div class="title">
...
$userTypeNames = implode(',',$userType[$value['touid']]);
										 ...
										 #-->
										<span class="J_group_names" title="{{ $userTypeNames }}">
@if ($userType[$value['touid']])
{@FollowController::bulidGroup($userType[$value['touid']])}
@else
未分组<!--# } #--></span><em class="core_arrow"></em></a>
									</div>
									<div class="attribute">

@if (isset($fans[$value['touid']]))
...
@foreach ($recommend as $value)
								...
								<li><a data-uid="{{ $value['uid'] }}" class="J_user_card_show" href="{{ url('space/index/run?uid=' . $value['uid']) }}"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($value['uid'], 'middle') }}" data-type="middle" width="90" height="90" alt="{{ $value['username'] }}" /></a><label title="{{ $value['username'] }}"><input class="J_check" type="checkbox" name="uids[]" value="{{ $value['uid'] }}">{{ $value['username'] }}</label></li>
								<!--# } #-->
							</ul>
							<div class="ft">
				...
				<div class="box_wrap">
					...
					<div class="side_cate_list">
						<ul id="J_side_group_list" class="cc">
							<li>
								<a href="{{ url('my/follow/run') }}" class="title"><span class="">全部</span><em>({Core::getLoginUser()->info['follows']})</em></a>
							</li>
