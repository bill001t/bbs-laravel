	...
	<!--#
	 ...
	 #-->
		<div class="J_friend_maybe_items">
			<dl>
				<dt><a href="{{ url('space/index/run?uid=' . $pUid) }}" class="J_user_card_show" data-uid="{{ $pUid }}"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($pUid,'small') }}" width="50" height="50" data-uid="{{ $pUid }}" data-type="small" /></a></dt>
				<dd>
...
@if ($v['cnt'])
					...
					<a href="{{ url('my/follow/samefriend?uid=' . $pUid) }}" class="unmore J_friend_view" data-uid="{{ $pUid }}">{{ $v['cnt'] }}个共同好友{!! $arrow !!}</a>
					<!--# } #-->
					</p>
				</dd>
			</dl>
