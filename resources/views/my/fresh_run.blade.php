...
<head>
@include('common.head')
<link href="{{ asset('assets/themes/site/default/css/dev/fresh.css') }} "rel="stylesheet" />
<style>
.aPre{
...
{{-- @include('common.header') --}}
	<div class="main_wrap">
		<div class="bread_crumb">
			...
			<a href="{{ url('my/fresh/run') }}">我的关注</a>
@if($currents != 'all')
<em>&gt;</em><a href="{{ url('my/fresh/run?gid=' . $currents) }}">{{ $type[$currents] }}</a><!--#}#-->
		</div>
		<div class="main cc">
			<div class="main_body">
...
<span>（您已经{$unpost}没发帖了）</span><!--# } #-->
					...
					<div class="box_wrap fresh_page">
						<div class="fresh_nav">
							<ul class="cc">
								<li class="{{ App\Core\Tool::isCurrent($currents=='all') }}"><a href="{{ url('my/fresh/run/') }}">全部</a></li>

@foreach ($type as $key => $value)

								<li class="line"></li>
								<li class="{{ App\Core\Tool::isCurrent($currents==$key) }}"><a href="{{ url('my/fresh/run?gid=' . $key) }}">{{ $value }}</a></li>
								<!--# } #-->
							</ul>
						</div>
...
Wind.use('jquery', 'global', 'swfupload', 'ajaxForm', function(){
	//支持的图片
	var filetype ={!! App\Core\Tool::array2str(App\Core\Tool::subArray(Core::C('attachment', 'extsize'), array('jpg', 'jpeg', 'png', 'gif', 'bmp'))) !!},
		sup_pic = false;
	for(i in filetype) {
		if(i) {
			sup_pic = true;
