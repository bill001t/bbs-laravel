...
{{-- @include('common.header') --}}
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<a href="{{ url() }}" class="home" title="{{ Core::C('site', 'info.name') }}">首页</a><em>&gt;</em><a href="{{ url('tag/index/run') }}">话题</a><em>&gt;</em><a href="{{ url('tag/index/view?name=' . $tag['tag_name']) }}">{{ $tag['tag_name'] }}</a>
		</div>
			...
			<div class="main_body">
				<div class="main_content">
					<div class="box_wrap tag_info_box cc">
						<div class="banner"><img src="{{ App\Core\Tool::getPath($tag['tag_logo']) }}" width="110" alt="{{ $tag['tag_name'] }}" onerror="this.onerror=null;this.src={{ asset('assets/images/tags/default.png') }}"/></div>
...
@else
						...
						<div class="notice">
						<!--# $subExcerpt = App\Core\Tool::substrs($tag['excerpt'], 170); #-->
...
@if ($tag['excerpt'] != $subExcerpt)
					...
					<div class="box_wrap tag_page_view">
						<div class="content_nav">
							...
							#-->
								<li {!! $thread_current !!}><a href="{{ url('tag/index/view?name=' . $tag['tag_name']) }}">最新帖子</a></li>
								<li {!! $user_current !!}><a href="{{ url('tag/index/view?name=' . $tag['tag_name'] . '&type=users') }}">会员</a></li>
							</ul>
						</div>
						<div class="">
...
@foreach ($users as $u)
								...
								<li>
									<a href="{{ url('space/index/run?uid=' . $u['uid']) }}" class="J_user_card_show" data-uid="{{ $u['uid'] }}"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($u['uid'], 'middle') }}" data-type="middle" width="100" height="100" alt="{{ $u['username'] }}" /></a>
...
if (!$value['content']) continue;
							#-->
							<dl class="cc">
								<dt class="face"><a href="{{ url('space/index/run?uid=' . $value['created_userid']) }}" class="J_user_card_show" data-uid="{{ $value['created_userid'] }}"><img class="J_avatar" src="{{ App\Core\Tool::getAvatar($value['created_userid'], 'small') }}" data-type="small" width="50" height="50" alt="{{ $value['created_username'] }}" /></a></dt>
...
@else
									...
									<div class="content">
										...
										<div class="descrip">{{ App\Core\Tool::substrs(App\Core\Tool::stripWindCode($value['content'],true),125) }}</div>
									</div>
									<div class="info">
										<span class="time">{{ $value['created_time_auto'] }}</span>&nbsp;来自版块&nbsp;-&nbsp;<a href="{{ url('bbs/thread/run?fid=' . $value['fid']) }}">{!! $value['forum_name'] !!}</a>

@if($relatedTags[$value['tid']])


@foreach($relatedTags[$value['tid']] as $k=>$v)
if ($k==$key) continue; #-->
