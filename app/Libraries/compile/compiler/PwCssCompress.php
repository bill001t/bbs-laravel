...
class PwCssCompress extends AbstractPwCompiler {
	...
	public function doCompile() {
		$config = app('APPCENTER:service.srv.PwInstallApplication')->getConfig('style-type');
		foreach ($config as $k => $v) {
			...
			foreach ($files as $v) {
				...
				if (($r = $this->_doCss($dir . '/' . $v)) instanceof ErrorBag) return $r;
			}
		}
	}
	...
	private function _doCss($stylePackage) {
		...
		foreach ($files as $v) {
			if (WindFile::getSuffix($v) === 'css') {
				...
				if (WindFile::write($css, $_data) === false) return new ErrorBag('STYLE:style.css.write.fail');
			}
		}
		return true;
	}
