...
class PwVerifyCode {
	...
	public function checkVerify($inputCode='') {
        ...
        $inputCode = Tool::encrypt(strtolower($inputCode), $this->_config['key']);
	...
	private function _readVerifyCode() {
		//return Tool::getCookie('Pw_verify_code');
	...
	private function _saveVerifyCode() {
		...
		$code = WindConvert::convert(PwBaseCode::getCode(), Core::V('charset'), 'UTF-8');
		$code = Tool::encrypt(strtolower($code), $this->_config['key']);

        //
		$verify_session_id = Tool::getCookie('Pw_verify_code');
        if( !$verify_session_id ){
            $verify_session_id = md5($code);
            Tool::setCookie('Pw_verify_code',md5($code), 3600);
        }
