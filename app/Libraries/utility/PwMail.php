...
class PwMail {
	...
	public function __construct() {
		...
		$this->_mail->setCharset(Core::V('charset'));
		$this->_mail->setDate(date('r', Tool::getTime()));
		$this->_mail->setContentEncode(WindMail::ENCODE_BASE64);
		$this->_mail->setContentType(WindMail::MIME_HTML);
		$this->_mail->setFrom($this->_config['from'], Core::C('site', 'info.name'));
	}
	...
	public function sendMail($toUser, $subject, $content) {
		if (!$this->_config['mailOpen']) return new ErrorBag('ADMIN:email.close');
		...
		try {
			...
			if (false === $rt) {
				return new ErrorBag('ADMIN:email.server.error');
			}
		} catch(Exception $e) {
