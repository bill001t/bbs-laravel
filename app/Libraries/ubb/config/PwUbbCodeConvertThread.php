...
class PwUbbCodeConvertThread extends PwUbbCodeConvertConfig {
	...
	public function isPost() {
		...
		if (!isset(self::$_ispost[$tid])) {
			if ($this->thread->info['created_userid'] == $this->user->uid) {
				self::$_ispost[$tid] = 2;
			} elseif (app('forum.PwThread')->countPostByTidAndUid($tid, $this->user->uid) > 0) {
				self::$_ispost[$tid] = 1;
			} else {
				self::$_ispost[$tid] = 0;
			}
	...
	public function isBuy() {
		...
		$tid = $this->thread->tid;
		isset(self::$_isbuy[$tid]) || self::$_isbuy[$tid] = app('forum.PwThreadBuy')->getByTidAndUid($tid, $this->user->uid);
	...
	public function getSellInfo() {
		return array(
			url('bbs/buythread/record', array('tid' => $this->thread->tid, 'pid' => $this->pid)),
			url('bbs/buythread/buy', array('tid' => $this->thread->tid, 'pid' => $this->pid)),
