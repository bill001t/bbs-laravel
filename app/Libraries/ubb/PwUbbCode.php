...
class PwUbbCode {
	...
	public static function convert($message, PwUbbCodeConvertConfig $config = null) {
		...
		$message = SimpleHook::getInstance('PwUbbCode_convert')->runWithFilters($message);
	...
	public static function createEmotion($key) {
		is_null(self::$_emotion) && self::$_emotion = Core::cache()->get('all_emotions');
		...
		$html = "<img src=\"" . Core::url()->images . "/emotion/" . $emotion['emotion_folder'] . '/' . $emotion['emotion_icon'] . "\" />";
	...
	public static function createImg($path, $maxWidth = 0, $maxHeight = 0, $original = '', $isLazy = false) {
		...
		if ($isLazy) {
			$html = '<img class="J_post_img J_lazy" data-original="' . $path . '" src="' .  Core::url()->images . '/blank.gif" border="0"';
		} else {
	...
	public static function createImgLink($path) {
        $path = self::escapeUrl($path); //by taishici
		$html = "<img src=\"" . Core::url()->images . "/wind/file/img.gif\" align=\"absbottom\"> <a target=\"_blank\" href=\"$path \">$path</a>";
		return self::_pushCode($html);
	}
	...
	public static function createQoute($str, $username = '', $rpid = 0) {
		$str = str_replace('\\"', '"', $str);
		$username && $str = "<span class=\"fl\"><a href=" . url('space/index/run', array('username' => $username)) . ">" . $username . '</a>：' . $str . '</span>';
		$rpid && $str .= '<a href="' . url('bbs/read/jump', array('pid' => $rpid)) . '" class="return">回到原帖</a>';
	...
	public static function createRemind($username, $uArray) {
		return isset($uArray[$username]) ? '<a href="'. url('space/index/run', array('uid' => $uArray[$username])) . '">@' . $username . '</a>' : '@' . $username;
	}
