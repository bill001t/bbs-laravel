...
class PwSimpleUbbCode {
	...
	protected static function _substrs($message, $length) {
		$strlen = Tool::strlen($message);
		if ($strlen > $length) {
			$message = Tool::substrs($message, $length);
			$strlen = $length;
			self::$_isSubstr = true;
		}
	...
	public static function createRemind($length, $username, $uArray) {
		...
		isset($uArray[$username]) && $html = '<a href="' . url('space/index/run', array('uid' => $uArray[$username])) . '">@' . $username . '</a>';
		return array($html, $strlen);
	}
