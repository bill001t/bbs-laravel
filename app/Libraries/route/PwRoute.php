...
class PwRoute extends AbstractPwRoute {
	...
	public function match($request) {
		...
		if (empty($rawPath) && empty($args) && empty($_args)) {
			return Core::C()->site->get('homeRouter', array());
		}
	...
	private function _matchDomain($host) {
		...
		$space_root = Core::C()->site->get('domain.space.root', '');
		if ($space_root) {
			$rawHost = str_replace($this->scheme, '', $host);
			if ($pos = strpos($rawHost, $space_root)) {
				$uid = app('domain.PwSpaceDomain')->getUidByDomain(substr($rawHost, 0, $pos - 1));
				if ($uid) {
					return array('m' => 'space', 'c' => 'index', 'a' => 'run', 'uid' => $uid);
				}
			}
		}
		$domain = Core::C()->site->get('domain', array());
					...
					if (preg_match($v['pattern'], $rewritePath, $matches)) {
						...
						if ($k == 'thread' || $k == 'cate') {
							if (!isset($matches['fid']) && isset($matches['fname'])) {
								...
								$forum = app('forum.PwForum')->getForum($matches['fid']);
								$action = array(
	...
	private function _getRule() {
		static $conf = null;
		if ($conf === null) {
			$conf = Core::C()->site->get('rewrite', array());
		}
		return $conf;
	}
	...
	private function _getDomainByType($type) {
		...
		if (!isset($domain[$domain_type])) {
			$temp = array();
			if (Core::C('site', "domain.{$domain_type}.isopen")) {
	...
	private function _getDomain($key = 'domain', $value = 'domain', $type = 'forum', $absolute = false) {
		...
		if (!isset($result[$type][$key][$value])) {
			...
			$domain === null && $domain = app('domain.PwDomain')->getAll();
			foreach ($domain as $v) {
	...
	private function _getAppType() {
		static $conf = null;
		if ($conf === null) {
			$conf = Core::C()->site->get('domain.app', array());
		}
		return $conf;
	}
	...
	private function _getModuleDomain($_m) {
		...
		isset($appType['default']) || $this->dynamicHost = '/';
		return isset($appType['default']) ? $appType['default'] . $this->base : ($this->origialBase != $this->base ? '/' : '');
	}
