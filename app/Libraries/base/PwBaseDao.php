...
class PwBaseDao extends WindDao {
	...
	public function getBaseInstance() {
		if (!$this->_baseInstance) {
			...
			$this->_baseInstance = app($this->_defaultBaseInstance);
		}
	...
	protected function _add($fields, $getId = true) {
		...
		SimpleHook::getInstance($this->_class() . '_add')->runDo($result, $fields);
		return $result;
	}
	...
	protected function _update($id, $fields, $increaseFields = array(), $bitFields = array()) {
		...
		SimpleHook::getInstance($this->_class() . '_update')->runDo($id, $fields, $increaseFields);
		return $result;
	}
	...
	protected function _batchUpdate($ids, $fields, $increaseFields = array(), $bitFields = array()) {
		...
		SimpleHook::getInstance($this->_class() . '_batchUpdate')->runDo($ids, $fields, $increaseFields);
		return true;
	}
	...
	protected function _delete($id) {
		...
		$smt = $this->getConnection()->createStatement($sql);
		$result = $smt->update(array($id));
		SimpleHook::getInstance($this->_class() . '_delete')->runDo($id);
		return $result;
	}
	...
	protected function _batchDelete($ids) {
		...
		$this->getConnection()->execute($sql);
		SimpleHook::getInstance($this->_class() . '_batchDelete')->runDo($ids);
		return true;
	}
