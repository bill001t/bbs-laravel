...
class PwBaseController extends WindController {
	...
	public function beforeAction($handlerAdapter) {
		...
		$this->loginUser = Core::getLoginUser();
	...
	protected function showMessage($message = '', $referer = '', $refresh = false) {
		...
		return $this->showError($message, $referer, $refresh);
	}
	...
	protected function showError($error = '', $referer = '', $refresh = false) {
		if ($referer && !CommonValidator::isUrl($referer)) {
			$_referer = explode('#', $referer, 2);
			$referer = url($_referer[0], array(),
	...
	protected function setDefaultTemplateName($handlerAdapter) {
		view($handlerAdapter->getController() . '_' . $handlerAdapter->getAction());
	}
	...
	protected function setTheme($type, $theme) {
		...
		// 风格预览，管理员权限
		if ($style = Tool::getCookie('style_preview')) {
			list($s_theme, $s_type) = explode('|', $style, 2);
			if ($s_type == $type) {
				$theme = $s_theme;
				Core::C()->site->set('theme.' . $type . '.default', $theme);
			}
		}
