...
class PwCache {
	...
	public function __construct() {
		...
		if (Core::V('mem.isopen') && class_exists(Core::V('mem.server'))) $this->_cacheOpen |= self::USE_MEN;
		if (Core::V('redis.isopen') && class_exists('Redis')) $this->_cacheOpen |= self::USE_REDIS;
	}
	...
	public function mergeKeys($keys) {
		$this->keys = array_merge($this->keys, Core::V('distributed') ? self::getDistributed($keys) : $keys);
	}
		...
		foreach ($sids as $sid) {
			...
			foreach ($result as $vkey => $value) {
				...
				if (is_array($this->keys[$key][5])) {
					...
					$result[$vkey] = call_user_func_array(array(app($srv), $method), $param);
				} else {
	...
	protected function _getConfig($use, $mod) {
		switch ($use) {
			...
			case self::USE_DB:
				$config = array(
					'table-name' => Core::V('db.table.name'),
					'field-key' => 'cache_key',
				...
				break;
			case self::USE_MEN:
				$servers = Core::V('mem.servers');
				!isset($servers[$mod]) && $mod = 'default';
				$config = array(
					'key-prefix' => Core::V('mem.key.prefix'),
					'servers' => $servers[$mod],
				);
				break;
			case self::USE_REDIS:
				$servers = Core::V('redis.servers');
				!isset($servers[$mod]) && $mod = 'default';
				$config = array(
					'key-prefix' => Core::V('redis.key.prefix'),
					'servers' => $servers[$mod],
				);
				break;
		}
	...
	protected function _getCacheServer($use, $config) {
		switch ($use) {
			...
			case self::USE_DB:
				...
				break;
			case self::USE_MEN:
				$class = Wind::import('WIND:cache.strategy.Wind' . Core::V('mem.server'));
				$server = new $class();
				break;
			case self::USE_REDIS:
