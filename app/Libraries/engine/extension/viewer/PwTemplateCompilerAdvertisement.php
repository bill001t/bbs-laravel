...
class PwTemplateCompilerAdvertisement extends AbstractWindTemplateCompiler {
	...
	public function compile($key, $content) {
		$pwAdDs = app('SRV:advertisement.PwAd');
		if($this->sys == 1){
	...
	private function _buildHtml($ad){
		...
		$html = '<?php $pwAdService = app("SRV:advertisement.srv.PwAdService");'."\r\n";
		$html .= '$ads = Core::cache()->get("advertisement");'."\r\n";
		...
		$html .= '<script type="text/javascript" charset="gb2312" src="http://js.adm.cnzz.net/s.php?sid='.$pid.'&proid=<?php echo Security::escapeHTML($proid);?>&fid=<?php echo Security::escapeHTML($fid);?>&mid='.$mode.'&floorid=<?php echo Security::escapeHTML($read[lou])?>&pid='.$adSrc.'"></script>'."\r\n";
		$html .= '<?php }else{ ?>'."\r\n";
		$html .= '<div class="J_ad_iframes_div" data-src="http://js.adm.cnzz.net/pwaos.php?sid='.$pid.'&proid=<?php echo Security::escapeHTML($proid);?>&fid=<?php echo Security::escapeHTML($fid);?>&mid='.$mode.'&floorid=<?php echo Security::escapeHTML($read[lou])?>&pid='.$adSrc.'" data-width="<?php echo Security::escapeHTML($currentAd[width]);?>" data-height="<?php echo Security::escapeHTML($currentAd[height]);?>"></div>'."\r\n";
		$html .= '<?php }}?>'."\r\n";
		return $html;
	}
	...
	private function _getAdService(){
		return app('SRV:advertisement.srv.PwAdService');
	}
}