...
class PwTemplateCompilerUrlCreater extends AbstractWindTemplateCompiler {
	...
	public function compile($key, $content) {
		...
		$content = url($content, array(), '', $route, false);
		...
		/* foreach ((array) Core::getGlobal('_url_') as $key => $value) {
			$this->_variables[$key] = $value;
			$content .= ',\'&' . $key . '=\',Core::getGlobal(\'_url_\', ' . $key . ')';
		} */
	...
	private function _variable($matches) {
		$key = Utility::generateRandStr(10);
