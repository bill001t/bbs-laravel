...
class PwStorage {
	...
	public function getStorages() {
		...
		$storages = SimpleHook::getInstance('PwStorage_getStorages')->runWithFilters($storages);
	...
	public function setStoragesComponents($storageType) {
		...
		if (!array_key_exists($storageType, $storages)) return new ErrorBag('ADMIN:att.storage.type.not.exit');
		$storage = $storages[$storageType];
		if (!isset($storage['components']['path'])) return new ErrorBag('ADMIN:att.storage.config.fail');
		/* @var $componentService PwComponentsService */
		$componentService = app('hook.srv.PwComponentsService');
