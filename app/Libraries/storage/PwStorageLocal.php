...
class PwStorageLocal {
	...
	public function get($path, $ifthumb) {
		...
		return Core::url()->attach . '/' . $dir . $path;
	}
	...
	public function delete($path, $ifthumb = 0) {
		Tool::deleteFile(ATTACH_PATH . $path);
		if ($ifthumb) {
			($ifthumb & 1) && Tool::deleteFile(ATTACH_PATH . 'thumb/' . $path);
			($ifthumb & 2) && Tool::deleteFile(ATTACH_PATH . 'thumb/mini/' . $path);
		}
		return true;
	}
}
