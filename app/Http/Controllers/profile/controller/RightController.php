...
class RightController extends BaseProfileController {
	...
	public function run() {
		...
		->with($myGroup, 'myGroupPermission');
		->with($totalCredit, 'myCredit');
		$this->_appendBread('权限查看', 'profile/right/run');
		return view('profile_right');
	...
	public function dosetAction(Request $request) {
		$gid = $request->get('gid', 'post');
		if (!$gid) return $this->showError('USER:right.gid.require');
		if (in_array($this->loginUser->gid, $this->banGid)) return $this->showError('USER:update.error');
		...
		return $this->showMessage('USER:update.success', 'profile/right/run');
	}
	...
	private function listGroups() {
		...
		->with(array('member', 'special', 'system'), 'showTypes');
		->with($allGroups, 'allGroups');
		->with($groupsType, 'groupTypes');
		->with($myGroups, 'myGroups');
		->with($switchGroups, 'switchGroups');
	}
