...
class ManageController extends AdminBaseController {
	...
	public function sendAction(Request $request) {
		...
		->with($othergroup, 'othergroup');
		->with($memberGroupTypes, 'memberGroupTypes');
		->with($groupGroupTypes, 'groupGroupTypes');
	}
	...
	public function doSendAction(Request $request) {
		list($type,$content,$title,$step,$countStep) = $request->get(array('type','content','title','step','countStep'));
		...
		switch ($type) {
			case 1:  // 根据用户组
				list($user_groups,$grouptype) = $request->get(array('user_groups','grouptype'));
				Wind::import('SRV:user.vo.PwUserSo');
				$vo = new PwUserSo();
				$searchDs = app('SRV:user.PwUserSearch');
				if (!$user_groups) return $this->showError('Message:user.groups.empty');
				...
				if ($step <= $countStep) {
					list($start, $limit) = Tool::page2limit($step, $this->perstep);
					$userInfos = $searchDs->searchUser($vo, $limit, $start);
				}
				break;
			case 2:  // 根据用户名
				$touser = $request->get('touser');
				!$touser && return $this->showError('Message:receive.user.empty');
				...
				break;
			case 3:  // 根据在线用户(精确在线)
				$onlineService = app('online.srv.PwOnlineCountService');
		...
		Tool::echoJson(array('data' => $data));exit;
	}
