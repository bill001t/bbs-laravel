...
class IndexController extends WindController {
	...
	public function beforeAction($handlerAdapter) {
		...
		if ('finish' != $handlerAdapter->getAction()) Core::createapp('install');
		...
		->with(NEXT_VERSION, 'wind_version');
		
		WindFile::isFile($this->_getInstallLockFile()) && return $this->showError('INSTALL:have_install_lock');
	}
	...
	public function run() {
		...
		->with($wind_license, 'wind_license');
	}
	...
	public function checkAction(Request $request) {
		...
		->with($lowestEnvironment, 'lowestEnvironment');
		->with($currentEnvironment, 'currentEnvironment');
		->with($recommendEnvironment, 'recommendEnvironment');
	}
	...
	public function infoAction(Request $request) {
		...
		->with($database_writable, 'database_writable');
		->with($founder_writable, 'founder_writable');
	}
	...
	public function databaseAction(Request $request) {
		$keys = array(
			...
			'dbpw',
			'engine');
		$input = $request->get($keys, 'post');
		$force = $request->get('force');
		$input = array_combine($keys, $input);
		foreach ($input as $k => $v) {
			if (!in_array($k, array('dbpw', 'engine')) && empty($v)) return $this->showError("INSTALL:input_empty_$k");
		}
		if ($input['manager_pwd'] !== $input['manager_ckpwd']) {
			return $this->showError('INSTALL:manager_pwd.eque.ckpwd');
		}
		if (!preg_match('/^[\x7f-\xff\dA-Za-z\.\_]+$/', $input['manager'])) {
			return $this->showError('INSTALL:founder.name.error');
		}
		$usernameLen = Tool::strlen($input['manager']);
		$passwordLen = Tool::strlen($input['manager_pwd']);
		if ($usernameLen < 1 || $usernameLen > 15 || $passwordLen < 1 || $passwordLen > 25) {
			return $this->showError('INSTALL:founder.init.fail');
		}
		if (false === CommonValidator::isEmail($input['manager_email'])) {
			return $this->showError('INSTALL:founder.init.email.error');
		}
		...
		try {
			...
			$error = $e->getMessage();
			return $this->showError($error, false);
		}
		if ($dbnameExist && !$force) return $this->showError('INSTALL:have_install', true, 'index/database', true);
		if (!$dbnameExist) {
			try {
				$pdo = new WindConnection($dsn, $input['dbuser'], $input['dbpw'], $charset);
		...
		if (!$this->_checkWriteAble($this->_getDatabaseFile())) {
			return $this->showError('INSTALL:error_777_database');
		}
		if (!$this->_checkWriteAble($this->_getFounderFile())) {
			return $this->showError('INSTALL:error_777_founder');
		}
		...
		return $this->showMessage('success', false, 'index/table');
	}
	...
	public function tableAction(Request $request) {
        ...
        try {
			...
			return $this->showError($e->getMessage(), false);
		}
	...
	public function finishAction(Request $request) {
		//Core::createapp('phpwind');
		Core::C()->reload('windid');
		app(\App\Services\Api\UserApi::class);
		$db = $this->_checkDatabase();
		//更新HOOK配置数据
		...
		//初始化站点config
		$site_hash = Utility::generateRandStr(8);
		$cookie_pre = Utility::generateRandStr(3);
		app('config.PwConfig')->setConfig('site', 'hash', $site_hash);
		app('config.PwConfig')->setConfig('site', 'cookie.pre', $cookie_pre);
		app('config.PwConfig')->setConfig('site', 'info.mail', $db['founder']['manager_email']);
		app('config.PwConfig')->setConfig('site', 'info.url', PUBLIC_URL);
		app('nav.srv.PwNavService')->updateConfig();
		...
		//风格默认数据
		app('APPCENTER:service.srv.PwStyleInit')->init();

		//计划任务默认数据
		app('cron.srv.PwCronService')->updateSysCron();

		//更新数据缓存
		/* @var $usergroup PwUserGroupsService */
		$usergroup = app('SRV:usergroup.srv.PwUserGroupsService');
		...
		$emotion = app('SRV:emotion.srv.PwEmotionService');
		$emotion->updateCache();
		...
		//门户演示数据
		app('SRV:design.srv.PwDesignDefaultService')->likeModule();
		app('SRV:design.srv.PwDesignDefaultService')->tagModule();
		app('SRV:design.srv.PwDesignDefaultService')->reviseDefaultData();
		...
		//全局缓存更新
		app('SRV:cache.srv.PwCacheUpdateService')->updateConfig();
		app('SRV:cache.srv.PwCacheUpdateService')->updateMedal();
	...
	protected function setDefaultTemplateName($handlerAdapter) {
		...
		return view(strtolower($template));
	}
	...
	protected function showMessage($message = '', $lang = true, $referer = '', $refresh = false) {
		...
		return $this->showError($message, $lang, $referer, $refresh);
	}
	...
	private function _writeFounder($manager, $manager_pwd, $manager_email) {
		...
		$windidConfig->set('security.ban.username', '')
			...
			->set('security.username.min', 1)
			->flush();
		Core::C()->reload('register');
		app(\App\Core\WindidBo::class)->config->C()->reload('reg');
		//TODO结束
		$userDm = new PwUserInfoDm();
		$userDm->setUsername($manager)->setPassword($manager_pwd)->setEmail($manager_email)->setGroupid(3)->setRegdate(
			Tool::getTime())->setLastvisit(Tool::getTime())->setRegip(Wind::getApp()->getRequest()->getClientIp())->setGroups(array('3'=>'0'));
		
		//特殊操作  gao.wanggao
		if (true !== ($result = $userDm->beforeAdd())) {
			return $this->showError($result->getError());
		}
		if (($uid = app('WSRV:user.WindidUser')->addUser($userDm->dm)) < 1) {
			return $this->showError('WINDID:code.' . $uid);
		}
		...
		Core::loadDaoFromMap(PwUser::FETCH_ALL, $daoMap, 'PwUser')->addUser($userDm->getSetData());
		//特殊操作  
			
			
		//$uid = app('user.PwUser')->addUser($userDm);
		...
		if ($uid instanceof ErrorBag) {
			return $this->showError($uid->getError());
		}
		app('user.PwUserBelong')->update($uid, array(3 => 0));
		
		//特殊操作  gao.wanggao
        $this->_defaultAvatar($uid);
        $this->_defaultAvatar(0);
        //特殊操作  
		//app('user.srv.PwUserService')->restoreDefualtAvatar($uid);//用户的默认头像需要设置
		//app('user.srv.PwUserService')->restoreDefualtAvatar(0);//游客的默认头像需要设置
		
		//站点统计信息
		Wind::import('SRV:site.dm.PwBbsinfoDm');
		$dm = new PwBbsinfoDm();
		$dm->setNewmember($manager)->addTotalmember(1);
		app('site.PwBbsinfo')->updateInfo($dm);
		return $uid;
	}
	...
	private function _writeWindid() {
		$baseUrl = '/';
		$key = md5(Utility::generateRandStr(10));
		...
		if ($result instanceof WindidError) return $this->showError('INSTALL:windid.init.fail');
		app(AvatarApi::class)->setStorages('local');
		app('config.PwConfig')->setConfig('site', 'avatarUrl', $baseUrl . '/windid/attachment');
		Wind::import('WINDID:service.config.srv.WindidConfigSet');
		$windidConfig = new WindidConfigSet('site');
	...
	private function _checkDatabase() {
		if (!WindFile::isFile($this->_getDatabaseFile()) || !WindFile::isFile($this->_getTableSqlFile())) {
			return $this->showError('INSTALL:database_config_noexists');
		}
		if (!$this->_checkWriteAble($this->_getDatabaseFile())) {
			return $this->showError('INSTALL:error_777_database');
		}
		...
		/*if (!$this->_checkWriteAble($this->_getWindidFile())) {
			return $this->showError('INSTALL:error_777_windid');
		}*/
		
		$database = include $this->_getTempFile();
		if (!$database['founder']) {
			return $this->showError('INSTALL:database_config_error');
		}
		return $database;
	}
