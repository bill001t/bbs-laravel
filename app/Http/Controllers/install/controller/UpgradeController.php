...
class UpgradeController extends WindController {
	...
	public function run() {
		...
		$forumMisc = app('forum.srv.PwForumMiscService');
...
// 		WindFile::del(Wind::getRealPath('DATA:setup.tmp_dbsql.php', true));
	...
	public function avatarAction(Request $request) {
		...
		list($start_uid, $end) = $this->_getStartAndLimit(intval($request->get('uid', 'get')), $end_uid, $ftp ? true : false);
		while ($start_uid < $end) {
			...
			$_toPath = '/avatar/' . Tool::getUserDir($start_uid) . '/';
		...
		if ($end < $end_uid) {
			->with($end, 'uid');
			->with($request->get('token', 'get'), 'token');
			return view('upgrade_avatar');
		} else {
			return $this->showMessage('升级成功！');
		}
	}
	...
	private function _getStartAndLimit($start_uid, $end_uid, $isFtp = true) {
		...
		if ($start_uid >= $end_uid) {
			return $this->showMessage('头像升级成功');
		}
	...
	private function getMaxUid() {
		...
		if (!$link) {
			return $this->showError("Access denied for user '{$db_config['src_username']}'@'{$db_config['src_host']}' (using password: YES)");
		}
		$rt = mysql_select_db($db_config['src_dbname'], $link);
		if (false === $rt) {
			return $this->showError('SQL ERROR:' . mysql_error($link));
		}
		$sql = sprintf("SELECT MAX(uid) FROM %smembers", trim($pre));
		$rt = mysql_query($sql, $link);
		if (false === $rt) {
			return $this->showError('SQL ERROR:' . mysql_error($link) . ' IN "' . $sql . '"');
		}
		$result = mysql_fetch_array($rt, MYSQL_NUM);
		if (!$result[0]) {
			return $this->showMessage('没有用户头像需要转换');
		}
		return $result[0];
	}
	...
	protected function _designUpgrade() {
		...
		while (false !== ($file = @readdir($handle))) {
			...
			if (is_file($fileName)){
			    ...
			    app('design.PwDesignPortal')->updatePortal($dm);
...
// 		$referer && $referer = url($referer);
		$this->addMessage('up87to90.php?step=init&action=end&seprator=1&token=' . $request->get('token', 'get'), 'referer');
