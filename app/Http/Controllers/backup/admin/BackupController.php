...
class BackupController extends AdminBaseController {
	...
	public function subcatAction(Request $request){
		...
		$name = Security::escapePath($name);
		!$name && return $this->showError('BACKUP:name.empty');
		$files = WindFolder::read(Security::escapePath($this->_bakupDir.$name));
		$filedb = array();
		foreach ($files as $v) {
	...
	public function batchdeleteAction(Request $request) {
		...
		!$files && return $this->showError('BACKUP:name.empty');
		foreach($files as $value){
			$value = Security::escapePath($value);
			if (!$value) continue;
	 ...
	 * @return void
	 */
	public function dobackAction(Request $request){
		$siteState = Core::C('site', 'visit.state');
		if ($siteState != 2) {
			return $this->showError('BACKUP:site.isopen');
		}
		@set_time_limit(500);
		list($sizelimit, $compress, $start, $tableid, $step, $dirname) = $request->get(array('sizelimit', 'compress', 'start', 'tableid', 'step', 'dirname'));
		list($tabledb, $insertmethod, $tabledbname) = $request->get(array('tabledb', 'insertmethod', 'tabledbname'));
		...
		((!is_array($tabledb) || !$tabledb) && !$step) && return $this->showError('BACKUP:name.empty');
		// 读取保存的需要操作的表
		
		if (!$tabledb && $step) {
			$cachedTable = WindFile::read(Security::escapePath($tabledbTmpSaveDir . $tabledbname . '.tmp'));
		...
		if (!$step) {
			...
			if ($tabledb) {
				...
				WindFile::write(Security::escapePath($tabledbTmpSaveDir . $tabledbname . '.tmp'), implode("|", $tabledb), 'wb');
			}
			// 备份数据表结构
			// 备份特殊表结构和数据
			if ($specialTables) {
				...
				return $this->showMessage('正在备份',$referer,true);
			}
		}
		if (!$tabledb) {
			return $this->showMessage(array('BACKUP:bakup_success', array(
		...
		if ($continue) {
			...
			return $this->showMessage(array('BACKUP:bakup_step', array(
			...
			unlink(Security::escapePath($tabledbTmpSaveDir . $tabledbname . '.tmp'));
			return $this->showMessage(array('BACKUP:bakup_success', array(
				'{path}'=>$backupService->getSavePath(). $dirname,
			)),'admin/backup/backup/run');
		}
	...
	public function optimizeAction(Request $request) {
		...
		!$tabledb && return $this->showError('BACKUP:table.empty');
		//关闭站点
		$config = new PwConfigSet('site');
		$siteState = Core::C()->getValues('site');
	...
	public function importAction(Request $request) {
		list($file,$dir) = $request->get(array('file','dir'));
		list($step,$count,$isdir) = $request->get(array('step','count','isdir'));
		!$dir && return $this->showError('BACKUP:name.empty');
		$siteState = Core::C('site', 'visit.state');
		if ($siteState != 2) {
			return $this->showError('BACKUP:site.isopen');
		}
		if (!$count) {
			$count = 1;
			$files = WindFolder::read(Security::escapePath($this->_bakupDir.$dir));
			foreach ($files as $v) {
		...
		if($count > 1 && $step <= $count){
			...
			return $this->showMessage(array('BACKUP:bakup_import', array(
				'{i}'=>$i
			)),$referer,true);
		}

		return $this->showMessage('success','admin/backup/backup/restore');
	}
	...
	private function _getBackupService(){
		return app('backup.srv.PwBackupService');
	}
}
