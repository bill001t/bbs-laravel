...
class PunchController extends Controller{
	...
	public function beforeAction($handlerAdapter) {
		...
		$this->config = Core::C()->getValues('site');
		$this->_creditBo = PwCreditBo::getInstance();
		
		// 是否开启
		if(!$this->config['punch.open']){
			return $this->showError('SPCAE:punch.not.open');
		}
	}
	...
	public function punchAction(Request $request) {
		...
		if ($userInfo['punch']) {
			...
			if ($havePunch) {
				($punchData['username'] == $userInfo['username']) && return $this->showError('SPACE:punch.today.punch');
				$helpPunch = 1;
			}
		}
		// 奖励积分数
		...
		Tool::echoJson(array('state' => 'success', 'data' => $result));exit;
	}
	...
	public function friendAction(Request $request) {
		...
		->with($result, 'friendNum');
		->with($friendReward, 'reward');
		->with($userFollors, 'follows');
	...
	public function getfollowAction(Request $request) {
		...
		Tool::echoJson(array('state' => 'success', 'data' => $this->_fetchFollowUsers($uids), 'page' => $page));exit;
	}
	 ...
	 *
	 */
	public function dofriendAction(Request $request) {
		$friends = $request->get('friend');
		!is_array($friends) && $friends = array($friends);
		if (count($friends) < 0) {
			return $this->showError('SPACE:punch.data.error');
		}
		// 是否关注的人
		$follows = app('attention.PwAttention')->fetchFollows($this->loginUser->uid, $friends);
		...
		if ($allowNum instanceof ErrorBag) {
			return $this->showError($allowNum->getError());
		}
		...
		foreach ($followUids as $uid) {
			...
			$this->_getUserBehaviorDs()->replaceDayBehavior($this->loginUser->uid,'punch_num',Tool::getTime());
	 ...
	 *
	 */
	public function punchtipAction(Request $request) {
	...
	private function _punchBehavior($userInfo,$awardNum,$behaviorNum = '') {
		...
		//埋点[s_punch]
		SimpleHook::getInstance('punch')->runDo($dm);
	...
	private function _checkPunchNum() {
		...
		if ($allowNum < 1) {
			return new ErrorBag('SPACE:punch.friend.num.error',array('{num}'=>$this->config['punch.friend.reward']['friendNum']));
		}
		return $allowNum;
	}
	...
	private function _getPunchService(){
		return app('space.srv.PwPunchService');
	}
