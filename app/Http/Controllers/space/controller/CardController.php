...
class CardController extends Controller{
	...
	public function run() {
		...
		list($uid, $username) = $request->get(array('uid', 'username'));
		if ($uid) {

		} elseif ($username) {
			$info = app('user.PwUser')->getUserByName($username);
			$uid = $info['uid'];
		} else {
			$uid = 0;
		}
		...
		if ($uid) {
			...
			$service = app('attention.PwAttention');
			...
			if ($follow2num > 0) {
				...
				$usernames = app('user.PwUser')->fetchUserByUid(array_keys($uids));
				->with($usernames, 'usernames');
			}
			if (Core::C('site','medal.isopen')) {
				$medalIds = explode(',', $user->info['medal_ids']);
				$medals = app('medal.srv.PwMedalCache')->fetchMedal($medalIds);
			}
		} else {
			$isFollowed = false;
			$isFans = false;
		...
		->with($user->info['gender'] == 1, 'female');
		->with(Tool::checkOnline($user->info['lastvisit']), 'isol');
		->with($uid, 'uid');
		->with($follow2num, 'follow2num');
		->with($isFollowed, 'isFollowed');
		->with($isFans, 'isFans');
		->with($user, 'user');
		->with($medals, 'medals');
		->with(count($medals), 'medalNum');
		return view('TPL:common.card_run');
	}
}