...
class RewriteController extends AdminBaseController {
	 ...
	 */
	public function run() {
		$addons = app('domain.srv.PwDomainService')->getRewriteAddOns();
		$rewrite = Core::C('rewrite');
		->with($addons, 'addons');
		->with($rewrite, 'rewrite');
	}
	...
	public function doModifyAction(Request $request) {
		list($format, $isopen) = $request->get(array('format', 'isopen'));
		$bo = new PwConfigSet('rewrite');
		foreach ($format as $k => $v) {
			if (empty($v) && isset($isopen[$k])) return $this->showError('REWRITE:format.empty');
			$bo->set("format.$k", $v);
		}
		$addons = app('domain.srv.PwDomainService')->getRewriteAddOns();
		$rewriteData = array();
		...
		foreach ($addons as $k1 => $v1) {
			...
			if ($open) {
				...
				if (in_array($format_i, $unique)) {
					return $this->showError(array('REWRITE:format.conflict', array($format[$k1])));
				}
				$unique[] = $format_i;
				if ($k1 == 'thread') {
		...
		$bo->flush();
		Core::C()->setConfig('site', 'rewrite', $rewriteData);
