...
class DomainController extends AdminBaseController {
	 ...
	 */
	public function run() {
		$root = Core::C('site', 'cookie.domain');
		if ($root) {
			$domain = Core::C('domain');
	...
	public function doModifyAction(Request $request) {
		$root = Core::C('site', 'cookie.domain');
		if (empty($root)) return $this->showError('REWRITE:cookie.domain.fail');
		$root[0] != '.' && $root = '.' . $root;
		list($app, $domain, $domain_hold) = $request->get(
		...
		foreach ($app as $key => $value) {
			...
			if ($value) {
				//域名重复
				in_array($value, $unique) && return $this->showError(
		...
		foreach ($domain as $k => $v) {
			$domain_root = '';
			if ($v['isopen']) {
				if (!$app['default']) return $this->showError('REWRITE:default.empty');
				if ($k == 'space' && !$v['root']) return $this->showError('REWRITE:root.empty');
				$space_root = isset($domain['space']['root']) ? $domain['space']['root'] : '';
				if ($k != 'space' && $domain['space']['isopen']) {
					if ($v['root'] == $space_root) {
						return $this->showError('REWRITE:root.same');
					}
				}
				$unique[] = $v['root'];
		...
		app('SRV:nav.srv.PwNavService')->updateConfig();
	...
	private function _service() {
		return app('domain.srv.PwDomainService');
	}
}
