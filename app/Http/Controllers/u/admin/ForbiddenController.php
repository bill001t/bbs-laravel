...
class ForbiddenController extends AdminBaseController {
	...
	public function dorunAction(Request $request) {
		...
		$key = $request->get('key', 'post');
		if (!in_array($key, array('1', '2'))) return $this->showError('USER:ban.error.data.format');
		$array = array();
		list($end_time, $reason, $types) = $request->get(array('end_time', 'reason', 'type'), 'post');
		$userInfos = $this->_getUids(explode(',', $request->get('value', 'post')), intval($key));
		if (!$userInfos) return $this->showError('USER:ban.user.illegal');
		...
		if ($end_time > 0) $end_time = Tool::str2time($end_time);
		...
		if ($r instanceof ErrorBag) return $this->showError($r->getError(), 'u/forbidden/run');
		
		$this->_getService()->sendNotice($_notice, 1);
		return $this->showMessage('USER:ban.success', 'u/forbidden/run');
	}
	...
	public function autoAction(Request $request) {
		...
		->with(array_merge($default, $config), 'config');
		
		Wind::import('SRV:credit.bo.PwCreditBo');
		/* @var $pwCreditBo PwCreditBo */
		$pwCreditBo = PwCreditBo::getInstance();
		->with($pwCreditBo, 'creditBo');
		$banService = $this->_getService();
		->with($this->_getBanDayType(), 'dayTypes');
		->with($banService->getBanType(), 'types');
	}
	 ...
	 * 设置自动禁止
	 */
	public function dosetautoAction(Request $request) {
		$request->isPost() || return $this->showError('operate.fail');

		$config = new PwConfigSet('site');
		list($open, $condition, $type, $reason) = $request->get(array('open', 'condition', 'type', 'reason'), 'post');
		if ($open == 1) {
			if (!$condition['num']) return $this->showError('USER:ban.auto.credit.num.require');
			if (!$type) return $this->showError('USER:ban.type.require');
			if (!$reason) return $this->showError('USER:ban.reason.require');
		}
		$config->set('autoForbidden.open', $open)
			->set('autoForbidden.condition', $condition)
			->set('autoForbidden.day', $request->get('day', 'post'))
			->set('autoForbidden.type', $type)
			->set('autoForbidden.reason', $reason)
			->flush();
		return $this->showMessage('USER:ban.auto.set.success', 'u/forbidden/auto');
	}
	...
	public function listAction(Request $request) {
		...
		$searchSo->setType($request->get('key'))
			->setKeywords($request->get('value'))
			->setCreatedUsername($request->get('operator'))
			->setStartTime($request->get('start_time'))
			->setEndTime($request->get('end_time'));
		$result = array();
		/* @var $banDs PwUserBan */
		$banDs = app('user.PwUserBan');
		...
		->with($result, 'list');
		->with($count, 'count');
		->with($searchSo, 'searchSo');
		->with($page, 'page');
		->with($perpage, 'perpage');
		->with($searchSo->getArgsUrl(), 'urlArgs');
	}
	...
	public function delAction(Request $request) {
		...
		if (!$ids) return $this->showError('operate.select');
		/* @var $banSrv PwUserBanService */
		$banSrv = app('SRV:user.srv.PwUserBanService');
		...
		return $this->showMessage('USER:ban.delete.success');
	}
	...
	private function _getUids($values, $type = 1) {
		...
		if (!$values) return $this->showError('USER:ban.user.require');
		switch (intval($type)) {
			case 1:
	...
	private function _getService() {
		return app('SRV:user.srv.PwUserBanService');
	}
}
