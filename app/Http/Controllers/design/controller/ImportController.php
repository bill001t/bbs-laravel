...
class ImportController extends Controller{
	...
	public function dorunAction(Request $request) {
		...
		if (!$ext || !in_array($ext, array('txt', 'zip'))) return $this->showMessage("DESIGN::upload.fail");
		
		if ($ext == 'zip'){//return $this->showError("DESIGN:page.emport.fail");
	...
	public function editstyleAction(Request $request) {
		$styleid = $request->get('styleid', 'get');
		$portalid = (int)$request->get('portalid', 'get');
		$ds = $this->_getPortalDs();
		$portal = $ds->getPortal($portalid);
		if (!$portal) return $this->showError("operate.fail");
		$styleDs = app('APPCENTER:service.PwStyle');
		$style = $styleDs->getStyle($styleid);
		if (!$style || $style['style_type'] !='portal') return $this->showError("operate.fail");
		...
		if (!$srv->appcenterToLocal($style['alias'])) return $this->showError("operate.fail");
	...
	protected function doZip($pageBo) {
		...
		Wind::import('SRV:design.srv.PwDesignImportZip');
		$srv = new PwDesignImportZip($pageBo);
		if (!$srv->checkDirectory()) return $this->showError("DESIGN:directory.not.writeable");
		$file = $this->_uploadFile();
		if (!$file || $file['type'] != 'zip') return $this->showMessage("DESIGN::upload.fail");
	...
	protected function doTxt($pageInfo) {
		$srv = app('design.srv.PwDesignImportTxt');
		$file = $this->_uploadFile();
		if (!$file || $file['type'] != 'txt') return $this->showMessage("DESIGN:upload.fail");
