...
class AdminFounderService {
	...
	public function login($username, $password) {
		...
		if (!$result = $this->checkPwd($founder[$username], $password)) {
			return new ErrorBag('ADMIN:login.fail.user.illegal');
		}
		return array(AdminUserService::FOUNDER, $username, Tool::getPwdCode($result));
	}
	...
	public function isLogin($username, $password) {
		...
		if (Tool::getPwdCode($md5pwd) != $password) {
			return array();
		}
	...
	public function add($username, $password, $email) {
		if (!$this->isWriteable()) return new ErrorBag('ADMIN:founder.file.write.fail');
		$this->getFounders();
		if (isset($this->_founder[$username])) return new ErrorBag(
			'ADMIN:founder.add.fail.username.duplicate');
		
		$user = $this->loadUserService()->getUserByName($username);
		if (!$password && !isset($user['password'])) return new ErrorBag(
	...
	public function edit($username, $password, $email) {
		if (!$this->isWriteable()) return new ErrorBag('ADMIN:founder.file.write.fail');
		$this->getFounders();
		if (!isset($this->_founder[$username])) return new ErrorBag('ADMIN:founder.edit.fail');
	...
	public function encryptPwd($password) {
		$salt = Utility::generateRandStr(6);
	...
	public function del($username) {
		if (!$this->isWriteable()) return new ErrorBag('ADMIN:founder.file.write.fail');
		$this->getFounders();
		if (!isset($this->_founder[$username])) return new ErrorBag('ADMIN:founder.del.fail');
		unset($this->_founder[$username]);
		if (empty($this->_founder)) return new ErrorBag('ADMIN:founder.del.fail.all');
	...
	private function updateFounder() {
		...
		return $r ? $r : new ErrorBag('ADMIN:founder.file.write.fail');
	}
	...
	private function getFounderFilePath() {
		return Wind::getRealPath(Core::app()->founderPath, true);
	}
