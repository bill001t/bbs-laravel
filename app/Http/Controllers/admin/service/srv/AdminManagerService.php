...
class AdminManagerService {
	...
	public function login($username, $password) {
		if (!$this->loadSafeService()->ipLegal(Wind::getComponent('request')->getClientIp())) {
			return new ErrorBag('ADMIN:login.fail.ip');
		}
		...
		/* @var $auth AdminAuth */
		$auth = app('ADMIN:service.AdminAuth');
		if (!$auth->findByUid($user['uid'])) {
			return new ErrorBag('ADMIN:login.fail.allow');
		}
		$u = $this->loadUserService()->getUserByUid($user['uid']);
		return array(AdminUserService::USER, $user['uid'], Tool::getPwdCode($u['password']));
	}
	...
	public function isLogin($uid, $password) {
		...
		if (Tool::getPwdCode($user['password']) != $password) {
			return array();
		}
		$auth = app('ADMIN:service.AdminAuth');
		if (!$auth->findByUid($user['uid'])) {
