...
class AdminUserService {
	...
	public function verifyUserMenuAuth($user, $m, $c, $a) {
		...
		if (empty($_menus) || !is_array($_menus)) return new ErrorBag('ADMIN:menu.fail.allow');
		...
		return new ErrorBag('ADMIN:menu.fail.allow');
	}
	...
	public function getAuths($user) {
		...
		$roleService = app('ADMIN:service.AdminRole');
		$roles = $roleService->findRolesByNames($roles);
		if ($roles instanceof ErrorBag) return new ErrorBag('ADMIN:fail');
	...
	public function login($username, $password) {
		...
		if (($result = $srv->login($username, $password)) instanceof ErrorBag) {
			return $result;
		}
		Tool::setCookie($this->cookieName, Tool::encrypt(implode("\t", $result)), 1800);
		return true;
	}

	public function isLogin() {
		if (!($userCookie = Tool::getCookie('AdminUser'))) {
			return array();
		}
		list($type, $uid, $password) = explode("\t", Tool::decrypt($userCookie));
		if ($type == AdminUserService::FOUNDER) {
			$srv = $this->loadFounderService();
		} else {
			$srv = $this->loadManagerService();
		}
		Tool::setCookie('AdminUser', $userCookie, 1800);
	...
	public function logout() {
		return Tool::setCookie($this->cookieName, '', -1);
	}
	...
	private function loadManagerService() {
		return app('ADMIN:service.srv.AdminManagerService');
	}
}
?>