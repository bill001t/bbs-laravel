...
class AdminAuth {
	...
	public function del($id) {
		if (!$id) return new ErrorBag('ADMIN:auth.del.fail');
	...
	public function add($username, $uid, $roles) {
		if (empty($username)) return new ErrorBag('ADMIN:auth.add.fail');
		if (empty($uid)) return new ErrorBag('ADMIN:auth.add.fail');
		if (empty($roles)) return new ErrorBag('ADMIN:auth.add.fail.role.empty');
		if ($this->getAdminAuthDao()->findByUsername($username)) {
			return new ErrorBag('ADMIN:auth.add.fail.username.duplicate');
		}
		$fields['uid'] = $uid;
		$fields['username'] = $username;
		$fields['roles'] = implode(',', (array) $roles);
