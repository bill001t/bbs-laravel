...
class AdminRole {
	...
	public function delById($rid) {
		if (empty($rid)) return new ErrorBag('ADMIN:role.del.fail.id.empty');
		$result = $this->getAdminRoleDao()->del($rid);
		if (!$result) return new ErrorBag('ADMIN:role.del.fail');
		return true;
	}
	...
	public function addRole($name, $auths) {
		if (empty($name)) return new ErrorBag('ADMIN:role.add.fail.name.empty');
		if ($this->getAdminRoleDao()->findByName($name)) {
			return new ErrorBag('ADMIN:role.add.fail.name.exist');
		}
		$fields['name'] = $name;
		...
		if (!$result) return new ErrorBag('ADMIN:role.add.fail');
		return true;
	}
	...
	public function editRole($id, $name, $auths) {
		...
		foreach ($_roles as $key => $value) {
			if ($value['id'] !== $id) return new ErrorBag('ADMIN:role.add.fail.name.exist');
		}
	...
	public function findRolesByNames($names) {
		...
		if (!$result) return new ErrorBag('ADMIN:role.find.fail');
		return $result;
	}
	...
	private function getAdminRoleDao() {
		return app('ADMIN:service.dao.AdminRoleDao');
	}
}
