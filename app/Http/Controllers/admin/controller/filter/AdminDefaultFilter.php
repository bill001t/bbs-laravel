...
class AdminDefaultFilter extends PwBaseFilter {
	...
	public function preHandle() {
		...
		$safeService = app('ADMIN:service.srv.AdminSafeService');
		/* @var $founderService AdminFounderService */
		$founderService = app('ADMIN:service.srv.AdminFounderService');
		...
		if (!in_array(strtolower($request['c']), $_unVerifyTable)) {
			if ($request['c'] != 'adminlog') {
				...
				$logService->log($request, $loginUser->username, $request['m'], $request['c'], $request['a']);
			}
			$_result = $userService->verifyUserMenuAuth($loginUser, $request['m'], $request['c'], $request['a']);
			if ($_result instanceof ErrorBag) $this->errorMessage->sendError($_result->getError());
		}
	}
