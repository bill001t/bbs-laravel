...
class PwWindidInform {
	...
	public function synLogin($uid) {
		...
		if ($userBo->isExists() && !Tool::getstatus($userBo->info['status'], PwUser::STATUS_UNACTIVE)) {
	...
	public function addUser($uid) {
		...
		if ($result instanceof ErrorBag) return $result->getError();
		return true;
	}
	...
	public function uploadAvatar($uid) {
		SimpleHook::getInstance('update_avatar')->runDo($uid);
		return true;
	}
	...
	public function editMessageNum($uid) {
		$result = app('message.srv.PwMessageService')->synEditUser($uid);
		return $result;
	}
	...
	public function deleteUser($uid) {
		...
		if ($result instanceof ErrorBag) return false;
		return true;
	}
	...
	public function setCredits() {
		...
		$credit = Core::C()->getValues('credit');
		...
		app('credit.srv.PwCreditSetService')->setLocalCredits($credits);
		return true;
	}
