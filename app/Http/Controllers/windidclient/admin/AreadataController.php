...
class AreadataController extends AdminBaseController {
	...
	public function updateAction(Request $request) {
		list($update, $add, $parentid) = $request->get(array('update', 'add', 'parentid'), 'post');
		...
		if ($parentid) {
			...
			switch (count($rout)) {
				case 0:
					return $this->showError('ADMIN:area.parentid.error');
					break;
				case 3:
					return $this->showError('ADMIN:area.level.limit');
					break;
			}
			$joinnames = array();
			foreach ($rout as $i) {
		...
		foreach ($update as $id => $name) {
			...
			if (true !== ($r = $this->_loadAreaDs()->updateArea($dm))) {
				return $this->showError(array('ADMIN:area.error.' . $r->getCode(), array('{flag}' => "“& \" ' < > \ / ”")));
			}
		}
		$addDms = array();
		foreach ($add as $name) {
		...
		return $this->showMessage('success', 'admin/windidclient/areadata/run?parentid=' . $parentid);
	}
	...
	public function deleteAction(Request $request) {
		...
		if (!$areaid) return $this->showError('ADMIN:area.areaid.error');
		$childs = $this->_loadAreaDs()->getByParentid($areaid);
		if ($childs) return $this->showError('ADMIN:area.delete.error.has.children');
		$this->_loadAreaDs()->deleteArea($areaid);
		return $this->showMessage('ADMIN:area.delete.success');
	}
