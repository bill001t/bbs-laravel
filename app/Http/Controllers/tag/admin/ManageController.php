...
class ManageController extends AdminBaseController {
	...
	public function beforeAction($handlerAdapter) {
		...
		->with($handlerAdapter->getControllerKey(),'controllerKey');
		->with($handlerAdapter->getModuleKey(),'moduleKey');
	}
	...
	public function run() {
		...
		list($page,$keyword,$ifhot,$categoryId,$minAttention,$maxAttention,$minContent,$maxContent) = $request->get($conditions);
		...
		//->with(ceil($count/$this->perpage), 'totalpage');
	}
	...
	public function doaddAction(Request $request){
		$tag = $request->get('tag');
		if (!$tag['name']) return $this->showError('Tag:tagname.empty');
		if ($this->_getTagDs()->getTagByName($tag['name'])) return $this->showError("话题{$tag['name']}已存在");
		$tag = $request->get('tag');
		...
		if (is_numeric($tagId = $this->_getTagDs()->addTag($dm))) {
			...
			return $this->showMessage('话题添加成功！');
		} else {
			if ($tagId instanceof ErrorBag) {
				return $this->showError($tagId->getError());
			}
			return $this->showError('话题添加失败！');
		}
	}
	...
	public function editAction(Request $request) {
		$tagId = intval($request->get('id','get'));
		...
		if ($relatedTag) {
			...
			->with(implode(',', $relatedTagHtml), 'relatedTags');
		}
		//话题分类
		$tagCategories = $this->_getTagCateGoryDs()->getCategoriesByTagId($tagId);
		$tagCategories and ->with($tagCategories, 'tagCategories');
		->with($tag, 'tag');
		->with($categories, 'categories');
	}
	...
	public function doeditAction(Request $request){
		...
		if ($logo && $logo != $tagInfo['tag_logo']) {
			Tool::deleteAttach($tagInfo['tag_logo']);
		}
		//取消原关联话题
		...
		if ($tag['relate_tags']) {
			$tagInfo = $this->_getTagDs()->getTag($tag['tag_id']);
				$tagInfo['parent_tag_id'] && return $this->showError(sprintf('话题"%s"已经有关联话题,不允许再合并关联',$tagInfo['tag_name']));
			$tagNames = explode(',', $tag['relate_tags']);
			foreach ($tagNames as $v) {
				...
				if ($mergeTag['parent_tag_id']) {
					...
					return $this->showError(sprintf('话题"%s"的关联话题为%s,不允许再合并关联',$mergeTag['tag_name'],$parentTag['tag_name']));
				}
		...
		$this->_deleteHotTagCache();
		return $this->showMessage('话题编辑成功！');
	}
	...
	public function deleteAction(Request $request){
		$tagIds = $request->get('tag_id', 'post');
		is_array($tagIds) or $tagIds = explode(',', $tagIds);
		if (!$tagIds) return $this->showError('operate.select');
		$this->_getTagService()->deleteByTagIds($tagIds);
		$this->_deleteHotTagCache();
		return $this->showMessage('话题删除成功！');
	}
	 ...
	 * 取消热门话题
	 */
	public function deletehotAction(Request $request){
	...
	public function domergeAction(Request $request){
		$tagIds = explode(',', $request->get('tag_id', 'post'));
		$tagName = $request->get('tag_name', 'post');
		if (!$tagIds || !$tagName) return $this->showError('请输入关联话题名称');
		...
		foreach ($checkTags as $v) {
			...
			$parentTag && return $this->showError(sprintf('话题"%s"已经合并到话题"%s",不允许再合并到其他话题',$v['tag_name'],$parentTag['tag_name']));
			$tagInfo = $this->_getTagDs()->getTagByParent($v['tag_id']);
			$tagInfo && return $this->showError(sprintf('话题"%s"已经存在合并话题"%s",不允许被合并到其他话题',$v['tag_name'],$tagInfo['tag_name']));
		}
		if (!$tag) {
			$dm = new PwTagDm();
			$dm->setName($tagName);
			...
			if ($tag['parent_tag_id']) {
				...
				return $this->showError(sprintf('话题"%s"已经合并到话题"%s",不允许合并其他话题',$tag['tag_name'],$parentTag['tag_name']));
			}
		}
		foreach ($tagIds as $tagId) {
		...
		$this->_deleteHotTagCache();
		return $this->showMessage('合并关联话题成功！');
	}
	...
	public function domoveAction(Request $request){
		...
		if (!$tagIds || !$categoryIds) return $this->showError('operate.select');
	 ...
	 * 提交编辑
	 */
	public function doEditCategoryAction(Request $request) {
	...
	public function setCategoryAction(Request $request) {
		list($data,$newdata) = $request->get(array('data','newdata'));
		...
		if ($data) {
			foreach ($data as $v) {
				unset($categorys[$v['category_id']]);
					...
					if ($aliasWord !== true) {
						return $this->showError($aliasWord);
					}
				...
				if (($result = $this->_getTagCateGoryDs()->updateTagCategory($dm)) instanceof ErrorBag) {
					return $this->showError($result->getError());
				}
			}
		}
		if ($newdata) {
			foreach ($newdata as $v) {
				...
				$dm->setCategoryName($v['category_name'])
					->setCategoryAlias($v['alias'])
					->setVieworder($v['vieworder']);
				if (($result = $this->_getTagCateGoryDs()->addTagCategory($dm)) instanceof ErrorBag) {
					return $this->showError($result->getError());
				}
			}
		}
