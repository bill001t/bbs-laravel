...
class PwApplication {
	...
	public function add($application) {
		...
		if (true !== $error) return new ErrorBag('APPCENTER:validate.fail', array('error' => $error));
	...
	public function update($application) {
		if (true !== ($error = $application->beforeUpdate())) return new ErrorBag('APPCENTER:validate.fail',
			array('error' => $error));
		$_r = $this->_load()->update($application->getField('app_id'), $application->getData());
		if (!$_r) return new ErrorBag('APPCENTER:update.fail');
		return true;
	}
	 ...
	 *
	 * @param string $alias
	 * @return ErrorBag|Ambigous <multitype:, boolean, multitype:unknown , mixed>
	 */
	...
	private function _load() {
		return app('APPCENTER:service.dao.PwApplicationDao');
	}
}
