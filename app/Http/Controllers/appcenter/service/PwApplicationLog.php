...
class PwApplicationLog {
	 ...
	 *
	 * @param array $fields
	 * @return ErrorBag|Ambigous <boolean, Ambigous, rowCount, number>
	 */
	public function add($appId, $logType, $data) {
		if (!$appId || !$logType) return new ErrorBag('APPCENTER:validate.fail');
		$fields = array(
			'app_id' => $appId, 
			'log_type' => $logType, 
			'data' => serialize($data), 
	 ...
	 *
	 * @param array $fields
	 * @return ErrorBag|Ambigous <Ambigous, rowCount, boolean, number>
	 */
	public function batchAdd($fields) {
		if (!$fields) return new ErrorBag('APPCENTER:validate.fail');
	...
	public function delByAppId($app_id) {
		if (!$app_id) return new ErrorBag('APPCENTER:validate.fail.appid.not.exit');
	 ...
	 *
	 * @param string $app_id
	 * @return ErrorBag|Ambigous <Ambigous, multitype:, multitype:multitype: Ambigous <multitype:, multitype:unknown , mixed> >
	 */
	...
	private function _load() {
		return app('APPCENTER:service.dao.PwApplicationLogDao');
	}
}
