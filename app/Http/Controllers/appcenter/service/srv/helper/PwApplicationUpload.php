...
class PwApplicationUpload {
	...
	public function getSaveName(PwUploadFile $file) {
		$filename = substr(md5(Tool::getTime() . $file->id . Utility::generateRandStr(8)), 10, 15);
	...
	public function checkFile($file) {
		if (!$file->ext || !isset($this->ftype[$file->ext])) {
			return new ErrorBag(array('upload.ext.error', array('{ext}' => '.' . $file->ext)));
		}
		if ($file->size < 1) {
			return new ErrorBag('upload.size.less');
		}
		if ($file->size > $this->ftype[$file->ext] * 1024) {
			return new ErrorBag(array('upload.size.over', array('{size}' => $this->ftype[$file->ext])));
		}
		return true;
	}
