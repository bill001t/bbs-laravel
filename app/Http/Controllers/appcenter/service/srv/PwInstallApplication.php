...
class PwInstallApplication {
	...
	public function __construct() {
		$this->_appId = 'L000' . time() . Utility::generateRandStr(4);
		$this->_config = @include (Wind::getRealPath(self::CONF_PATH, true));
		$this->tmpPath = Wind::getRealPath($this->getConfig('tmp_dir') . '.' . Tool::getTime(), false);
	...
	public function install($id) {
		$this->_appId = $id;
		$_r = $this->download();
		if ($_r instanceof ErrorBag) return $_r;
		file_put_contents(DATA_PATH . 'tmp/log', 'Download!');
		$extends = $this->getOnlineInfo();
		if ($extends instanceof ErrorBag) return $extends;
	...
	public function onlineInstall($id) {
		...
		if ($manifest instanceof ErrorBag) return $manifest;
	...
	public function extractPackage($packageFile) {
		...
		if ($this->tmpPackage === false || !is_dir($this->tmpPackage)) return new ErrorBag(
	...
	public function initInstall($manifest = '', $extends = array()) {
		if ($manifest === '') {
			...
			if (!is_file($manifest)) return new ErrorBag('APPCENTER:install.mainfest.not.exist');
		} 
	...
	public function doInstall($step, $hash) {
		try {
			...
			if (empty($service)) return new ErrorBag('APPCENTER:install.service.fail');
			$this->_appId = $install->getAppId();
			
			$next = true;
			if ($step !== 'all') {
				if (!isset($service[$step])) return new ErrorBag('APPCENTER:install.step.fail');
			...
			foreach ($service as $key => $var) {
				...
				$_install = app($var['class']);
				if (!$_install instanceof iPwInstall) return new ErrorBag(
			...
			return new ErrorBag($error[0], $error[1]);
		}
	}
	...
	public function rollback() {
		...
		foreach ($rollback as $var) {
			if (!isset($var['class'])) continue;
			$_install = app($var['class']);
			if (!$_install instanceof iPwInstall) return new ErrorBag('APPCENTER:install.classtype');
	...
	public function downloadFiles() {
		return new ErrorBag('APPCENTER:unsupport.zip');
	}
	...
	public function downloadInstallPack() {
		...
		if ($info['code'] !== '0') return new ErrorBag('APPCENTER:install.download.fail',
			array('{{error}}' => $info['msg']));
		list($bool, $package) = PwApplicationHelper::requestAcloudData($info['info']['download'], 
			$this->tmpPath);
		if (!$bool) return new ErrorBag('APPCENTER:install.download.fail', array('{{error}}' => $package));
	...
	public function getOnlineInfo() {
		...
		if ($data['code'] !== '0') return new ErrorBag('APPCENTER:install.fail',
	...
	private function _loadInstallLog() {
		return app('APPCENTER:service.PwApplicationLog');
	}
