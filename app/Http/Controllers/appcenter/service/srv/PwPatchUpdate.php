...
class PwPatchUpdate {
	...
	public function __construct() {
		$this->tmpPath = DATA_PATH . 'tmp/' . Tool::getTime();
		$this->ftp = Core::cache()->get('system_patch_ftp');
	}
	...
	public function getOnlinePatchList() {
		...
		foreach ($r['info'] as $v) {
			$v['id'] = $v['name'];
			$temp[$v['id']] = WindConvert::convert($v, Core::V('charset'), 'utf8');
		}
		return $temp;
	}
	...
	public function install($patch) {
		...
		foreach ($patch['rule'] as $rule) {
			...
			if ($realCount != $count) {
				return new ErrorBag('APPCENTER:upgrade.patch.update.fail', array($patch['id']));
			}
			$bakfile = basename($rule['filename']) . '.' . Tool::time2str(WEKIT_TIMESTAMP, 'Ymd') . '.bak';
			...
			if ($this->ftp) {
				...
				if (!@copy($filename, $bakfile)) {
					return new ErrorBag('APPCENTER:upgrade.copy.fail', array($rule['filename']));
				}
	...
	public function clear() {
		...
		Core::cache()->delete('system_patch_ftp');
	}
