...
class PwInstall implements iPwInstall {
	...
	public function install($install) {
		...
		if ($result) return new ErrorBag('APPCENTER:install.exist.fail', array('{{error}}' => $appId));
		$alias = $manifest->getApplication('alias');
		if (!$alias) return new ErrorBag('APPCENTER:install.fail.alias.empty');
		/*
		 * if (!preg_match('/^[a-z][a-z0-9]+$/i', $alias)) return new
		 * ErrorBag('APPCENTER:illegal.alias');
		 */
		$result = $this->_load()->findByAlias($alias);
		if ($result instanceof ErrorBag) return $result;
		if ($result) return new ErrorBag('APPCENTER:install.exist.fail', array('{{error}}' => $alias));
		
		$hooks = $manifest->getHooks();
		if ($hooks) {
			$result = $this->_loadPwHooks()->batchFetchByName(array_keys($hooks));
			if ($result) {
				return new ErrorBag('HOOK:hook.exit',
				...
				if ($result) {
					...
					foreach ($inject as $key => $value) {
						...
						if (isset($injects[$_hookName]) && in_array($value['alias'], $injects[$_hookName])) {
							return new ErrorBag('HOOK:inject.exit', array('{{error}}' => $value['alias']));
						}
					}
				}
			}
	...
	public function afterInstall($install) {
		if ($install->getTmpPackage()) {
			...
			if (!$writable) return new ErrorBag('APPCENTER:install.mv.fail',
	...
	public function registeData($install) {
		try {
			...
			return new ErrorBag('APPCENTER:install.fail', array('{{error}}' => $e->getMessage()));
		}
	...
	public function registeApplication($install) {
		...
		$application->setCreatedTime(Tool::getTime());
		$application->setModifiedTime(Tool::getTime());
		...
		if (!$application->beforeAdd()) return new ErrorBag('APPCENTER:install.mainfest.fail');
	...
	private function _loadPwHookInject() {
		return app('SRV:hook.PwHookInject');
	}
