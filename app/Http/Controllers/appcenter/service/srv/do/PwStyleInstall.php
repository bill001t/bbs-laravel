...
class PwStyleInstall extends PwInstall {
	...
	public function install($install) {
		...
		if ($result) return new ErrorBag('APPCENTER:install.exist.fail');
		$alias = $manifest->getApplication('alias');
		if (!$alias) return new ErrorBag('APPCENTER:install.fail.alias.empty');
		/* if (!preg_match('/^[a-z][a-z0-9]+$/i', $alias)) return new ErrorBag('APPCENTER:illegal.alias'); */
	...
	public function afterInstall($install) {
		...
		if (!PwSystemHelper::checkWriteAble($targetDir . '/')) {
			return new ErrorBag('APPCENTER:install.mv.fail',
