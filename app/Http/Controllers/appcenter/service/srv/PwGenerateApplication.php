...
class PwGenerateApplication {
	...
	public function generate() {
		...
		if (is_dir($this->baseDir)) return new ErrorBag('APPCENTER:alias.exist');
		...
		if (!$writable) {
			return new ErrorBag('APPCENTER:generate.copy.fail');
		}
	...
	public function generateHook($hookname) {
		...
		if ('s_' == $prefix) {
			...
			return new ErrorBag('APPCENTER:generate.unsupport.hook');
		}
	}

	public function generateServiceHook($hookname) {
		$hookInfo = app('hook.PwHooks')->fetchByName($hookname);
		list($description, , $interface) = explode("\r\n", $hookInfo['document']);
		if (!$interface) return new ErrorBag('APPCENTER:generate.unsupport.hook');
		$this->baseDir = Wind::getRealDir('EXT:' . $this->alias);
	...
	public function generateSimpleHook($hookname) {
		...
		if (!$doc) return new ErrorBag('APPCENTER:generate.unsupport.hook');
		...
		$method = Utility::lcfirst($this->_ucwords($this->alias)) . 'Do';
		$hook = array(
			'app_' . $this->alias => array(
				'class' => 'EXT:' . $this->alias . '.service.srv.' . $classname, 
				'loadway' => 'load', 
