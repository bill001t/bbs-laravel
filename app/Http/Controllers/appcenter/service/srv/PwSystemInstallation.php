...
class PwSystemInstallation extends PwInstallApplication {
	...
	public function checkUpgrade() {
		...
		$r = WindConvert::convert($r, Core::V('charset'), 'utf8');
	...
	public function getNotice($loginUser) {
		...
		if (app('ADMIN:service.srv.AdminFounderService')->isFounder($loginUser->username)) {
			$upgradeInfo = $this->checkUpgrade();
			$upgradeInfo && is_array($upgradeInfo) && $notice |= 1;
			$patchInfo = app('APPCENTER:service.srv.PwPatchUpdate')->checkUpgrade();
		...
		switch ($notice) {
			case 1:
				...
				$url = 'appcenter/upgrade/run';
				break;
			case 2:
				$notice = '您正在使用的版本有更新补丁，请安装补丁。';
				$url = 'appcenter/fixup/run';
				break;
			case 3:
	...
	public function checkEnvironment() {
		if (!function_exists('curl_init')) return new ErrorBag('APPCENTER:upgrade.curl');
		// if (!function_exists('gzinflate')) return new
		// ErrorBag('APPCENTER:upgrade.gzinflate');
		return true;
	}
	...
	public function download($downloadUrl, $hash, $file = '') {
		if ($this->useZip) {
			...
			if (!$bool) return new ErrorBag($package);
			if ($hash !== md5_file($package)) {
				$this->_log(
					'check Package fail. expected hash:' . $hash . 'real hash :' . md5_file(
						$package));
				return new ErrorBag('APPCENTER:install.checkpackage.fail');
			}
			...
			if ($r === false) {
				...
				return new ErrorBag('APPCENTER:install.checkpackage.format.fail',
					array('{{error}}' => $r));
			}
			...
			list($bool, $_file) = PwSystemHelper::download($downloadUrl . '/' . $file . 'wind', 
				$_file);
			if (!$bool) return new ErrorBag($_file);
			if ($hash !== md5_file($_file)) {
				$this->_log(
					'check file fail. expected hash:' . $hash . 'real hash :' . md5_file($_file));
				return new ErrorBag('APPCENTER:upgrade.check.file.fail');
			}
	...
	public function sortDirectory($fileList) {
		...
		if (!is_array($directory)) {
			return new ErrorBag('APPCENTER:upgrade.directory.fail');
		}
		...
		if (!$sourceMd5 = WindFile::read($sourceDir . DIRECTORY_SEPARATOR . 'conf/md5sum')) {
			return new ErrorBag('APPCENTER:upgrade.target.hash.fail');
		}
		...
		if (!$tmp = WindFile::read(CONF_PATH . 'md5sum')) {
			return new ErrorBag('APPCENTER:upgrade.hash.fail');
		}
		...
		Core::cache()->set('system_upgrade_replace', 1);
		return $newFileList;
	}
	...
	public function after($fileList, $useFtp, $oldList) {
		if (Core::cache()->get('system_upgrade_replace')) return true;
		$relativePath_1 = PwSystemHelper::resolveRelativePath(PUBLIC_PATH, 
			Wind::getRealPath('SRC:wekit'));
		$relativePath_2 = PwSystemHelper::resolveRelativePath(PUBLIC_PATH . 'aCloud', 
			Wind::getRealPath('SRC:wekit'));
		...
		if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
			foreach ($oldList as $v) {
				...
				if ('.php' === substr($baseFile, -4) && (in_array($baseFile, $entrance) || !strncasecmp($baseFile, 'update_' . DIRECTORY_SEPARATOR, 4))) {
					...
					$tmp = tempnam($this->tmpPath, basename($_v) . Utility::generateRandStr(3));
		...
		if ($useFtp) {
			try {
				...
				return new ErrorBag(array('APPCENTER:upgrade.ftp.fail', array($e->getMessage())));
			}
		}
		foreach ($move as $k => $v) {
			if ($useFtp) {
				try {
					...
					if ($useFtp['sftp'] && !$r && $e = $ftp->getError()) {
						return new ErrorBag('APPCENTER:upgrade.upload.fail',
	...
	public function doUpgrade($fileList, $useFtp = 0) {
		...
		foreach ($fileList as $v => $hash) {
			if (!is_file($source . DIRECTORY_SEPARATOR . $v)) {
				return new ErrorBag('APPCENTER:upgrade.file.lost');
			}
			if ($useFtp) {
				try {
					...
					return new ErrorBag('APPCENTER:upgrade.upload.fail', array($v));
				}
			} else {
				WindFolder::mkRecur(dirname(ROOT_PATH . $v));
				$r = @copy($source . DIRECTORY_SEPARATOR . $v, ROOT_PATH . $v);
				if (!$r) return new ErrorBag('APPCENTER:upgrade.copy.fail', array($v));
			}
	...
	public function backUp($fileList) {
		...
		foreach ($fileList as $v => $hash) {
			if (is_file($root . DIRECTORY_SEPARATOR . $v)) {
				...
				if (!$r) return new ErrorBag('APPCENTER:upgrade.backup.fail');
			}
		}
	...
	public function flushLog() {
		if ($this->target) {
			...
			if (file_exists($logFile) && $log = WindFile::read($logFile)) {
				app('patch.PwUpgradeLog')->addLog($this->target, substr($log, 0, 60000));
			}
		}
	}
