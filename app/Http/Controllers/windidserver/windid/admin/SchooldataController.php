...
class SchooldataController extends WindidBaseController {
	 ...
	 */
	public function run() {
		list($type, $areaid, $name, $first) = $request->get(array('type', 'areaid', 'name', 'first'));
		!$type && $type = 3;
		...
		$list = array();
		$areaRoute = app('WSRV:area.srv.WindidAreaService')->getAreaRout($areaid);
		...
		->with($list, 'list');
		->with($schoolSo->getData(), 'data');
		->with($_rout, 'route');
		->with($type, 'type');
		->with($this->type, 'schools');
	}
	...
	public function updateAction(Request $request) {
		list($update, $add, $areaid, $typeid) = $request->get(array('update', 'add', 'areaid', 'typeid'), 'post');
		...
		foreach ($update as $id => $name) {
			...
			if ($r instanceof WindidError) return $this->showError('WINDID:code.' . $r->getCode());
		}
		$addDms = array();
		foreach ($add as $name) {
			$dm = new WindidSchoolDm();
		...
		if ($r instanceof WindidError) return $this->showError('WINDID:code.' . $r->getCode());
		return $this->showMessage('success', 'windid/schooldata/run?type=' . $typeid . '&areaid=' . $areaid);
	}
	...
	public function deleteAction(Request $request) {
		...
		if (!$schoolid) return $this->showError('ADMIN:school.schoolid.error');
		$this->_getDs()->deleteSchool($schoolid);
		return $this->showMessage('ADMIN:school.delete.success');
	}
