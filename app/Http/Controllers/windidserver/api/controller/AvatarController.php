...
class AvatarController  extends OpenBaseController{
	...
	public function __construct() {
		$this->attachUrl = Core::url()->attach;
	}

	public function getAvatarUrlAction(Request $request) {
		$result = substr(Tool::getPath('1.gpg'), 0, -6);
		$this->output($result);
	}
	...
	public function setStoragesAction(Request $request) {
		$storage = $request->get('storage', 'post');
		$attService = app('LIB:storage.PwStorage');
	...
	public function getAction(Request $request) {
		...
		$size = $request->get('size', 'get');
		!$size && $size = 'middle';
 		$file = $uid . (in_array($size, array('middle', 'small')) ? '_' . $size : '') . '.jpg';
		$result = $this->attachUrl . '/avatar/' . Tool::getUserDir($uid) . '/' . $file;
		$this->output($result);
	}
	...
	public function defaultAction(Request $request) {
		...
		!$type && $type = 'face';
		$srv = app('WSRV:user.srv.WindidUserService');
	 ...
	 * 上传头像
	 */
	public function doavatarAction(Request $request) {
