...
class SetbbsController extends AdminBaseController {
	...
	public function run() {
		return redirect('bbs/setbbs/thread');
	...
	public function dorunAction(Request $request) {
		...
		list($showBirthdayMembers, $showLinks, $showOnlineUsers, $listOnlineUsers) = $request->get(
	...
	public function dothreadAction(Request $request) {
		$request->isPost() || return $this->showError('operate.fail');

		list($newThreadMinutes, $perpage, $maxPages, $leftsideWidth, $hotthreadReplies) = $request->get(
	...
	public function readAction(Request $request) {
		...
		->with($order, 'order');
		->with($allInfo, 'allInfo');
		->with($config, 'config');
	}

	public function doreadAction(Request $request) {
		$request->isPost() || return $this->showError('operate.fail');

		$arrInput = $request->get(
			array(
				'perpage', 
		...
		$config->set('read.perpage', $arrInput['perpage'])
				...
				->flush();
		return $this->showMessage('success', 'bbs/setbbs/read');
	}
}
?>