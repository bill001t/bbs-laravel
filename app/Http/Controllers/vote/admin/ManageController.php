...
class ManageController extends AdminBaseController {
	...
	public function run() {
		...
		->with($map[0], 'cateList');
		->with($cateIds, 'cateIds');
		->with($pollOpenForum, 'pollOpenForum');
		->with($this->_buildGroup($this->_getUserGroupsDs()->getAllGroups()), 'groups');
		->with($this->_getUserGroupsDs()->getTypeNames(), 'groupsTypeName');
		->with($this->_buildPermission($this->_getUserPermissionDs()->fetchPermissionByRkey(array('allow_add_vote', 'allow_participate_vote', 'allow_view_vote'))), 'permission');
	}
	
	public function dogroupAction(Request $request){
		$request->isPost() || return $this->showError('operate.fail');
		$view = $request->get('view', 'post');
	...
	public function doeditforumAction(Request $request){
		$request->isPost() || return $this->showError('operate.fail');
		$forumid = $request->get('fid', 'post');
