...
namespace App\Services\invite\bs;

use App\Core\ErrorBag;
use App\Services\invite\dm\PwInviteCodeDm;
use App\Services\invite\ds\dao\PwInviteCodeDao;
...
class PwInviteCode
{
    ...
    {
        if (($r = $dm->beforeAdd()) instanceof ErrorBag) return $r;
    ...
    public function batchAddCode($codeDms)
    {
        ...
        foreach ($codeDms as $dm) {
            if (!($dm instanceof PwInviteCodeDm)) return new ErrorBag('USER:invite.data.format.error');
            if (($r = $dm->beforeAdd()) instanceof ErrorBag) return $r;
            $data[] = $dm->getData();
        }
    ...
    {
        if (($r = $dm->beforeUpdate()) instanceof ErrorBag) return $r;
    ...
    {
        return app(PwInviteCodeDao::class);
    }
}