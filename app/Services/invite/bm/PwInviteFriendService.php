...
namespace App\Services\invite\bm;

use App\Core\ErrorBag;
use App\Services\attention\bm\PwAttentionService;
...
class PwInviteFriendService
{
    ...
    public function inviteRegist($inviteCode, $invited_uid)
    {
        ...
        if (!$bUser) return new ErrorBag('USER:invite.friend.code.invitedUid.error');

        /* @var $inviteLogDs PwInviteFriendLog */
        /* $inviteLogDs = app('invite.PwInviteFriendLog');
        $inviteLogDs->addLog($aUser['uid'], $invited_uid, Tool::getTime()); */

        //邀请成功，被邀请者关注邀请者
        /* @var $attentionSrv PwAttentionService */
        $attentionSrv = app('attention.srv.PwAttentionService');
        $attentionSrv->addFollow($invited_uid, $aUser['uid']);
// 		$attentionSrv->addFollow($aUser['uid'], $invited_uid);

        //邀请成功，增加积分
        /* $inviteConfig = Core::getConfig('site');
        ...
        // 邀请成功--邀请好友的任务
// 		SimpleHook::getInstance('PwInviteFriendService_invite')->runDo($aUser['uid'], $invited_uid);
        return true;
    }
    ...
    public function invite($inviteCode, $invited_uid)
    {
        ...
        $bUser = $this->_getUserDs()->getUserByUid($invited_uid);
        if (!$bUser) return new ErrorBag('USER:invite.friend.code.invitedUid.error');
        if ($aUser['uid'] == $bUser['uid']) return new ErrorBag('USER:invite.friend.invite.self');
    ...
    {
// 		$invite_config = Core::C('site');
// 		if (!$invite_config['invite.friend.isOpen']) return new ErrorBag('USER:invite.friend.code.close');
        if (2 == Core::C('register', 'type')) return new ErrorBag('USER:invite.friend.code.dumplic');
        return true;
    }
    ...
    public function checkInviteCode($code)
    {
        ...
        if (!$info) return new ErrorBag('USER:invite.friend.code.illage');
        return $info;
    }
    ...
    private function _addFans($aUser, $bUser)
    {
        ...
        $attentionDs = app(PwAttention::class);
        ...
        if ($aFollowedB && $bFollowedA) {
            return new ErrorBag('USER:invite.friend.exists', array('{name}' => $aUser['username']));
        } elseif (!$aFollowedB && !$bFollowedA) {
            //邀请成功，相互关注
            /* @var $attention PwAttentionService */
            $attention = app(PwAttentionService::class);
