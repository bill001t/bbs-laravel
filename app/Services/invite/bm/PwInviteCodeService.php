...
use App\Services\invite\dm\PwInviteCodeDm;
use App\Services\invite\vo\PwInviteCodeSo;
use App\Services\invite\bs\PwInviteCode;
...
class PwInviteCodeService
{
    ...
    public function allowUseInviteCode($code)
    {
        ...
        if (!$info) return new ErrorBag('USER:invite.code.error');
        if (1 == $info['ifused']) {
            return new ErrorBag('USER:invite.code.isused');
        }

        $time = Core::C('register', 'invite.expired');
        $expireTime = Tool::getTime() - ($time * 86400);

        if ($info['created_time'] < $expireTime) return new ErrorBag('USER:invite.code.expired');
    ...
    public function searchInvitecodeList(PwInviteCodeSo $search, $limit = 10, $start = 0)
    {
        ...
        $result = array();
        $time = Core::C('register', 'invite.expired');
        $expire = Tool::getTime() - ($time * 86400);
        $_invitedUid = array();
    ...
    {
        if (!CommonValidator::isPositive($num)) return new ErrorBag('USER:invite.buy.num.error');
        $num = intval($num);
        ...
        if (($readyBuy + $num) > $gidLimit) {
            return new ErrorBag('USER:invite.buy.num.24h.limit', array('{num}' => $gidLimit, '{readynum}' => $readyBuy));
        }

        $price = abs(ceil($user->getPermission('invite_buy_credit_num')));
        if (($price * $num) > $user->getCredit($creditType)) {
            return new ErrorBag('USER:invite.buy.credit.no.enough', array('{num}' => $user->getCredit($creditType), '{buynum}' => $num));
        }
    ...
    {
        $time = Tool::getTime();
        $string = Utility::generateRandStr(32 - strlen($uid) - strlen($time));
