...
namespace App\Services\user\cache;
...
class PwUserDbCache extends PwBaseMapDbCache {
	...
	public function getUserByUid($uid) {
		$data = Core::cache()->fetch($this->getKeysByUid($uid));
		$result = array();
	...
	public function fetchUserByUid($uids) {
		$result = array();
		$data = Core::cache()->fetch($this->fetchKeysByUid($uids));
		foreach ($data as $key => $value) {
	...
	public function editUser($uid, $fields, $increaseFields = array(), $bitFields = array()) {
		Core::cache()->batchDelete($this->getKeysByUid($uid));
	...
	public function batchDeleteUser($uids) {
		Core::cache()->batchDelete($this->fetchKeysByUid($uids));
