...
class PwUserBo
{
    ...
    public function comparePermission($uids)
    {
        ...
        if ($gids = array_diff(Tool::collectByKey($users, 'groupid'), array('0'))) {
    ...
    public function reset()
    {
        ...
        $this->username = '游客';
        $this->info = array(
            'lastpost' => Tool::getCookie('guest_lastpost')
        );
    }
    ...
    private function _initGroup()
    {
        ...
        if (($group = Core::cache()->get('group', $this->gid)[0]) === false) {
            $group = Core::cache()->get('group', 1)[0];
        }
