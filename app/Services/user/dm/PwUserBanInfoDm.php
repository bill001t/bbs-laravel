...
class PwUserBanInfoDm extends BaseDm
{
    ...
    {
        if (!$this->getField('created_time')) $this->_data['created_time'] = Tool::getTime();
        if (!$this->getField('uid')) return new ErrorBag('USER:ban.type.require');
        if (!$this->getField('typeid')) return new ErrorBag('USER:ban.type.require');
        if (!$this->getField('reason')) return new ErrorBag('USER:ban.reason.require');
        return true;
    }
