...
class PwUserInfoDm extends BaseDm
{
    ...
    public function getDm()
    {
        if (!is_object($this->dm)) {
            $dm = app(\App\Services\Api\user\dm\WindidUserDm::class);
    ...
    public function setPassword($password)
    {
        ...
        $this->_data['password'] = md5(Utility::generateRandStr(16));
        return $this;
    }
    ...
    {
        if (0 >= $this->uid) return new ErrorBag('USER:illegal.id');
    ...
    {
        if (!$this->dm) {
            return new ErrorBag('USER:user.info.error');
        }
    ...
    protected function check()
    {
        ...
        //【用户资料验证】邮编格式验证
        if (($_tmp = $this->getField('zipcode')) && (false === CommonValidator::isZipcode($_tmp))) {
            return new ErrorBag('USER:error.zipcode');
        }
        //【用户资料验证】个人主页长度限制
        if (($_tmp = $this->getField('homepage')) && (false === CommonValidator::isUrl($_tmp) || true === CommonValidator::isLegalLength($_tmp, 200))) {
            return new ErrorBag('USER:error.homepage');
        }
        //【用户资料验证】自我简介长度限制
        if (($_tmp = $this->getField('profile')) && (true === CommonValidator::isLegalLength($_tmp, 250))) {
            return new ErrorBag('USER:error.profile.length', array('{length}' => 250));
        }

        //TODO【用户资料验证】BBS签名验证长度判断----后台权限设置
        /*
        if (($_tmp = $this->getField('bbs_sign')) && (true === CommonValidator::isLegalLength($_tmp, 500))) {
            return new ErrorBag('USER:error.bbs_sign.length', array('{length}' => 500));
        }
        */
        return true;
    }
    ...
    protected function _getWindidMsg($result)
    {
        ...
        return new ErrorBag('WINDID:code.' . $errorCode, $var);
    }
}