...
class PwUserValidator
{
    public static function isUsernameHasIllegalChar($username)
    {
        ...
        if (0 >= preg_match('/^[\x7f-\xff\dA-Za-z\.\_]+$/', $username)) {
            return new ErrorBag('USER:error.username');
        }
        return false;
    }
    ...
    {
        if (0 >= preg_match('/^1\d{10}$/', $mobile)) return new ErrorBag('USER:mobile.error.formate');
        return true;
    }
    ...
    public static function isTelPhone($telPhone)
    {
        if (0 >= preg_match('/^[0-9][-\d]*\d*$/', $telPhone)) {
            return new ErrorBag('USER:error.telphone');
        }
        return true;
    }
...
//		$userDs = Wekit::load('user.PwUser');
    ...
    public static function isEmailValid($email, $username = '', $uid = 0)
    {
        ...
        if ($result < 1) {
            return new ErrorBag('USER:user.error.' . $result);
        }
        return true;
    }
    ...
    {
        if (!$username) return new ErrorBag('USER:user.error.-1');
        $result = self::_getWindid()->checkUserInput($username, 1, '', $uid);
        if ($result < 1) {
            if ($result == -2) {
                $config = app(\App\Core\WindidBo::class)->config->C('reg');
                return new ErrorBag('WINDID:code.-2', array('min' => $config['security.username.min'], 'max' => $config['security.username.max']));
            }
    ...
    public static function checkUsernameExist($username, $exceptUid = 0)
    {
        ...
        if ($result < 1) return new ErrorBag('WINDID:code.' . $result);
        /* @var $userDs PwUser */
        /*	$userDs = Wekit::load('user.PwUser');
    ...
    public static function isPwdValid($password, $username)
    {
        ...
        if ($result < 1) {
            ...
            return new ErrorBag('WINDID:code.' . $result, $var);
        }
    ...
    {
        $register = app(\App\Core\WindidBo::class)->config->C('reg');
        if (!($pwdConfig = $register['security.password'])) return true;
        $config = array_sum($pwdConfig);
        if (in_array(9, $pwdConfig)) {
            $config = $config - 9;
            if ($username == $password) return new ErrorBag('USER:pwd.error.equalUsername');
        }
        if ($config == 0) return true;
        if (self::_complexCaculate($password, $config)) return new ErrorBag('USER:pwd.error.complex', array('type' => self::buildPwdComplexMsg($pwdConfig)));
        return true;
    }
    ...
    public static function buildPwdShowMsg()
    {
        ...
        if(isset($config['security.password.min'])){
            $_min = $config['security.password.min'];
        }else{
            $_min = '';
        }

        if(isset($config['security.password.max'])){
            $_max = $config['security.password.max'];
        }else{
            $_max = '';
        }

        if(isset($config['security.password'])){
    ...
    public static function buildNameShowMsg()
    {
        ...
        if(isset($config['security.username.min'])){
            $_min = $config['security.username.min'];
        }else{
            $_min = '';
        }

        if(isset($config['security.username.max'])){
