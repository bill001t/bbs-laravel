...
use App\Services\user\ds\dao\PwUserRegisterIpDao;
...
class PwUserRegisterIp {
	...
	public function updateRecodeByIp($ip, $lastDate) {
		if (!$ip) return false;
		!$lastDate && $lastDate = Tool::getTime();
		return $this->_getDao()->_update($ip, $lastDate);
	}
	...
	private function _getdao() {
		return app(PwUserRegisterIpDao::class);
	}
}