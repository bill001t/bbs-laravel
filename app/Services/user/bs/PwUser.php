...
use App\Services\credit\dm\PwCreditDm;
...
class PwUser
{
    ...
    public function editUser(PwUserInfoDm $dm, $type = self::FETCH_ALL)
    {
        ...
        SimpleHook::getInstance('PwUser_update')->runDo($dm);
        return true;
    }
    ...
    public function addUser(PwUserInfoDm $dm)
    {
        ...
        if (($uid = $this->_getWindid()->addDmUser($dm->dm)) < 1) {
            return new ErrorBag('WINDID:code.' . $uid);
        }
        $dm->setUid($uid);
        $this->_getDao(self::FETCH_ALL)->addUser($dm->getSetData());
        SimpleHook::getInstance('PwUser_add')->runDo($dm);
        return $uid;
    }
    ...
    public function activeUser($uid)
    {
        ...
        $data['password'] = md5(Utility::generateRandStr(16));
    ...
    public function synEditUser($uid, $changepwd = 0)
    {
        ...
        $changepwd && $data['password'] = md5(Utility::generateRandStr(16));
    ...
    {
        if (0 >= ($uid = intval($uid))) return new ErrorBag('USER:illegal.id');
        $this->_getDao(self::FETCH_ALL)->deleteUser($uid);
        $this->_getWindid()->deleteUser($uid);
        SimpleHook::getInstance('PwUser_delete')->runDo($uid);
        return true;
    }
    ...
    public function batchDeleteUserByUid($uids)
    {
        ...
        SimpleHook::getInstance('PwUser_batchDelete')->runDo($uids);
        return true;
    }
    ...
    protected function _getDao($type = self::FETCH_MAIN)
    {
        ...
        $type == self::FETCH_MAIN && $dao = \App\Services\user\ds\dao\PwUserDao::class;
        $type == self::FETCH_DATA && $dao = \App\Services\user\ds\dao\PwUserDataDao::class;
        $type == self::FETCH_INFO && $dao = \App\Services\user\ds\dao\PwUserInfoDao::class;
        empty($dao) && $dao = \App\Services\user\ds\dao\PwUserAllDao::class;
