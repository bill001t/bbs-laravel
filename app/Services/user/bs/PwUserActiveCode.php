...
use App\Services\user\ds\dao\PwUserActiveCodeDao;
...
class PwUserActiveCode {
	...
	public function addActiveCode($uid, $email, $code, $time, $typeid = self::REGIST) {
		if (($uid = intval($uid)) <= 0) return new ErrorBag('USER:illegal.id');
		if (!CommonValidator::isEmail($email)) return new ErrorBag('USER:user.error.-7');
	...
	public function activeCode($uid, $activeTime) {
		if (($uid = intval($uid)) <= 0) return new ErrorBag('USER:illegal.id');
		return $this->getDao()->_update($uid, $activeTime);
	}
	...
	private function getDao() {
		return app(PwUserActiveCodeDao::class);
	}
}