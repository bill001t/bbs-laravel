...
class PwUserBanAvatar implements PwUserBanTypeInterface
{
    ...
    public function afterBan(PwUserBanInfoDm $dm)
    {
        ...
        if (Tool::getstatus($info['status'], PwUser::STATUS_BAN_AVATAR)) return $info['status'];//已经禁止不需要再次更改
    ...
    public function deleteBan($uid)
    {
        ...
        if (!Tool::getstatus($info['status'], PwUser::STATUS_BAN_AVATAR)) return $info['status'];//已经解禁不需要再次更改
