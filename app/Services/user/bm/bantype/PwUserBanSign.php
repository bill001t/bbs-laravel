...
class PwUserBanSign implements PwUserBanTypeInterface
{
    ...
    public function afterBan(PwUserBanInfoDm $dm)
    {
        ...
        if (Tool::getstatus($info['status'], PwUser::STATUS_BAN_SIGN)) return $info['status'];//已经禁止不需要再次更改

        //$end_time = $dm->getField('end_time') > 0 ? Tool::time2str($dm->getField('end_time'), 'Y年m月d日 H:i') : '永久';
        //构建显示禁止签名的文本内容
    ...
    public function deleteBan($uid)
    {
        ...
        if (!Tool::getstatus($info['status'], PwUser::STATUS_BAN_SIGN)) return $info['status'];//已经解禁不需要再次更改
