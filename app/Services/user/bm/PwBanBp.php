...
class PwBanBp
{
    ...
    {
        $this->uid = intval($uid);
        $this->banType = app(PwUserBanService::class)->getBanType();
...
// 		if ($this->_getBanInfo($type)) return false;
        $class = app($this->banType[$type]['class']);
    ...
    private function _autoEndBan($type = PwUserBan::BAN_SPEAK)
    {
        ...
        foreach ($this->banInfo[$type][1] as $_key => $item) {
            if (($item['end_time'] != 0) && $item['end_time'] < $now) {
                $clear[] = $item['id'];
                //操作相关类型的后续操作
                $class = app($this->banType[$item['typeid']]['class']);
        ...
        if ($clear) {
            ...
            $banService = app(PwUserBanService::class);
