...
class PwFindPassword
{
    ...
    {
        if (!$this->info) return new ErrorBag('USER:illegal.request');
        if ($this->info['email'] != $email) {
            return new ErrorBag('USER:findpwd.error.email');
        }
    ...
    public function sendResetEmail($state)
    {
        ...
        $url = url('u/findPwd/resetpwd', ['code' => $code, '_statu' => $state]);
        list($title, $content) = $this->_buildTitleAndContent($this->info['username'], $url);
        $activeCodeDs = app(PwUserActiveCode::class);
        $activeCodeDs->addActiveCode($this->info['uid'], $this->info['email'], $code, Tool::getTime(), PwUserActiveCode::RESETPWD);
        $mail = new PwMail();
    ...
    public function checkResetEmail($email, $code)
    {
        if (empty($this->info) || $this->info['email'] != $email) {
            return new ErrorBag('USER:illegal.request');
        }
        /* @var $activeCodeDs PwUserActiveCode */
        $activeCodeDs = app(PwUserActiveCode::class);
        $info = $activeCodeDs->getInfoByUid($this->info['uid'], PwUserActiveCode::RESETPWD);
        if (!$info || $info['email'] != $email || $info['code'] != $code) return new ErrorBag("USER:findpwd.email.code.expired");
        /*找回密码：验证码不需要过期及验证机制*/
        if ($info['active_time'] > 0) return new ErrorBag('USER:findpwd.email.code.expired');
...
//		$activeCodeDs->activeCode($this->info['uid'], Tool::getTime());
        return true;
    }
    ...
    private function _buildTitleAndContent($username, $url = '')
    {
        ...
        $title = str_replace($search, $replace, Core::C('login', 'resetpwd.mail.title'));
        $search[] = '{time}';
        $search[] = '{url}';
        $replace[] = Tool::time2str(Tool::getTime(), 'Y-m-d H:i:s');
        $replace[] = $url ? sprintf('<a href="%s">%s</a>', $url, $url) : '';
        $content = str_replace($search, $replace, Core::C('login', 'resetpwd.mail.content'));
    ...
    {
        if (false === $this->isBindMail()) return new ErrorBag('USER:findpwd.notbind.email');
    ...
    {
        if (false === $this->isBindMobile()) return new ErrorBag('USER:findpwd.notbind.mobile');
    ...
    {
        return $this->allowFindBy(self::WAY_EMAIL) instanceof ErrorBag ? true : false;
    }
    ...
    {
        return $this->allowFindBy(self::WAY_MOBILE) instanceof ErrorBag ? true : false;
    }
    ...
    {
        $code = Tool::encrypt($username . '|' . $way . '|' . $value, Core::C('site', 'hash') . '___findpwd');
    ...
    {
        return explode("|", Tool::decrypt(rawurldecode($identify), Core::C('site', 'hash') . '___findpwd'));
    }
    ...
    private function allowFindBy($type = self::WAY_EMAIL)
    {
        ...
        if (($time != Tool::time2str(Tool::getTime(), 'Y-m-d')) || $num < $tryNum) return true;
        return new ErrorBag('USER:findpwd.over.limit.' . $type);
    }
    ...
    public function setRecode($type = self::WAY_EMAIL)
    {
        ...
        /*如果重来没有尝试找回过密码*/
        if (!$findPwd) {
            $recodes[$typeCode] = Tool::time2str(Tool::getTime(), 'Y-m-d') . ':1';
        } else {
            ...
            if (!($recode = $recodes[$typeCode])) {
                ...
                if (($time != Tool::time2str(Tool::getTime(), 'Y-m-d'))) {
                    $recodes[$typeCode] = Tool::time2str(Tool::getTime(), 'Y-m-d') . ':1';
