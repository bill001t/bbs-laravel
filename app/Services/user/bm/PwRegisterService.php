...
use App\Services\user\bs\PwUserActiveCode;
use App\Services\user\bs\PwUserRegisterCheck;
use App\Services\user\bs\PwUserRegisterIp;
 ...
 */
class PwRegisterService extends BaseHookService
{
    public $config = null;
    ...
    public function checkIp($ip)
    {
        ...
        $registerDs = app(PwUserRegisterIp::class);
    ...
    {
        if (!$this->userDm) return new ErrorBag('USER：illegal.request');
        if (($result = $this->checkIp($this->userDm->getField('regip'))) instanceof ErrorBag) {
            return $result;
        }
        //[c_register]:调用插件中用户注册操作的前置方法beforeRegister
        if (($result = $this->runWithVerified('beforeRegister', $this->userDm)) instanceof ErrorBag) {
            return $result;
        }
        if (($uid = $this->_getUserDs()->addUser($this->userDm)) instanceof ErrorBag) {
            return $uid;
        }
        //记录一下ip次数
        $ip = Request::ip();
        $this->_recordIpLimit($uid, $ip);
        //
    ...
    public function sysUser($uid)
    {
        ...
        if (!$info) {
            ...
            app(PwUserService::class)->restoreDefualtAvatar($info['uid']);

            $pwUserInfoDm->setUsername($_userinfo['username']);
            $pwUserInfoDm->setEmail($_userinfo['email']);
            $pwUserInfoDm->setRegip(Request::ip());
    ...
    public function sendEmailActive($username, $email, $statu = '', $uid = 0)
    {
        ...
        if ($uid == 0 || !$statu) {
            ...
            if ($info['email'] != $email) return new ErrorBag('USER:illegal.request');
            $uid = $info['uid'];
            $statu = self::createRegistIdentify($uid, $info['password']);
        }

        $code = substr(md5(Tool::getTime()), mt_rand(1, 8), 8);
        $url = url('u/register/activeEmail', ['code' => $code, '_statu' => $statu]);
        ...
        $activeCodeDs = app(PwUserActiveCode::class);
        $activeCodeDs->addActiveCode($uid, $email, $code, Tool::getTime());
        $mail = new PwMail();/*亟需补充*/
    ...
    public function checkIfActiveEmailSend($uid, $email)
    {
        ...
        //过期了
        if (($info['send_time'] + $validTime) < Tool::getTime()) return false;
        return true;
    }
    ...
    public function activeEmail($uid, $email, $code)
    {
        $activeCodeDs = app(PwUserActiveCode::class);
        $info = $activeCodeDs->getInfoByUid($uid);
        if (!$info || $info['email'] != $email || $info['code'] != $code) return new ErrorBag("USER:illegal.request");
        if ($info['active_time'] > 0) return new ErrorBag('USER:active.email.dumplicate');
        $validTime = $this->activeCodeValidTime * 3600;
        if (($info['send_time'] + $validTime) < Tool::getTime()) return new ErrorBag('USER:active.email.overtime');
        $activeCodeDs->activeCode($uid, Tool::getTime());

        $info = $this->_getUserDs()->getUserByUid($uid, PwUser::FETCH_MAIN);
        if (Tool::getstatus($info['status'], PwUser::STATUS_UNACTIVE)) {
            $userDm = new PwUserInfoDm($info['uid']);
            $userDm->setUnactive(false);
            (!Tool::getstatus($info['status'], PwUser::STATUS_UNCHECK)) && $userDm->setGroupid(0);
            $this->_getUserDs()->editUser($userDm, PwUser::FETCH_MAIN);
        }
        $registerCheckDs = app(PwUserRegisterCheck::class);
    ...
    public function sendWelcomeMsg($uid, $username, $email)
    {
        ...
        if (in_array(1, $this->config['welcome.type'])) {
            ...
            $notice = app(PwNoticeService::class);
    ...
    {
        $code = Tool::encrypt($uid . "\t" . Tool::getPwdCode($pwd));
    ...
    {
        return explode("\t", Tool::decrypt(rawurldecode($identify)));
    }
    ...
    {
        //app('user.srv.PwUserService')->restoreDefualtAvatar($uid); windid处理
        //获得注册积分
        $creditBo = PwCreditBo::getInstance();
        $creditBo->operate('register', new PwUserBo($userDm->uid));

        $this->updateRegisterIp($userDm->getField('regip'), Tool::getTime());
        ...
        if (($result = $this->runWithVerified('afterRegister', $userDm)) instanceof ErrorBag) return $result;
    ...
    protected function filterUserDm(PwUserInfoDm $userDm, $hasCredit = array())
    {
        ...
        $credit = $groupService->calculateCredit(Core::C('site', 'upgradestrategy'), $_credit);
    ...
    private function _buildTitleAndContent($titleKey, $contentKey, $username, $url = '')
    {
        ...
        $replace = array($username, Core::C('site', 'info.name'));
        $title = str_replace($search, $replace, $this->config[$titleKey]);
        $search[] = '{time}';
        $search[] = '{url}';
        $replace[] = Tool::time2str(Tool::getTime(), 'Y-m-d H:i:s');
    ...
    private function _recordIpLimit($uid, $ip)
    {
        $pwdBp = new PwTryPwdBp();
        $pwdBp->allowTryAgain($uid, $ip, true);
    }
}
