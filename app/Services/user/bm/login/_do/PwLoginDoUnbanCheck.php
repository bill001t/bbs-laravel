...
namespace App\Services\user\bm\login\_do;

use App\Core\Tool;
use App\Services\user\bm\login\PwUserLoginDoBase;
...
class PwLoginDoUnbanCheck extends PwUserLoginDoBase
{
    ...
    public function welcome(PwUserBo $userBo, $ip)
    {
        ...
        if ($banBp->checkIfBan()) {
            if (Tool::getstatus($userBo->info['status'], PwUser::STATUS_BAN_AVATAR) && false === $banBp->checkIfBanAvatar()) {
                $banBp->recoveryBanAvatarError();
            } else {
                $banBp->endDateTimeBanAvatar();
            }
            if (Tool::getstatus($userBo->info['status'], PwUser::STATUS_BAN_SIGN) && false === $banBp->checkIfBanSign()) {
            ...
            $userBo->info = array_merge($userBo->info, app('user.PwUser')->getUserByUid($userBo->uid));
