...
use App\Services\Api\AvatarApi;
...
class PwUserService
{
    ...
    public function verifyUser($username, $password, $type = 1, $question = null, $answer = null)
    {
        ...
        switch ($result[0]) {
            case 1://用户信息正常
                return $result[1];
            case -14://用户不存在
                return new ErrorBag('USER:verify.error.name');
            case -13://用户密码错误
                return new ErrorBag('USER:verify.error.pwd');
            case -20://用户安全问题错误
                return new ErrorBag('USER:verify.error.question');
        }
    }
    ...
    public function logout()
    {
        ...
        SimpleHook::getInstance('PwUserService_logout')->runDo($loginUser);
        $loginUser->reset();
        return Tool::setCookie('winduser', '', -1);
    }
    ...
    public function caculateUserGroupid($groupid, $userGroups)
    {
        ...
        if ($temp) {
            ...
            $groupDs = app(PwUserGroups::class);
    ...
    {
        $identity = Tool::encrypt($uid . "\t" . Tool::getPwdCode($password));
        return Tool::setCookie('winduser', $identity, $rememberme ? 31536000 : NULL);
    }
    ...
    {
        $dm = app(PwUserInfoDm::class, [$uid]);
        $dm->setLastvisit(Tool::getTime())->setLastloginip($ip);
    ...
    {
        $info = $this->_getWindidUser()->getUser($uid, 1);
    ...
    public function mustSettingSafeQuestion($uid)
    {
        ...
        $mustSettingGroups = Core::C('login', 'question.groups');
    ...
    {
        if (!$uid) return array();
        $info = app(PwUser::class)->getUserByUid($uid, PwUser::FETCH_MAIN);
