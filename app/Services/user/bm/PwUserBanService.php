...
use App\Services\user\vo\PwUserBanSo;
use App\Services\user\bs\PwUserBan;
...
class PwUserBanService
{
    ...
    public function banUser($dmList)
    {
        ...
        foreach ($dmList as $_dm) {
            ...
            //操作相关类型的后续操作
            $class = app($banTypes[$_dm->getField('typeid')]['class']);
    ...
    public function autoBan($uid, $fields, $increaseFields)
    {
        ...
        //执行禁止操作
        $dmList = array();
        $endTime = $config['autoForbidden.day'] > 0 ? $config['autoForbidden.day'] * 24 * 3600 + Tool::getTime() : 0;
