...
namespace App\Services\advertisement\bm;

use App\Core\ErrorBag;
use App\Core\Tool;
use App\Services\advertisement\bs\PwAd;
use App\Services\advertisement\dm\PwAdDm;
use Core;
...
class PwAdService
{
    ...
    {
        list($id, $width, $heigth, $status) = array(intval($id), intval($width), intval($height), intval($status));

        if ($status > 1) return new ErrorBag('ADVERTISEMENT:position.status.error');
        if (empty($id)) return new ErrorBag('ADVERTISEMENT:position.error');
        if ($identifier && $this->_getAdDs()->getByIdentifier($identifier)) return new ErrorBag('ADVERTISEMENT:position.exists');
        if (!array_key_exists($type, $this->_getAdDs()->getAdType())) return new ErrorBag('ADVERTISEMENT:type.error');
        $checkPostition = $this->_getAdDs()->getByPid($id);
        if ($checkPostition) return new ErrorBag('ADVERTISEMENT:position.exists');
        ...
        if (!$result) return new ErrorBag('ADVERTISEMENT:add.fail');
    ...
    {
        list($id, $width, $heigth, $status, $showType, $condition) = array(intval($id), intval($width), intval($height), intval($status), intval($showType), $condition);

        if ($status > 1) return new ErrorBag('ADVERTISEMENT:position.status.error');
        if (empty($id)) return new ErrorBag('ADVERTISEMENT:position.error');
        if (!array_key_exists($type, $this->_getAdDs()->getAdType())) return new ErrorBag('ADVERTISEMENT:type.error');
        $checkPostition = $this->_getAdDs()->getByPid($id);
        if (!$checkPostition) return new ErrorBag('ADVERTISEMENT:position.not.exists');
        ...
        if (!$result) return new ErrorBag('ADVERTISEMENT:edit.fail');
    ...
    */
    ...
    {
        $ads = $this->_getAdDs()->getAllAd();
        Core::cache()->set('advertisement', $ads);
    }

    private function _getAdDs()
    {
        return app(PwAd::class);
    }
}