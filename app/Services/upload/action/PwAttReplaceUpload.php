...
class PwAttReplaceUpload extends PwUploadAction {
	...
	public function __construct($user, $aid) {
		...
		$this->aid = $aid;
		$this->attach = app('attach.PwThreadAttach')->getAttach($aid);
		$this->forum = new PwForumBo($this->attach['fid']);
		$this->ftype = Core::C('attachment', 'extsize');
	}
	...
	public function check() {
		if (!$this->attach) {
			return new ErrorBag('attach.exists.not');
		}
		if (!$this->user->isExists()) {
			return new ErrorBag('login.not');
		}
		if (!$this->forum->isForum()) {
			return new ErrorBag('BBS:forum.fid.select');
		}
		if (($result = $this->forum->allowUpload($this->user)) !== true) {
			return new ErrorBag('BBS:forum.permissions.upload.allow', array('{grouptitle}' => $this->user->getGroupInfo('name')));
		}
		if (!$this->forum->foruminfo['allow_upload'] && !$this->user->getPermission('allow_upload')) {
			return new ErrorBag('permission.upload.allow', array('{grouptitle}' => $this->user->getGroupInfo('name')));
		}
		return true;
	}
	...
	function getSaveName(PwUploadFile $file) {
		$prename  = substr(md5(Tool::getTime() . $file->id . Utility::generateRandStr(8)), 10, 15);
	...
	public function allowThumb() {
		...
		return Core::C('attachment', 'thumb');
	}
	...
	public function update($uploaddb) {
		$srv = app('attach.PwThreadAttach');
		foreach ($uploaddb as $key => $value) {
			Tool::deleteAttach($this->attach['path'], $this->attach['ifthumb']);
			...
			$att->setCreatedTime(Tool::getTime());
			$att->setApp('thread');
			...
			if ($this->attach['tid'] && $this->attach['pid'] == 0 && $this->attach['type'] != $value['type']) {
				...
				if (!app('attach.PwThreadAttach')->countType($this->attach['tid'], 0, $this->attach['type'])) {
	...
	public function getAttachInfo() {
		...
		return array('aid' => $array['aid'], 'path' => Tool::getPath($array['path']), 'name' => $array['name']);
	}
}
?>