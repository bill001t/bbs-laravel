...
class PwMedalUpload extends PwUploadAction {
	...
	public function check() {
		if (!$_FILES[$this->key]['size']) return new ErrorBag('MEDAL:image.empty.fail');
		return true;
	}
	...
	public function getSaveName(PwUploadFile $file) {
		$prename  = substr(md5(Tool::getTime() . Utility::generateRandStr(8)), 10, 15);
	...
	public function getAttachInfo() {
		//$path  = Core::getGlobal('url', 'attach').'/';
