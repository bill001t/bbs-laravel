...
class PwAttUpload extends PwUploadAction {
	...
	public function check() {
		...
		if (!$this->forum->isForum()) {
			return new ErrorBag('FORUM_IS_NOT_EXISTS');
		}
		...
		if ($uploadPerday = $this->user->getPermission('uploads_perday')) {
			...
			$todayupload = $this->user->info['lastpost'] < Tool::getTdtime() ? 0 : $this->user->info['todayupload'];
			if ($count > 0 && ($count + $todayupload) > $uploadPerday) {
				return new ErrorBag('permission.upload.nums.perday', array('{nums}' => $uploadPerday));
			}
		}
		return true;
	}
	...
	public function transfer() {
		...
		foreach ($attach as $rt) {
			...
			if (!isset($this->flashatt[$aid])) {
				Tool::deleteAttach($rt['path'], $rt['ifthumb']);
				$deltmp[] = $aid;
				continue;
			}
	...
	public function getAttachInfo() {
		...
		$path  = Core::getGlobal('url', 'attach') . '/' . $array['path'];
