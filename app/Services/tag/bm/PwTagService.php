...
namespace App\Services\tag\bm;

use App\Core\ErrorBag;
use App\Core\Tool;
use App\Services\forum\bs\PwThread;
use App\Services\forum\dm\PwTopicDm;
use App\Services\tag\bs\PwTag;
use App\Services\tag\bs\PwTagAttention;
use App\Services\tag\bs\PwTagCateGory;
use App\Services\tag\dm\PwTagDm;
...
class PwTagService
{
    ...
    {
        if (!$typeId || !$paramId) {
            return new ErrorBag('data.error');
        }
        ...
        app(PwThread::class)->updateThread($dm, PwThread::FETCH_CONTENT);
        return true;
    }
    ...
    public function getContentsByTypeName($tagId, $typeName, $ifcheck, $offset = 0, $num = 4)
    {
        ...
        if (!$action) return new ErrorBag('undefined content type');
    ...
    {
        return Core::cache()->get('hot_tags', array($categoryId, $num));
    }
    ...
    {
        // 删除过期数据
        $updateTime = Tool::getTime() - 86400 * $this->expireDay;
    ...
    {
        // 是否关注过了
        if ($this->_getTagAttentionDs()->isAttentioned($uid, $tagId)) return new ErrorBag('Tag:have.attentioned');
        if (($count = $this->_getTagAttentionDs()->countAttentionByUid($uid)) > 49) {
            return new ErrorBag('Tag:attentioned.count.error');
        }
    ...
    public function deleteByTagIds($tagIds)
    {
        ...
        foreach ($result as $tag) {
            $tag['tag_logo'] && Tool::deleteAttach($tag['tag_logo']);
        }
    ...
    private function _getTagAction($typeName)
    {
        ...
        include_once(app()->path() . '/Services/tag/bm/action/' . $className);
    ...
    {
        return app(PwTagCateGory::class);
    }
    ...
    {
        return app(PwTagAttention::class);
    }
