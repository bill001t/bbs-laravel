...
use App\Services\tag\ds\dao\PwTagDao;
use App\Services\tag\ds\dao\PwTagRecordDao;
use App\Services\tag\ds\dao\PwTagRelationDao;
...
class PwTag
{
    ...
    {
        if (!$dm instanceof PwTagDm) return new ErrorBag('TAG:data_error');
    ...
    public function updateTag(PwTagDm $dm)
    {
        ...
        return $this->_getTagDao()->_update($dm->tag_id, $dm->getData(), $dm->getIncreaseData());
    }
    ...
    public function addRelation(PwTagDm $dm)
    {
        ...
        $this->_getTagDao()->_update($dm->tag_id, '', array('content_count' => $result));
    }
    ...
    public function deleteRelation($typeId, $paramId = null, $tagId)
    {
        ...
        $result && $this->_getTagDao()->_update($tagId, array(), array('content_count' => -$result));
        return true;
    }
    ...
    {
        if (!is_array($tagIds) || !count($tagIds)) return new ErrorBag('TAG:data_error');
        return $this->_getTagDao()->batchUpdate($tagIds, $dm->getData(), $dm->getIncreaseData());
    }
    ...
    {
        return app(PwTagRecordDao::class);
    }
    ...
    {
        return app(PwTagRelationDao::class);
    }
}