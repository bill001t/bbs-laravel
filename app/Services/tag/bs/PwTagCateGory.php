...
use App\Services\tag\ds\dao\PwTagCategoryDao;
use App\Services\tag\ds\dao\PwTagCategoryRelationDao;
...
class PwTagCateGory
{
    ...
    public function updateTagCategory(PwTagDm $dm)
    {
        if (!$dm->getField('category_name')) {
            return new ErrorBag('TAG:category.name.empty');
        }
        return $this->_getTagCategoryDao()->_update($dm->tag_id, $dm->getData());
    }
    ...
    {
        return app(PwTagCategoryDao::class);
    }
    ...
    {
        return app(PwTagCategoryRelationDao::class);
    }
}