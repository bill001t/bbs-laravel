...
namespace App\Services\tag\dm;

use App\Core\BaseDm;
use App\Core\ErrorBag;
use App\Core\Tool;

class PwTagDm extends BaseDm
{
    ...
    public function checkTagName($tagName = '')
    {
        ...
        if (Tool::strlen($tagName) > $maxLength) {
            return new ErrorBag('TAG:tagname.length.error', array('{maxlength}' => $maxLength));
        }
        if (Tool::strlen($this->_data['excerpt']) > 255) {
            return new ErrorBag('TAG:excerpt.length.error', array('{maxlength}' => 255));
        }
        return true;
    }
    ...
    private function isNameHasIllegalChar($tagName)
    {
        if (0 >= preg_match('/^[\x7f-\xff\dA-Za-z\.\_]+$/', $tagName)) {
            return new ErrorBag('TAG:error.tagname');
        }
        return false;
    }
}
