...
use App\Services\message\bs\PwMessageNotices;
use App\Services\message\dm\PwMessageNoticesDm;
...
class PwNoticeService
{
    ...
    public function sendNotice($uid, $type, $param = 0, $extendParams = array(), $updateUnRead = true)
    {
        ...
        if (!$action) return new ErrorBag('MESSAGE::notice.type.undefined');
    ...
    public function formatNoticeList($noticeList)
    {
        ...
        //取私信相关信息
        $messageInfos = $this->_getMessageApi()->getDialogByUsers($uid, $messageFromUids);
        if ($messageInfos) {
    ...
    protected function _getAction($type)
    {
        ...
        if (!method_exists($this, $actionMethod)) {
            ...
            include app()->path() . 'Services/message/bm/notice/' . $className;
    ...
    {
        return app(PwMessageNotices::class);
    }
