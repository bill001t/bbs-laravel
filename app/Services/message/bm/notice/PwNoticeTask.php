...
class PwNoticeTask extends PwNoticeAction{
	...
	public function formatExtendParams($extendParams, $aggregatedNotice = null) {
		...
		$array = array('id' =>$extendParams['taskid'] ,'title' => $extendParams['title'], 'icon' => $extendParams['icon'], 'url' => $url, 'created_time' => Tool::getTime());
	...
	public function getDetailList($notice) {
		$notice['is_ignore'] = app('message.srv.PwNoticeService')->isIgnoreNoticeType($notice['uid'], $notice['typeid']);
	...
	public function ignoreNotice($notice,$ignore = 1){
		...
		$dm->setIgnore($ignore);
		app('message.PwMessageNotices')->batchUpdateNoticeByUidAndType($notice['uid'], $notice['typeid'], $dm);
		return app('message.srv.PwNoticeService')->setIgnoreNotice($notice['typeid'],$notice['uid'],$ignore);
	}
}