...
namespace App\Services\hook\bs;

use App\Services\hook\ds\dao\PwHookDao;
...
class PwHooks
{
    ...
    public function add($hook)
    {
        ...
        if ($error !== true) return new ErrorBag($error[0], $error[1]);
    ...
    {
        $r = $hook->beforeUpdate();
        if ($r !== true) return new ErrorBag($r[0], $r[1]);
        return $this->_load()->_update($hook->getField('name'), $hook->getData());
    }
