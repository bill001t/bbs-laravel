...
namespace App\Services\credit\bo;

use App\Core\Tool;
use App\Services\credit\bm\PwCreditOperationConfig;
use App\Services\credit\bs\PwCreditLog;
use App\Services\credit\dm\PwCreditDm;
use App\Services\credit\dm\PwCreditLogDm;
...
class PwCreditBo
{
    ...
    {
        $strategy = Core::C('credit', 'strategy');
    ...
    public function operate($operation, PwUserBo $user, $delay = false, $log = array(), $creditset = array())
    {
        ...
        if (!empty($creditset['limit']) || (!empty($creditset['credit']) && false === $this->_checkCreditSetEmpty($creditset['credit']))) {
    ...
    {
        if (!empty($this->_log)) {
            app(PwCreditLog::class)->batchAdd($this->_log);
        }
        if (!empty($this->_num)) {
            $tmp = array();
            $t = Core::V('timestamp');
            foreach ($this->_num as $uid => $ops) {
                foreach ($ops as $op => $n) {
                    $tmp[] = array('uid' => $uid, 'operate' => $op, 'num' => $n, 'update_time' => $t);
                }
            }
            app(PwCreditLog::class)->batchAddOperate($tmp);
        }
        ...
        if (!empty($this->_userLog)) {
            ...
            foreach ($this->_userLog as $_uid => $_log) {
                $_dm = app(PwUserInfoDm::class, [$_uid]);
    ...
    public function getOperateCount($uid, $operate)
    {
        ...
        isset($this->_ops[$uid]) || ($this->_ops[$uid] = app(PwCreditLog::class)->getOperate($uid));
        $ops = $this->_ops[$uid];
        if (isset($ops[$operate]) && $ops[$operate]['update_time'] > Tool::getTdtime()) {
