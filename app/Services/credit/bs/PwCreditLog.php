...
namespace App\Services\credit\bs;

use App\Core\ErrorBag;
use App\Services\credit\dm\PwCreditLogDm;
use App\Services\credit\ds\dao\PwCreditLogDao;
use App\Services\credit\ds\dao\PwCreditOperateLogDao;
use App\Services\credit\vo\PwCreditLogSc;
...
class PwCreditLog
{
    ...
    public function batchAdd($dmArr)
    {
        ...
        foreach ($dmArr as $key => $dm) {
            ...
            if ($dm->beforeAdd() instanceof ErrorBag) continue;
            $data[] = $dm->getData();
        }
    ...
    {
        return app(PwCreditLogDao::class);
    }

    private function _getOperateDao()
    {
        return app(PwCreditOperateLogDao::class);
    }
}