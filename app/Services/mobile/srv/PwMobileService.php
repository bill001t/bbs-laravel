...
namespace App\Services\mobile\bm;
...
class PwMobileService
{
    ...
    {
        if (!$this->plat) {
            return new ErrorBag('USER:mobile.plat.choose.error');
        }
    ...
    {
        if (!$mobile || !$inputCode) return new ErrorBag('USER:mobile.code.mobile.empty');
        $info = $this->_getDs()->getMobileVerify($mobile);
        if (!$info) return new ErrorBag('USER:mobile.code.error');
        if ($info['expired_time'] < Tool::getTime()) return new ErrorBag('USER:mobile.code.expired_time.error');
        if ($inputCode !== $info['code']) return new ErrorBag('USER:mobile.code.error');
        // 手机验证通过后扩展
        SimpleHook::getInstance('PwMobileService_checkVerify')->runDo($mobile);
        return true;
    }
    ...
    public function checkTodayNum($mobile)
    {
        ...
        if ($number > $this->sendNumDay) {
            return new ErrorBag('USER:mobile.code.send.num.error');
        }
        return $number;
    }
    ...
    protected function _buildContent($code)
    {
        ...
        $replace = array($code, Core::C('site', 'info.name'));
        $content = str_replace($search, $replace, Core::C('register', 'mobile.message.content'));
        return $content;
    }
    ...
    {
        return app('user.PwUserMobileVerify');
    }
}