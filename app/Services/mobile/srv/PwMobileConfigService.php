...
class PwMobileConfigService {
	...
	public function __construct($hookKey) {
		$this->sHook = SimpleHook::getInstance($hookKey);
	}
	...
	public function setPlatComponents($platType) {
		$plats = $this->getPlats();
		if (!array_key_exists($platType, $plats)) return new ErrorBag('USER:mobile.plat.type.not.exit');
		$plat = $plats[$platType];
		if (!isset($plat['components']['path'])) return new ErrorBag('USER:mobile.plat.config.fail');
