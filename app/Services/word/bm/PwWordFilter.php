...
namespace App\Services\word\bm;

use App\Core\config\PwConfigSet;
use App\Services\word\bs\PwWord;
use Core;
...
class PwWordFilter
{
    ...
    {
        $this->isTip = intval(Core::C('word', 'istip'));
    ...
    {
        $word_version = Core::C('bbs', 'word_version');
    ...
    {
        return Core::cache()->get(self::WORD_ALL_KEY);
    }
    ...
    {
        return Core::cache()->get(self::WORD_REPLACE_KEY);
    }
    ...
    {
        Core::cache()->set(self::WORD_ALL_KEY, $data);
        return true;
    }

    public function updateReplaceWordCache($data)
    {
        Core::cache()->set(self::WORD_REPLACE_KEY, $data);
        return true;
    }
    ...
    private function _getAlgorithms($data = array())
    {
        ...
        include_once(app()->path() . '/Services/word/bm/filter/' . $className);
