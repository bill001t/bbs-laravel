...
class PwOnlineService
{
    ...
    public function loginOnline($uid, $username, $gid, $ip)
    {
        ...
        list($ip, $createdTime, $modifyTime) = $vistor;
        app(PwGuestOnline::class)->deleteInfo($ip, $createdTime);
    ...
    public function logoutOnline($uid)
    {
        ...
        app(PwUserOnline::class)->deleteInfo($uid);
        $this->signVisitor($ip, $this->time, $this->time);
    }
    ...
    {
        $ip = ip2long($ip);
        $sign = Tool::encrypt($ip . '_' . $createdTime . '_' . $modifyTime . '_' . serialize($extension));
        return Tool::setCookie('visitor', $sign);
    }
    ...
    {
        $sign = Tool::getCookie('visitor');
        if (empty($sign)) return true;
        $sign = Tool::decrypt($sign);
    ...
    public function clearNotOnline()
    {
        ...
        app(PwUserOnline::class)->deleteInfoByTime($expiredTime);
        app(PwGuestOnline::class)->deleteInfoByTime($expiredTime);
    }
