...
class PwNavBo
{
    ...
    protected $default = array('m' => 'bbs', 'c' => 'index', 'a' => 'run');
    ...
    public function isForum()
    {
        ...
        list($m,) = array_pad(explode('.', Route::currentRouteName()), 3, '');
        if ($m == 'bbs') return true;
        return false;
    }
    ...
    {
        list($m, $c, $a) = array_pad(explode('.', Route::currentRouteName()), 3, '');
        empty($m) && ($m = 'bbs');
        empty($c) && ($c = 'index');
        empty($a) && ($a = 'run');
