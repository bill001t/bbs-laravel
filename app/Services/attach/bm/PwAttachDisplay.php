...
namespace App\Services\attach\bm;
...
class PwAttachDisplay
{
    ...
    public function __construct($tid, $pids, PwUserBo $user, $isLazy = false)
    {
        ...
        $this->imgWidth = Core::C('bbs', 'ubb.img.width');
        $this->imgHeight = Core::C('bbs', 'ubb.img.height');
        $this->imgLazy = $isLazy;
        $this->_init();
    }
    ...
    {
        $att['name'] = Security::escapeHTML($att['name']);
        $html = "<span id=\"td_att_{$att['aid']}\" class=\"J_attach_img_wrap single_img\">";
        $html .= '<div class="img_info J_img_info">';
        if ($att['descrip']) {
            $html .= '<p>描述：' . Tool::stripWindCode($att['descrip'], true) . '</p>';
        }
        $html .= '<p>图片：' . $att['name'];
        if ($this->isAdmin) {
            $html .= '<a class="J_read_img_del w" data-pdata="{\'aid\':\'' . $att['aid'] . '\'}" href="' . 'bbs/attach/delete' . '">[删除]</a>';
        }
        $html .= '</p></div>';
        $html .= $att['img'];
        $html .= '</span>';
    ...
    {
        $lang = array(0 => '', 1 => '');
        $att['descrip'] = Security::escapeHTML($att['descrip']);
        $att['name'] = Security::escapeHTML($att['name']);
    ...
    public function analyse($attach)
    {
        if ($attach['type'] == 'img' && $attach['cost'] == 0) {
            $atype = 'pic';
            $url = Tool::getPath($attach['path']);
            $img = PwUbbCode::createImg(Tool::getPath($attach['path'], $attach['ifthumb'] & 1), $this->imgWidth, $this->imgHeight, $url, $this->imgLazy);
            ...
            $attach['descrip'] && $attr .= ' alt="' . Security::escapeHTML($attach['descrip']) . '"';
            $attr && $img = substr($img, 0, -3) . $attr . ' />';

            $attach += array(
                'url' => $url,
                'img' => $img,
                'miniUrl' => Tool::getPath($attach['path'], $attach['ifthumb'])
            );
        } else {
            $atype = 'downattach';
            $attach += array(
    ...
    {
        return app(PwThreadAttach::class);
    }
}
