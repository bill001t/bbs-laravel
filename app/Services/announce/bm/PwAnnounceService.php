...
namespace App\Services\announce\bm;

use App\Core\Tool;
use App\Services\user\bs\PwUser;
use App\Services\announce\bs\PwAnnounce;
...
class PwAnnounceService
{
    ...
    {
        $announces = array();
        $announceInfos = $this->_getPwAnnounceDs()->getAnnounceByTimeOrderByVieworder(Tool::str2time(Tool::time2str(Tool::getTime(), 'Y-m-d')), 9, 0);

        foreach ($announceInfos as $value) {
            $announces[$value['aid']] = $value;
            $announces[$value['aid']]['start_date'] = Tool::time2str($value['start_date'], 'Y-m-d');
            if (Tool::strlen($value['subject']) > 18) {
                $announces[$value['aid']]['subject'] = Tool::substrs($value['subject'], 18);
            }
        }
