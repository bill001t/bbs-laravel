...
use App\Core\Tool;
use App\Services\attention\ds\dao\PwAttentionRelationDao;
use App\Services\attention\ds\dao\PwAttentionTypeDao;
...
class PwAttentionType
{
    ...
    {
        if (empty($name)) return new ErrorBag('USER:attention.type.name.empty');
        if (Tool::strlen($name) > 10) return new ErrorBag('USER:attention.type.name.length.over');
    ...
    {
        return app(PwAttentionTypeDao::class);
    }
    ...
    {
        return app(PwAttentionRelationDao::class);
    }
}