...
use App\Services\attention\bs\PwAttentionType;
...
class PwAttentionService
{
    ...
    {
        if (($result = $this->_getAttention()->addFollow($uid, $touid)) instanceof ErrorBag) {
            return $result;
        }
        ...
        SimpleHook::getInstance('addFollow')->runDo($uid, $touid);
    ...
    {
        if (($result = $this->_getAttention()->deleteFollow($uid, $touid)) instanceof ErrorBag) {
            return $result;
        }
        ...
        SimpleHook::getInstance('deleteFollow')->runDo($uid, $touid);
        return true;
    }
    ...
    public function addType($uid, $name)
    {
        ...
        if (count($types) > 20) {
            return new ErrorBag('USER:attention.type.count.error');
        }

        if (in_array($name, $types)) {
            return new ErrorBag('USER:attention.type.repeat');
        }
    ...
    {
        return app(PwAttentionType::class);
    }
    ...
    {
        return app(PwAttentionRecommendFriendsService::class);
    }
