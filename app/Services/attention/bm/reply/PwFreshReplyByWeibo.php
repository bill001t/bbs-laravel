...
class PwFreshReplyByWeibo {
	...
	public function execute() {
		...
		$this->dm->setCreatedTime(Tool::getTime());
		if (($result = $this->_getHook()->runWithVerified('check', $this->dm)) instanceof ErrorBag) {
			return $result;
		}
		$result = app('weibo.srv.PwWeiboService')->addComment($this->dm, $this->user);
		...
		if (($result = $this->_getHook()->runDo('addWeibo', $this->dm)) instanceof ErrorBag) {
			return $result;
		}
		return true;
	}
