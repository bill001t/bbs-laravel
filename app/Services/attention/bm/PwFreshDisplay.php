...
use App\Core\iPwDataSource;
use App\Core\iPwDataSource2;
use App\Core\Security;
use App\Core\Tool;
use App\Services\attach\bs\PwThreadAttach;
...
abstract class PwBaseFresh
{
    ...
    protected function _bulidContent($array, &$errcode)
    {
        ...
        } */ elseif (Tool::strlen($array['content']) > 140) {
            $errcode['is_read_all'] = true;
            $array['content'] = Tool::substrs($array['content'], 140);
        }
    ...
    {
        return '版块&nbsp;-&nbsp;<a href="' . url('bbs/thread/run', array('fid' => $fid)) . '">' . $fname . '</a>';
    }
}
...
class PwTopicFresh extends PwBaseFresh
{
    ...
    {
        $this->_topic = app(PwFreshContentFromTopic::class);
        $this->_forum = app(PwFreshContentFromForum::class);
        $this->_poll = app(PwFreshContentFromPoll::class);
        $this->_att = app(PwFreshContentFromAtt::class);
        $this->_topic->set($ids);
    }
    ...
    public function offer($id)
    {
        ...
        !$topic['word_version'] && $topic['content'] = app(PwWordFilter::class)->replaceWord($topic['content'], $topic['word_version']);
...
class PwReplyFresh extends PwBaseFresh
{
    ...
    public function __construct($ids)
    {
        $this->_reply = app(PwFreshContentFromReply::class);
        $this->_topic = app(PwFreshContentFromTopic::class);
        $this->_forum = app(PwFreshContentFromForum::class);
        $this->_att = app(PwFreshContentFromAtt::class);
        $this->_reply->set($ids);
    ...
    public function offer($id)
    {
        ...
        !$reply['word_version'] && $reply['content'] = app(PwWordFilter::class)->replaceWord($reply['content'], $reply['word_version']);
        ...
        $result = array(
            ...
            'quote' => array(
                ...
                'url' => url('bbs/read/run', array('tid' => $quote['tid'])),
                'from' => $from
            )
        );
...
class PwWeiboFresh extends PwBaseFresh
{
    ...
    public function __construct($ids)
    {
        $this->_weibo = app(PwFreshContentFromWeibo::class);
        $this->_weibo->set($ids);
    }
    ...
    public function offer($id)
    {
        ...
        if ($weibo['src_id']) {
            ...
            $result['quote'] = array(
                ...
                'created_time' => $quote['created_time'],
                'content' => $quote['content'],
                'url' => url('space/index/fresh', array('uid' => $quote['created_userid'], 'weiboid' => $quote['weibo_id'])),
    ...
    protected function _bulidFrom($fid, $name)
    {
        ...
        if ($from[1]) return '<a href="' . url($from[1]) . '">' . $from[0] . '</a>';
        return $from[0];
    }
}
...
class PwFreshContentFromTopic implements iPwDataSource2
{
    ...
    {
        return app(PwThread::class)->fetchThread($ids, PwThread::FETCH_ALL);
    }
}
...
class PwFreshContentFromReply implements iPwDataSource2
{
    ...
    public function getData($ids)
    {
        return app(PwThread::class)->fetchPost($ids);
    }
}
...
class PwFreshContentFromForum implements iPwDataSource2
{
    ...
    public function getData($ids)
    {
        return app(PwForum::class)->fetchForum($ids);
    }
}
...
class PwFreshContentFromWeibo implements iPwDataSource2
{
    ...
    public function getData($ids)
    {
        return app(PwWeibo::class)->getWeibos($ids);
    }
}
...
class PwFreshContentFromAtt implements iPwDataSource2
{
    ...
    public function getData($ids)
    {
        ...
        $result = array();
        $array = app(PwThreadAttach::class)->fetchAttachByTidAndPid($tids, $pids);
