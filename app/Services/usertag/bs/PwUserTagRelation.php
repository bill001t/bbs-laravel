...
use App\Services\usertag\ds\dao\PwUserTagRelationDao;
...
class PwUserTagRelation
{
    ...
    {
        if ($uid < 1 || $tag_id < 1) return new ErrorBag('USER:tag.illega.format');
    ...
    {
        if (($uid = intval($uid)) < 1) return new ErrorBag('USER:tag.uid.require');
    ...
    public function batchDeleteRelation($uid, $tag_ids)
    {
        ...
        SimpleHook::getInstance('PwUserTagRelation_batchDeleteRelation')->runDo($tag_ids, $this);
    ...
    {
        if (($uid = intval($uid)) < 1) return new ErrorBag('USER:tag.uid.require');
        SimpleHook::getInstance('PwUserTagRelation_deleteRelationByUid')->runDo($uid, $this);
    ...
    public function batchDeleteRelationByUids($uids)
    {
        ...
        SimpleHook::getInstance('PwUserTagRelation_batchDeleteRelationByUids')->runDo($uids, $this);
    ...
    {
        return app(PwUserTagRelationDao::class);
    }
}