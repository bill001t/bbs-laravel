...
namespace App\Services\log\bm;

use App\Core\MessageTool;
use App\Core\Security;
use App\Core\Tool;
use App\Services\forum\dm\PwTopicDm;
use App\Services\forum\ds\PwThread;
use App\Services\log\dm\PwLogDm;
use App\Services\log\so\PwLogSo;
...
class PwLogService
{
    ...
    public function addEditThreadLog(PwUserBo $user, $thread, $isReply = false)
    {
        ...
        $langArgs = array();
        $langArgs['{createdUser}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('space/index/run', ['uid' => $user->uid]), $user->username);

        $msg = '';
        if ($isReply) {
            $langArgs['{title}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('bbs/read/run', ['tid' => $thread['tid']], $thread['pid']), $this->_buildSecurity($thread['subject']));
            $msg = 'LOG:editThread.reply.message';
        } else {
            $langArgs['{title}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('bbs/read/run', ['tid' => $thread['tid']]), $this->_buildSecurity($thread['subject']));
    ...
    public function addDeleteAtachLog(PwUserBo $user, $attach)
    {
        ...
        $langArgs = array();
        $langArgs['{createdUser}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('space/index/run', ['uid' => $user->uid]), $user->username);
        $langArgs['{attach}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('bbs/read/run', ['tid' => $attach['tid']], $attach['pid'] ? $attach['pid'] : ''), $this->_buildSecurity($attach['name']));

        $_createdUser = app('user.PwUser')->getUserByUid($attach['created_userid']);
    ...
    public function addThreadManageLog(PwUserBo $user, $type, $threads, $reason, $extends = '', $useReplyMsg = false)
    {
        ...
        foreach ($threads as $thread) {
            ...
            $_dm = new PwLogDm();
            $_dm->setCreatedTime(Tool::getTime())
                ->setCreatedUser($user->uid, $user->username)
                ->setOperatedUser($thread['created_userid'], $thread['created_username'])
                ->setIp(Request::ip())
                ->setExtends($extends)
            ...
            if (!isset($thread['pid']) && !Tool::getstatus($thread['tpcstatus'], PwThread::STATUS_OPERATORLOG)) {
                $topicDm = new PwTopicDm($thread['tid']);
                $topicDm->setOperatorLog(true);
                app(PwThread::class)->updateThread($topicDm, PwThread::FETCH_MAIN);
            }
    ...
    public function addBanUserLog(PwUserBo $user, $uids, $types, $reason, $endTime = '')
    {
        ...
        $userList = app(PwUser::class)->fetchUserByUid($uids);
        ...
        foreach ($types as $_type) {
            ...
            $langArgs = array(
                '{operatedUser}' => '',
                '{createdUser}' => sprintf('<a href="%s" target="_blank">%s</a>', url('space/index/run', array('uid' => $user->uid)), $user->username),
                '{operattype}' => $title,
                '{reason}' => $reason ? $this->_buildSecurity($reason) : '无');

            foreach ($userList as $_uid => $_user) {
                $langArgs['{operatedUser}'] = sprintf('<a href="%s" target="_blank">%s</a>', url('space/index/run', ['uid' => $_uid]), $_user['username']);
    ...
    public function addDeleteFreshLog(PwUserBo $user, $data)
    {
        ...
        foreach ($data as $_item) {
            $title = Tool::substrs(strip_tags(Tool::stripWindCode($_item['content'])), 20, 0, true);
    ...
    {
        return (new MessageTool($msg, $args))->get($msg);
    }
