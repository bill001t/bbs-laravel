...
namespace App\Services\backup\bm;

use App\Services\backup\bs\PwBackup;
use App\Core\Tool;
...
class PwBackupService
{
    ...
    public function getDirectoryName()
    {
        ...
        //return 'pw_' . $version . '_' . Tool::time2str(Tool::getTime(), 'YmdHis') . '_' . Utility::generateRandStr(5);
        return Utility::generateRandStr(8) . '_pw_' . $version . '_' . Tool::time2str(Tool::getTime(), 'YmdHis');
    }
    ...
    {
        if ($extend == 'zip') {
            $zipService = app('LIB:utility.PwZip');
    ...
    private function _doBackIn($sql)
    {
        ...
        foreach ($sql as $value) {
            $value = trim($value);
            if (!$value || Tool::substrs($value, 2, '', false) === '--') continue;
