 ...
 class PwSpaceBo {
	...
	public function allowView($key = 'space') {
 	...
 	private function _getSpaceUser() {
		$this->spaceUser = app('user.PwUser')->getUserByUid($this->spaceUid, PwUser::FETCH_ALL);
	...
	private function _getSpace() {
 	...
 	private function _getDomain() {
 		if ($this->space['space_domain']) {
 			$root = Core::C('domain', 'space.root');
 			if ($root) return 'http://' . $this->space['space_domain'] . '.' . $root;
 		}
 		return url('space/index/run', array('uid' => $this->spaceUid));
 	}
	 ...
	 */
 	private function _getTome($spaceUid, $visitUid) {
 		...
 		if (app('attention.PwAttention')->isFollowed($spaceUid, $visitUid)) {
 			$attention = true; //self::ATTENTION; 
 		}
 		if (app('attention.PwAttention')->isFollowed($visitUid, $spaceUid)) {
