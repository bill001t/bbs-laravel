...
class PwSpaceModel extends PwSpaceBo {
	...
	protected function modelTag($limit) {
		$count = app('tag.PwTagAttention')->countAttentionByUid($this->spaceUid);
 		return array($count, app('tag.PwTag')->getAttentionByUid($this->spaceUid, 0, $limit));
	}
	
	protected function modelUserTag($limit) {
		$tags = app('usertag.srv.PwUserTagService')->getUserTagList($this->spaceUid);
		$count = count($tags);
 	...
 	protected function modelFollow($limit) {
 		...
 		$follows = app('attention.PwAttention')->getFollows($this->spaceUid, $limit, 0);
 		$uids = array_keys($follows);
 		return array($count, app('user.PwUser')->fetchUserByUid($uids));
	}
	
 	protected function modelFans($limit) {
 		$count = $this->spaceUser['fans'];
 		$fans = app('attention.PwAttention')->getFans($this->spaceUid, $limit, 0);
 		$uids = array_keys($fans);
 		return array($count, app('user.PwUser')->fetchUserByUid($uids));
	}
	
 	protected function modelThreadList($limit, $page) {
 		$ds = app('forum.PwThread');
 		$count =$ds->countThreadByUid($this->spaceUid);
		if ($count) {
			list($offset,$limit) = Tool::page2limit($page, $limit);
 	...
 	protected function modelFreshList($limit, $page) {
		...
		$count = app('attention.PwFresh')->countFreshByUid($this->spaceUid);
