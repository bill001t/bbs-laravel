...
class PwForumBo
{
    ...
    public function __construct($fid, $fetchAll = false)
    {
        ...
        !empty($this->foruminfo->forumStatistics->settings_basic) && $this->forumset = unserialize($this->foruminfo['settings_basic']);
        if (!empty($this->forumset) && !is_array($this->forumset['allowtype'])) $this->forumset['allowtype'] = array();
    }
    ...
    {
        return app(PwForumUser::class)->get($uid, $this->fid);
    }
    ...
    public function isBM($username)
    {
        ...
        if (isset($this->foruminfo['manager']) && strpos($this->foruminfo['manager'], ",$username,") !== false) return true;
        if (isset($this->foruminfo['uppermanager']) && strpos($this->foruminfo['uppermanager'], ",$username,") !== false) return true;
        return false;
    }
    ...
    public function getForumChain()
    {
        ...
        if ($this->foruminfo['type'] == 'category') {
            $guide[] = array(strip_tags($this->foruminfo['name']), url('bbs/cate/run', array('fid' => $this->fid)));
            return $guide;
        }
        $guide[] = array(strip_tags($this->foruminfo['name']), url('bbs/thread/run', array('fid' => $this->fid)));
        $info = $this->getParentForums();
        $count = count($info);
        $i = 0;
        foreach ($info as $fid => $value) {
            array_unshift($guide, array($value, url('bbs/' . (++$i < $count ? 'thread' : 'cate') . '/run', array('fid' => $fid))));
        }
        return $guide;
    }
    ...
    {
        if ($guide[1]) {
            return '<em>&gt;</em><a href="' . $guide[1] . '">' . Security::escapeHTML($guide[0]) . '</a>';
        }
        return '<em>&gt;</em>' . Security::escapeHTML($guide[0]);
    }
    ...
    {
        if (!isset($this->forumset['typeorder']) || !is_array($this->forumset['typeorder'])) return array();
        asort($this->forumset['typeorder']);
        $array = array();
        $tType = app(PwThreadType::class)->getTtype();
    ...
    {
        return app(PwForumService::class)->updateStatistics($this, 1, 0, 1, array(
    ...
    {
        return app(PwForumService::class)->updateStatistics($this, 0, 1, 1, array(
            'tid' => $tid, 'username' => $username, 'subject' => $subject
        ));
    }
