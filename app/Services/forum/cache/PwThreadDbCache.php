...
namespace App\Services\forum\cache;
...
class PwThreadDbCache extends PwBaseMapDbCache
{
    ...
    {
        $data = Core::cache()->fetch($this->getKeysByTid($tid));
        $result = array();
    ...
    {
        $result = array();
        $data = Core::cache()->fetch($this->fetchKeysByTid($tids));
        foreach ($data as $key => $value) {
    ...
    {
        $fver = Core::cache()->get('thread_fver', array($fid));
        $data = Core::cache()->get('thread_list', array($fver, $fid, $limit, $offset));
        if ($data === false) {
            $result = $this->_getDao()->getThreadByFid($fid, $limit, $offset);
            Core::cache()->set('thread_list', array_keys($result), array($fver, $fid, $limit, $offset));
        } else {
    ...
    public function updateThread($tid, $fields, $increaseFields = array(), $bitFields = array())
    {
        ...
        Core::cache()->batchDelete($this->getKeysByTid($tid));
    ...
    public function batchUpdateThread($tids, $fields, $increaseFields = array(), $bitFields = array())
    {
        ...
        Core::cache()->batchDelete($this->fetchKeysByTid($tids));
    ...
    {
        Core::cache()->increment('thread_fver', array($fid));
    }
