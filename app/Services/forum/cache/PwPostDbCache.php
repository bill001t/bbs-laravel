...
class PwPostDbCache extends PwBaseDbCache {
	...
	public function getPost($pid) {
		return Core::cache()->get('post', array($pid));
	}

	public function fetchPost($pids) {
		$result = array();
		$data = Core::cache()->fetch($this->fetchKeysByPid($pids));
	...
	public function getPostByTid($tid, $limit, $offset, $asc) {
		...
		$tver = Core::cache()->get('post_tver', array($tid));
		$data = Core::cache()->get('post_list', array($tver, $tid, $limit, $offset, $orderby));
		if ($data === false) {
			$result = $this->_getDao()->getPostByTid($tid, $limit, $offset, $asc);
			Core::cache()->set('post_list', array_keys($result), array($tver, $tid, $limit, $offset, $orderby));
		} else {
	...
	public function updatePost($pid, $fields, $increaseFields = array()) {
		...
		Core::cache()->delete('post', array($pid));
	...
	public function batchUpdatePost($pids, $fields, $increaseFields = array()) {
		...
		Core::cache()->batchDelete($this->fetchKeysByPid($pids));
	...
	public function clearPostListCache($tid) {
		Core::cache()->increment('post_tver', array($tid));
	}
