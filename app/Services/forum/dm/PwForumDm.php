 ...
 * @package forum
 */
class PwForumDm extends BaseDm
{
    ...
    protected function _beforeAdd()
    {
        if (empty($this->_data['name'])) {
            return new ErrorBag('BBS:forum.forumname.empty');
        }
    ...
    {
        if (empty($this->fid)) {
            return new ErrorBag('BBS:forum.fid.empty');
        }
    ...
    protected function _checkParentid()
    {
        ...
        if ($this->fid && $parentid == $this->fid) {
            return new ErrorBag('BBS:forum.parentid.same');
        }
        if ($parentid) {
            $forum = $this->_getForumService()->getForum($parentid);
            if (!$forum) {
                return new ErrorBag('BBS:forum.parentid.exists.not');
            }
            if ($forum['type'] == 'sub2') {
                return new ErrorBag('BBS:forum.parentid.issub2');
            }
    ...
    protected function _checkManager()
    {
        ...
        $users = app('user.PwUser')->fetchUserByName($this->_data['manager']);
        if (count($this->_data['manager']) != count($users)) {
            ...
            if ($diff = array_diff($this->_data['manager'], $array)) {
                return new ErrorBag('USER:exists.not', array('{username}' => implode('、', $diff)));
            }
        }
        return true;
    }
