...
use App\Libraries\utility\PwIptable;
use Core;
 ...
 * @package forum
 */
abstract class PwPostDm extends BaseDm
{
    ...
    public function setAuthor($uid, $username, $ip)
    {
        ...
        $this->_data['ipfrom'] = ''/*app(PwIptable::class)->getIpFrom($ip)*/;
        return $this;
    }
    ...
    {
        return !empty($this->_data['subject']) ? $this->_data['subject'] : '';
    }

    public function getContent()
    {
        return !empty($this->_data['content']) ? $this->_data['content'] : '';
    }

    public function getTopictype()
    {
        return !empty($this->_data['topic_type']) ? $this->_data['topic_type'] : '';
    }

    public function getIscheck()
    {
        return !empty($this->_data['ischeck']) ? $this->_data['ischeck'] : '';
    }
    ...
    public function checkData()
    {
        if (empty($this->_data) && empty($this->_increaseData) && empty($this->_bitData)) {
            return new ErrorBag('BBS:post.postdata.empty');
        }
        return true;
    }
    ...
    {
        $maxlen = Core::C('bbs', 'title.length.max');
        if ($maxlen > 0 && Tool::strlen($this->_data['subject']) > $maxlen) {
            return new ErrorBag('BBS:post.subject.length.limit', array('{len}' => $maxlen));
        }
        return true;
    }
    ...
    public function checkContent()
    {
        if ($this->_data['content'] === '') {
            return new ErrorBag('BBS:post.content.empty');
        }
        $len = Tool::strlen($this->_data['content']);
        $config = Core::C('bbs');
        if ($this->forum && isset($this->forum->forumset['minlengthofcontent'])) {
            $config['content.length.min'] = $this->forum->forumset['minlengthofcontent'];
        }
        if ($len < $config['content.length.min']) {
            return new ErrorBag('BBS:post.content.length.less', array('{min}' => $config['content.length.min']));
        }
        if ($len > $config['content.length.max']) {
            return new ErrorBag('BBS:post.content.length.more', array('{max}' => $config['content.length.max']));
        }
        if ($this->forum && $this->user) {
            if ((!empty($this->forum->forumset['allowhide']) || !empty($this->user->getPermission('allow_thread_extend.hide'))) && (PwUbbCode::hasTag($this->_data['content'], 'post') || PwUbbCode::hasTag($this->_data['content'], 'hide'))) {
                return new ErrorBag('BBS:post.content.hide');
            }
            if ((!empty($this->forum->forumset['allowsell']) || !empty($this->user->getPermission('allow_thread_extend.sell'))) && PwUbbCode::hasTag($this->_data['content'], 'sell')) {
                return new ErrorBag('BBS:post.content.sell');
            }
        }
        return true;
    }
...
//            $this->_data['content'] = PwUbbCode::autoUrl($this->_data['content'], true);
            ...
            if (isset($this->_data['reminds'])) $useubb = 1;
            if (!$useubb) {
