...
class PwForumService
{
    ...
    {
        isset(self::$_forums) || self::$_forums = $this->_getForum()->getForumList()->keyBy('fid');
        return self::$_forums;
    }
     ...
     * @return array
     */
    public function getAllowVisitForum(/*PwUserBo*/
        $user, $forums = null)
    {
     ...
     * @return array
     */
    public function getForbidVisitForum(/*PwUserBo*/
        $user, $forums = null, $includeHide = false)
    {
        $forums === null && $forums = $this->getForumList();
        $fids = array();
        /*foreach ($forums as $key => $value) {
    ...
    {
        return Tool::subArray($this->getForumList(), ['fid' => $fids]);
    }

    /**
     * 获取版块层级列表，返回的二维数组，key为id（其实取的parent_id，值为所有的子元素）
     *
     * @return array
     */
    ...
    /**
     * 获取分类和版块列表（子版除外），二维数组，key为id，子id
     *
     * @return array
     */
    public function getCommonForumList()
    {
        ...
        $forumList = $this->_getForum()->getCommonForumList();
    ...
    {
        if ($result = app(PwForumUser::class)->getFroumByUid($uid)) {
            $array = array();
            $tmp = app(PwForum::class)->fetchForum($result->pluck('fid'));
    ...
    public function updateStatistics($forum, $topic, $replies, $tpost = 0, $lastinfo = array())
    {
        ...
        if ($lastinfo) {
            !isset($lastinfo['time']) && $lastinfo['time'] = Tool::getTime();
            $dm['lastpost_tid'] = $lastinfo['tid'];
            $dm['lastpost_info'] = Tool::substrs($lastinfo['subject'], 26, 0, true);
            $dm['lastpost_username'] = $lastinfo['username'];
            $dm['lastpost_time'] = $lastinfo['time'];
        }
        $service = $this->_getForum();
        $service->updateForum($dm);
        ...
        if ($fids = $forum->getParentFids()) {
            ...
            $service->batchUpdateForum($fids, $dm);
        }
        return true;
    }
