...
class PwReplyDoNotice extends PwPostDoBase {
	...
	public function dataProcessing($postDm) {
		...
		$this->_content = Tool::substrs(Tool::stripWindCode($atc_content,true),30);
	...
	private function _sendThreadNotice($tid, $pid) {
		$info = app('thread.PwThread')->getThread($tid,PwThread::FETCH_MAIN);
		$param = $info['tid'];
		$params = array();
		$params['replyUserid'] = $this->user->uid;
		$params['replyUsername'] = $this->user->username;
		$params['threadTitle'] = Tool::substrs(Tool::stripWindCode($info['subject']), 30);
		...
		$blackUid = app('user.PwUserBlack')->checkUserBlack($this->user->uid, $info['created_userid']);
	...
	private function _sendReplyNotice($pid, $tid) {
		$info = app('thread.PwThread')->getPost($this->_rpid);
		$param = $info['pid'];
		$params = array();
		$params['replyUserid'] = $this->user->uid;
		$params['replyUsername'] = $this->user->username;
		$info['content'] = preg_replace('/\[quote(=.+?\,\d+)?\].*?\[\/quote\]/is', '', $info['content']);
		$params['postTitle'] = Tool::stripWindCode($info['content']);
		$params['postTitle'] = $params['postTitle'] ? $params['postTitle'] : 'Re:' . $this->info['subject'];
		$params['postTitle'] = Tool::substrs($params['postTitle'], 30);
		$params['threadId'] = $info['tid'];
		$params['pid'] = $pid;
