...
class PwPostDoWord extends PwPostDoBase {
	...
	public function check($postDm) {
		...
		foreach ($words as $k => $v) {
			if ($k < $banedStrLen) {
				return new ErrorBag('WORD:title.tag.error',array('{wordstr}' => implode(',', $words)));
			}
		}
		$errorTip = $isTip ? 'WORD:content.error.tip' : 'WORD:content.error';
		switch ($type) {
			case 1:
				return new ErrorBag($errorTip,array('{wordstr}' => implode(',', $words)));
			case 2:
				$this->_isVerified = 1;
				if ($this->_confirm) {
						return true;
				}
				return new ErrorBag($errorTip,array('{wordstr}' => implode(',', $words)),array('isVerified' => $this->_isVerified));
			case 3:	
				$this->_word = 1;
			default:
				return true;
	...
	public function dataProcessing($postDm) {
		$word_version = $this->_word ? 0 : (int)Core::C('bbs', 'word_version');
