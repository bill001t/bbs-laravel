...
class PwPostDoPoll extends PwPostDoBase {
	...
	public function check($postDm) {
		if ($this->action == 'add' && !$this->user->getPermission('allow_add_vote')) {
			return new ErrorBag('VOTE:group.permission.add', array('{grouptitle}' => $this->user->getGroupInfo('name')));
		}
	...
	public function addPoll($tid) {
		if (($attachInfo = $this->uploadOptionImage()) instanceof ErrorBag) return $attachInfo;
	...
	private function _updatePollOption($attachInfo) {
		...
		foreach (array_keys($optionInfo) as $_id) {
			...
			$this->_getPollOptionDS()->_update($dm);
		}
	...
	public function uploadOptionImage() {
		...
		if ($result !== true) {
			return $result == false ? new ErrorBag('operate.fail') : $result;
		}
	...
	private function _checkInModify() {
		$this->info	= $this->getThreadPollBo()->info;
		if (!$this->info) return new ErrorBag('VOTE:thread.not.exist');
		if ($this->info['poll']['voter_num']) return true;
		...
		if ($optionNum < 2) return new ErrorBag('VOTE:options.illegal');
		if ($optionNum != count(array_unique($reulst))) {
			return new ErrorBag('VOTE:options.repeat');
		}
		return true;
	}
