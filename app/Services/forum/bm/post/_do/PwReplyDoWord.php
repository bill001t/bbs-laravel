...
class PwReplyDoWord extends PwPostDoBase {
	...
	public function check($postDm) {
		$data = $postDm->getData();
		$content = Tool::substrs(Tool::stripWindCode($data['content']), 30) == $data['subject'] ? $data['content'] : $data['subject'].$data['content'];
		...
		foreach ($words as $k => $v) {
			if ($k < $banedStrLen) {
				return new ErrorBag('WORD:content.error.tip',array('{wordstr}' => implode(',', $words)));
			}
		}
		switch ($type) {
			case 1:
