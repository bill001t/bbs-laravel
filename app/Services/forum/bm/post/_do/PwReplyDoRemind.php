...
class PwReplyDoRemind extends PwPostDoBase {
	...
	public function dataProcessing($postDm) {
		...
		$this->_atc_title = Tool::substrs(trim(Tool::stripWindCode($atc_content,true)),20);
	...
	private function _check() {
		if ($this->loginUser->getPermission('remind_open') < 1) {
			return new ErrorBag("bbs:remind.remind_open.error");
		}
		return true;
	}
	...
	private function _checkBlack($remindUids) {
		$result = app('user.PwUserBlack')->checkUserBlack($this->loginUser->uid, $remindUids);
		if ($result) {
	...
	private function _getRemindService(){
		return app('remind.srv.PwRemindService');
	}
}