...
class PwTopicModify extends PwPostAction {
	...
	public function dataProcessing(PwPostDm $postDm) {
		...
		if (!Tool::getstatus($this->info['tpcstatus'], PwThread::STATUS_OPERATORLOG) && $this->info['created_userid'] != $this->user->uid) {
	...
	public function afterPost() {
		if ($this->postDm->getIscheck() != $this->info['ischeck']) {
			...
			app('forum.srv.PwForumService')->updateStatistics($this->forum, $topic, 0, $topic);
		}
		//编辑非自己的帖子添加管理日志
		if ($this->info['created_userid'] != $this->user->uid) {
			$thread = $this->info;
			$thread['subject'] = $this->postDm->getField('subject') ? $this->postDm->getField('subject') : $this->info['subject'];
			app('log.srv.PwLogService')->addEditThreadLog($this->user, $thread);
		}
	}
