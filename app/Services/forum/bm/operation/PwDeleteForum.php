...
class PwDeleteForum extends PwDoProcess {
	...
	protected function run() {
		if (!$this->forum->isForum(true)) {
			return new ErrorBag('BBS:forum.operate.error.exists.not');
		}
		if ($this->forum->getSubForums()) {
			return new ErrorBag('BBS:forum.delete.error.hassub');
		}
		app('forum.PwForum')->deleteForum($this->fid);
		app('forum.srv.PwForumService')->updateForumStatistics($this->forum->foruminfo['parentid']);
		return true;
	}
}