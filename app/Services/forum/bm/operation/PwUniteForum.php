...
class PwUniteForum extends PwDoProcess {
	...
	protected function run() {
		...
		if (!$forum->isForum(true)) {
			return new ErrorBag('BBS:forum.unite.error.fid.exists.not');
		}
		if ($forum->foruminfo['type'] == 'category') {
			return new ErrorBag('BBS:forum.unite.error.fid.category');
		}
		if ($forum->getSubForums()) {
			return new ErrorBag('BBS:forum.unite.error.hassub');
		}
		
		$toforum = new PwForumBo($this->tofid);
		if (!$toforum->isForum(true)) {
			return new ErrorBag('BBS:forum.unite.error.tofid.exists.not');
		}
		if ($toforum->foruminfo['type'] == 'category') {
			return new ErrorBag('BBS:forum.unite.error.tofid.category');
		}
		if ($this->fid == $this->tofid) {
			return new ErrorBag('BBS:forum.unite.error.same');
		}

		app('forum.PwForum')->deleteForum($this->fid);
		...
		app('forum.PwForum')->updateForum($dm, PwForum::FETCH_STATISTICS);

		app('forum.srv.PwForumService')->updateForumStatistics($forum->foruminfo['parentid']);
		app('forum.srv.PwForumService')->updateForumStatistics($this->tofid);
		return true;
	}
}