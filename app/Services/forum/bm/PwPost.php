...
use App\Services\user\bm\PwBanBp;
use App\Services\user\bs\PwUser;
use App\Services\forum\bm\post\PwPostAction;
 ...
 * -> 3.execute 发布
 */
class PwPost extends BaseHookService
{
    public $action;
    public $forum;
    public $user;
    ...
    public function checkForum()
    {
        if (!$this->forum->isForum()) {
            return new ErrorBag('BBS:post.forum.not.exists');
        }
        if (($result = $this->forum->allowVisit($this->user)) !== true) {
            return new ErrorBag('BBS:forum.permissions.visit.allow',
    ...
    {
        if (isset($this->user->groupid) && $this->user->groupid == 7) {
            return new ErrorBag('REG_CHECK');
        }
        /*
        $config = Core::C('bbs');
        if ($config['post.timing.open'] && !$this->user->inGroup($config['post.timing.groups']) && !self::inTime($config['post.timing.start_hour'], $config['post.timing.start_min'], $config['post.timing.end_hour'], $config['post.timing.end_min'])) {
            return new ErrorBag('BBS:post.timing');
        }
        */
        return true;
    }
    ...
    {
        if (($result = $this->action->beforeRun($postDm)) instanceof ErrorBag) {
            return $result;
        }
    ...
    public function updateUser()
    {
        ...
        if ($userDm = $this->action->getUserDm(false)) {
            app(PwUser::class)->editUser($userDm, PwUser::FETCH_DATA);
        }
    }
    ...
    {
        list($currentHour, $currentMin) = explode(':', Tool::time2str(Tool::getTime(), 'H:i'));
