...
class PwThreadManageDoMove extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (!$this->srv->user->comparePermission(Tool::collectByKey($this->srv->data, 'created_userid'))) {
			return new ErrorBag('permission.level.move', array('{grouptitle}' => $this->srv->user->getGroupInfo('name')));
		}
		if (isset($this->fid)) {
			Wind::import('SRV:forum.bo.PwForumBo');
			$this->forum = new PwForumBo($this->fid);
			if (!$this->forum->isForum()) {
				return new ErrorBag('BBS:manage.error.move.targetforum');
			}
	 ...
	 */
	public function run() {
		$threads = app('forum.PwThread')->fetchThread($this->tids);
		...
		if ($fids) {
			foreach ($fids as $fid => $value) {
				app('forum.srv.PwForumService')->updateStatistics($fid, $value['thread'], $value['replies']);
			}
		}
		if (!$this->forum->isOpen()) {
			app('attention.PwFresh')->batchDeleteByType(PwFresh::TYPE_THREAD_TOPIC, $this->tids);

			//回复与新鲜事的关联
			if ($data = app('attention.PwFreshIndex')->fetchByTid($this->tids)) {
				app('attention.PwFresh')->batchDelete(array_keys($data));
			}
		}
		//管理日志添加
		app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'move', $this->srv->getData(), $this->_reason, $this->fid . '|' . $this->topictype);
	}
