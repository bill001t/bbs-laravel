...
class PwThreadManageDoShield extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (!$this->srv->user->comparePermission(Tool::collectByKey($this->srv->data, 'created_userid'))) {
			return new ErrorBag('permission.level.shield', array('{grouptitle}' => $this->srv->user->getGroupInfo('name')));
		}
		return true;
	}
	...
	public function run() {
		...
		if ($this->pids) {
			...
			if (!$this->tids) {
				app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, $type, $this->threads['p'], $this->_reason, '' , true);
			}
		}
		...
		if ($this->tids) {
			...
			//帖子的屏蔽处理
			app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, $type, $this->threads['t'], $this->_reason);
		}
	}
