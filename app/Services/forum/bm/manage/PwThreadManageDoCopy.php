...
class PwThreadManageDoCopy extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (isset($this->fid)) {
			...
			if (!$forum->isForum()) {
				return new ErrorBag('BBS:manage.error.copy.targetforum');
			}
			...
			if ($forum->forumset['topic_type'] && $forum->forumset['force_topic_type'] && !$this->topictype) {
				$topicTypes = app('SRV:forum.PwTopicType')->getTypesByFid($forum->fid);
				if ($topicTypes) {
	...
	public function run() {
		foreach ($this->threads as $v) {
			...
			if ($tid) {
				...
				app('SRV:forum.PwForum')->updateForum($forumDm);
			}
		}

		//管理日志添加
		app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'copy', $this->threads, $this->_reason, $this->fid . '|' . $this->topictype);
	}
