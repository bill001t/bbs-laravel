...
class PwThreadManageDoType extends PwThreadManageDo {
	...
	public function getTopicTypes(){
		...
		return app('forum.srv.PwTopicTypeService')->getTopictypes($forumset['fid']);
	}
	...
	public function run() {
		...
		app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'type', $this->threadsInfo, $this->_reason, $this->topictype);
	}
	
	/**
	 * Enter description here ...
	 *
	 * @return ErrorBag|boolean|Ambigous <number, unknown>
	 */
	private function _getForum(){
		$fid = 0;
		foreach ($this->threadsInfo as $v) {
			if ($fid && $v['fid'] != $fid) return new ErrorBag('MANAGE.multiple_forums_typed');
			$fid = $v['fid'];
		}
		if (!$fid) return true;
	...
	private function checkTopicType() {
		...
		if (!array_key_exists($this->topictype, $topicTypes)) {
			return new ErrorBag('BBS:post.topictype.error');
