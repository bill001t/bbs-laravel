...
class PwThreadManageDoBan extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if ($_tmp && false === $this->canBan($_tmp)) {
			return new ErrorBag('USER:ban.banuser.forbidden');
		}
		return true;
	}
	...
	public function run() {
		...
		$service = app('user.srv.PwUserBanService');
		...
		$this->_delThreads();
		app('log.srv.PwLogService')->addBanUserLog($this->loginUser, $this->banInfo->uids, $this->banInfo->types, $this->banInfo->reason, $this->banInfo->end_time);
		return true;
	}
	...
	public function getBanUsers() {
		...
		$this->selectBanUsers = app('user.PwUser')->fetchUserByUid($users);
	...
	public function setBanUids($uids) {
		$this->selectBanUsers = app('user.PwUser')->fetchUserByUid(is_array($uids) ? $uids : array($uids));
		return $this;
	}
	...
	public function canBan($groupid) {
		$systemGroups = app('usergroup.PwUserGroups')->getGroupsByType('system');
	...
	private function _buildBanDm() {
		...
		if ($this->banInfo->end_time > 0) $this->banInfo->end_time = Tool::str2time($this->banInfo->end_time);
		$data = $_notice = array();
