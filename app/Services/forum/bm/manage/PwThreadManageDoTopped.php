...
class PwThreadManageDoTopped extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (array_diff(Tool::collectByKey($this->srv->data, 'topped'), array('0'))) {
			$log = app('log.PwLog')->fetchLogByTid(array_keys($this->srv->data), array('19', '20', '21'));
			if (!$this->srv->user->comparePermission(Tool::collectByKey($log, 'created_userid'))) {
				return new ErrorBag('permission.level.topped', array('{grouptitle}' => $this->srv->user->getGroupInfo('name')));
			}
		}
		if ($this->topped) {
			if ($this->topped > $permission['topped_type']) {
				return new ErrorBag('BBS:manage.error.topped.permission.' . $this->topped);
			}
		}
		return true;
	}
	...
	public function setOvertime($overtime) {
		if ($overtime) {
			$this->overtime = is_numeric($overtime) ? $overtime : Tool::str2time($overtime) + 86399;
		}
	}
	...
	public function run() {
		...
		foreach ($this->tids as $fid => $tids) {
			...
			if ($this->topped == 1) {
				...
				$fids = $this->fids ? $this->fids : array_keys(app('forum.srv.PwForumService')->getForumList());
			} else {
				$fids = array();
			}
		...
		app('forum.PwThread')->batchUpdateThread($_tids, $topicDm, PwThread::FETCH_MAIN);

		if ($this->overtime) {
			if ($this->overids) {
				$topicDm = new PwTopicDm(true);
				$topicDm->setOvertime($this->overtime);
				app('forum.PwThread')->batchUpdateThread($this->overids, $topicDm, PwThread::FETCH_MAIN);
			}
		...
		$sortDs = app('forum.PwSpecialSort');
	...
	public function _getCateList($fid) {
		$array = array();
		$list = app('forum.srv.PwForumService')->getForumList();
		$pa = $list[$fid];
