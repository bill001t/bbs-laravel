...
class PwThreadManageDoDeleteReply extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (!$this->srv->user->comparePermission(Tool::collectByKey($this->srv->data, 'created_userid'))) {
			return new ErrorBag('permission.level.delete', array('{grouptitle}' => $this->srv->user->getGroupInfo('name')));
		}
		return true;
	}
	...
	public function run() {
		if ($this->pids) {
			...
			$service1->setRecycle(true)
				...
				->execute();
			//删除帖子回复
			app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'delete', $service1->data, $this->_reason, '', true);
		}
		if ($this->tids) {
			...
			$service2->setRecycle(true)
				...
				->execute();
			//删除帖子
			app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'delete', $service2->data, $this->_reason);
		}
	}
