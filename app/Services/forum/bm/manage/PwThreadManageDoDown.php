...
class PwThreadManageDoDown extends PwThreadManageDo {
	...
	public function check($permission) {
		...
		if (!$this->srv->user->comparePermission(Tool::collectByKey($this->srv->data, 'created_userid'))) {
			return new ErrorBag('permission.level.down', array('{grouptitle}' => $this->srv->user->getGroupInfo('name')));
		}
		return true;
	}
	...
	public function run() {
		...
		app('forum.PwThread')->batchUpdateThread($this->tids, $dm, PwThread::FETCH_MAIN);
		
		app('log.srv.PwLogService')->addThreadManageLog($this->srv->user, 'down', $this->srv->getData(), $this->_reason, $this->downtime);
	}
}