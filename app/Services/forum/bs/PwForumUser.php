...
use App\Core\Tool;
use App\Services\forum\ds\dao\PwForumUserDao;
...
class PwForumUser
{
	...
	public function join($uid, $fid, $time = 0) {
		...
		return $this->_getDao()->add(array('uid' => $uid, 'fid' => $fid, 'join_time' => $time ? $time : Tool::getTime()));
	}
	...
	public function quit($uid, $fid) {
		if (!$uid || !$fid) return false;
		return $this->_getDao()->_delete($uid, $fid);
	}
	...
	protected function _getDao() {
		if(is_null(self::$_dao)){
			return self::$_dao = new PwForumUserDao();
		}
		return self::$_dao;
	}
}