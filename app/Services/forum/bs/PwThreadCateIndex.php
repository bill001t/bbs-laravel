...
use App\Services\forum\ds\dao\PwThreadsCateIndexDao;
...
class PwThreadCateIndex
{
    ...
    public function fetch($cid, $limit = 20, $offset = 0, $order = 'lastpost')
    {
        ...
        $result = $this->_getDao()->fetch($cid, $order, $limit, $offset);
        return $result->pluck('tid')->all();
    }
    ...
    protected function _getDao()
    {
        if (is_null(self::$_dao)) {
            return self::$_dao = new PwThreadsCateIndexDao();
        }
        return self::$_dao;
    }
}