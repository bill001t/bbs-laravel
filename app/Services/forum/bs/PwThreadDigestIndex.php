...
use App\Services\forum\dm\PwThreadDigestDm;
use App\Services\forum\ds\dao\PwThreadsDigestIndexDao;
...
class PwThreadDigestIndex
{
     ...
     * @return array
     */
    public function getThreadsByCid($cid, $limit = 20, $offset = 0, $order = 'lastpost_time')
    {
    ...
    public function batchAddDigest(array $dms)
    {
        ...
        foreach ($dms as $_dm) {
            if (!$_dm instanceof PwThreadDigestDm) {
                return new ErrorBag('BBS:digest.dm.data.format.error');
            }
            if (($r = $_dm->beforeAdd()) instanceof ErrorBag) {
                return $r;
            }
            $_data = $_dm->getData();
            $_data['tid'] = $_dm->tid;
    ...
    protected function _getDao()
    {
        if (is_null(self::$_dao)) {
            return self::$_dao = new PwThreadsDigestIndexDao();
        }
        return self::$_dao;
    }
}