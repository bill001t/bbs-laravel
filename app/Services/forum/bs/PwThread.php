...
use App\Services\forum\ds\dao\PwPostsDao;
use App\Services\forum\ds\dao\PwThreadMergeDao;
use App\Services\forum\ds\dao\PwThreadsDao;
use App\Services\forum\ds\dao\PwThreadsHitsDao;
use App\Services\forum\vo\PwPostSo;
use App\Services\forum\vo\PwThreadSo;
...
class PwThread
{
    ...
    {
        return $this->_getThreadDao()->getThreadByFid($fid, $perpage);
    }

    public function fetchThreadByTid($tids, $perpage)
    {
        return $this->_getThreadDao()->fetchThreadByTid($tids, $perpage);
    }
    ...
    {
        return $this->_getThreadHitsDao()->_update(intval($tid), intval($hits));
    }
    ...
    {
        $fetchmode == self::FETCH_MAIN && $dao = \App\Services\forum\ds\dao\PwThreadsDao::class;
        $fetchmode == self::FETCH_CONTENT && $dao = \App\Services\forum\ds\dao\PwThreadsContentDao::class;
        $fetchmode == self::FETCH_ALL && $dao = \App\Services\forum\ds\dao\PwThreadsAllDao::class;
    ...
    {
        return app(PwThreadMergeDao::class);
    }

    protected function _getPostDao()
    {
        return app(PwPostsDao::class);
    }

    protected function _getThreadHitsDao()
    {
        return app(PwThreadsHitsDao::class);
    }
}