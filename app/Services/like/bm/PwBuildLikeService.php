...
class PwBuildLikeService {
	...
	public function getFollowLogList($uid, $start = 0, $limit = 10){
		$follows = app('attention.PwAttention')->getFollows($uid, 100, 0);
	...
	private function _buildLikeContent($content) {
		//$content = Tool::stripWindCode($content);
		//return Tool::substrs($content,140);
		$errcode = array();
	...
	protected function _bulidContent($content, $ubb, &$errcode) {
		...
		} elseif (Tool::strlen($content) > 140) {
			$errcode['is_read_all'] = true;
			$content = Tool::substrs($content, 140);
		}
		//var_dump($content);
		//$content = Security::escapeHTML($content);
	...
	private function _getDataFromPost($infoids) {
		$_aPid = array(); //有附件的回复
		$datas = $data = array();
		$infos = app('forum.PwThread')->fetchPost($infoids);
		foreach ($infos AS $info) {
			...
			$data['url'] =  url('bbs/read/run', array(
				'tid' => $info['tid'],
				'fid' => $info['fid'],
				'pid' => $info['pid']
			), 'read_' . $info['pid']);
		...
		foreach ($images AS $img) {
			$datas[$img['pid']]['image'] = Tool::getPath($img['path'], $img['ifthumb']);
		}
		return $datas;
	}
	...
	private function _getDataFromThread($infoids) {
		$_aTid = array(); //有附件的帖子
		$datas = $data = array();
		$infos = app('forum.PwThread')->fetchThread($infoids, PwThread::FETCH_ALL);
		foreach ($infos AS $info) {
			$data['subject'] = $info['subject'];
			$data['lasttime'] = $info['created_time'];
			$data['content'] = $this->_buildLikeContent($info['content']);
			...
			$data['fid'] = $info['fid'];
			$data['like_count'] = $info['like_count'];
			$data['url'] =  url('bbs/read/run', array('tid' => $info['tid'], 'fid' => $info['fid']));
			$this->_appendFid($info['fid']);
		...
		foreach ($images AS $img) {
			$datas[$img['tid']]['image'] = Tool::getPath($img['path'], $img['ifthumb']);
		}
		return $datas;
	}
	...
	private function _getDataFromWeiBo($infoids) {
		$datas = $data = array();
		$infos = app('weibo.PwWeibo')->getWeibos($infoids);
		foreach ($infos AS $info) {
			...
			$data['username'] = $info['created_username'];
			$data['like_count'] = $info['like_count'];
			$data['url'] =  url('space/index/fresh', array('typeid' => 3, 'id' => $info['weibo_id'], 'uid' => $info['created_userid']));
	...
	private function _getDataFromApp($infoids) {
		$datas = $data = array();
		$infos = app('like.PwLikeSource')->fetchSource($infoids);
		foreach ($infos AS $info) {
			$data['subject'] = $info['subject'];
			$data['from'] = 'App';
			$data['like_count'] = $info['like_count'];
	...
	private function _getReplyFromPost($infoids) {
		...
		foreach ($infos AS $info) {
			...
			$data['uid'] = $info['created_userid'];
			$data['username'] = $info['created_username'];
			$data['url'] =  url('bbs/read/run', array('tid' => $info['tid'], 'fid' => $info['fid'], 'pid' => $info['pid']), $info['pid']);
	...
	private function _getThreadAttachs($tids = array()) {
		$attachs = array();
		foreach ($tids AS $tid) {
			$_attachs = app('attach.PwThreadAttach')->getAttachByTid($tid, array(0));
	...
	private function _getPostAttachs($pids = array()) {
		...
		foreach ($pids AS $v) {
			list($tid, $pid) = $v;
			$_attachs = app('attach.PwThreadAttach')->getAttachByTid($tid, $pid);
			foreach ($_attachs AS $v) {
				if ($v['type'] == 'img') {
					$attachs[$pid] = $v; 
					break;
	...
	private function _bindForum() {
		$_forumsInfo = app('forum.PwForum')->fetchForum($this->_fids);
