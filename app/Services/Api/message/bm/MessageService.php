...
use App\Services\Api\message\dm\WindidMessageDm;
 ...
 * @version $Id: MessageService.php 24834 2013-02-22 06:43:43Z jieyin $
...
class WindidMessageService
{
    ...
    {
        $data = $this->_getUserDs()->getUserByUid($uid, PwUser::FETCH_DATA);
    ...
    public function sendMessage($username, $content, $fromUid)
    {
        ...
        if (!$userInfo) return new ErrorBag(ErrorBag::USER_NOT_EXISTS);
    ...
    public function sendMessageByUid($uid, $content, $fromUid = 0)
    {
        ...
        if ($dialog) {
            ...
            $dm->increaseMessageCount()->setModifiedTime(Tool::getTime());
        ...
        if ($dialog) {
            $dialogId = $dialog['dialog_id'];
            $dm->increaseUnreadCount()->increaseMessageCount()->setModifiedTime(Tool::getTime());
            $this->_getMessageDs()->updateDialog($dialogId, $dm);
        } else {
            // 分组不存在添加一条
            $dm->setToUid($uid)
    ...
    public function sendMessageByUsernames($usernames, $content, $from_uid = 0)
    {
        ...
        if (!$userInfos) {
            return new ErrorBag(USER_NOT_EXISTS);
        }
    ...
    {
        if (!$content) {
            return new ErrorBag(MESSAGE_CONTENT_LENGTH_ERROR);
        }
    ...
    public function _delete($uid, $dialogId, $messageIds = array())
    {
