...
namespace App\Services\Api\config\bs;

use App\Services\Api\config\ds\dao\WindidConfigDao;
...
class WindidConfig
{
    ...
    {
        $config = $this->_getDao()->getConfigs($namespace)->keyBy('name');
        $clear = array();
        foreach ($config as $key => $item) {
            $clear[$item['name']] = $item['vtype'] != 'string' ? unserialize($item['value']) : $item['value'];
        }
        return $clear;
    }
    ...
    {
        return app(WindidConfigDao::class);
    }
}
