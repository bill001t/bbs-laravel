...
namespace App\Services\Api\user\validator;
...
class WindidUserValidator
{
    ...
    public static function checkName($username, $uid = 0, $checkUsername = '')
    {
        ...
        if (self::isNameLenValid($username, 'UTF-8')) return new WindidError(WindidError::NAME_LEN);
    ...
    {
        $len = mb_strlen($username, $charset);
    ...
    private static function isEmailValid($email)
    {
        ...
        if (false === CommonValidator::isEmail($email)) return new WindidError(WindidError::EMAIL_ILLEGAL);
    ...
    private static function getConfig($name = '')
    {
        static $config = null;
        if (null === $config) {
            $config = app(\App\Core\WindidBo::class)->config->reg->toArray();
        }
