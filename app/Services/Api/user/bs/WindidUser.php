...
use App\Services\Api\user\ds\dao\WindidUserSearchDao;
use App\Services\Api\user\vo\WindidUserSo;
...
class WindidUser
{
    ...
    private function _filterIds($id)
    {
        ...
        $clearIds = array();
        foreach ($id as $item) {
            if (CommonValidator::isPositive($item)) $clearIds[] = $item;
        }
        return $clearIds;
    }
    ...
    private function _getDao($type = self::FETCH_MAIN)
    {
        ...
        $type == self::FETCH_MAIN && $dao = \App\Services\Api\user\ds\dao\WindidUserDao::class;
        $type == self::FETCH_DATA && $dao = \App\Services\Api\user\ds\dao\WindidUserDataDao::class;
        $type == self::FETCH_INFO && $dao = \App\Services\Api\user\ds\dao\WindidUserInfoDao::class;
        empty($dao) && $dao = \App\Services\Api\user\ds\dao\WindidUserAllDao::class;
    ...
    {
        return app(WindidUserSearchDao::class);
    }
}