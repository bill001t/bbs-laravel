...
namespace App\Services\Api\area\bm;

use App\Services\Api\area\bs\Area;
use Storage;
...
class AreaService
{
    ...
    {
        $info = $this->_getAreaDs()->getArea($areaid);
    ...
    public function getAreaRout($areaid)
    {
        ...
        while ($parentid > 0) {
            $_info = $this->_getAreaDs()->getArea($parentid);
    ...
    {
        if (!$list = $this->_getAreaDs()->fetchByAreaid($areaids)) {
            return array();
        }
    ...
    {
        $list = $this->_getAreaDs()->fetchByAreaid($areaids);
        ...
        $list = $this->_getAreaDs()->fetchByAreaid(array_keys($parents));
        foreach ($list as $key => $_item) {
    ...
    {
        $areas = $this->_getAreaDs()->fetchAll();
    ...
    {
        $list = $this->_getAreaDs()->getAreaByParentid($areaid);
        $return = array();
    ...
    {
        $data = $this->_getAreaDs()->fetchAll();

        Storage::put('/area/area.php', $data);
    ...
    {
        /* $file = Wind::getRealPath('DATA:area.area.php', true);*/
        if (Storage::exists('/area/area.php')) return Storage::get('/area/area.php');
